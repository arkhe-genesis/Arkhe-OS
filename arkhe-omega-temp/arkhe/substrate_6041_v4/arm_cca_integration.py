...
log = logging.getLogger("arkhe.arm_cca")
...
class OPTEEBridge:
    ...
    def open_session(self, ta_uuid: str = None) -> bool:
        ...
        try:
            ...
            log.warning("⚠️  libteec não disponível — OP-TEE bridge em modo simulação")
            self._session = False
            return False
    ...
    def _generate_ta_uuid(self) -> str:
        ...
        return str(uuid.UUID(int=hashlib.sha256(
...
class PlatformTEE:
    ...
    @staticmethod
    def _check_sgx() -> bool:
        return os.path.exists("/dev/sgx_enclave") or os.path.exists("/dev/sgx/enclave")

    @staticmethod
    def _check_sev_snp() -> bool:
        return os.path.exists("/dev/sev-guest")

    @staticmethod
    def _check_arm_cca() -> bool:
        # Check for ARM CCA specific device nodes (e.g. /dev/arm-cca-guest)
        # This is a placeholder as actual CCA dev nodes might differ
        return os.path.exists("/dev/arm-cca-guest")

    @staticmethod
    def _check_optee() -> bool:
        return os.path.exists("/dev/tee0")
    ...
    def detect(cls) -> 'PlatformTEE':
        ...
        if machine in ('x86_64', 'amd64'):
            # Verificar SGX primeiro
            if cls._check_sgx():
                return PlatformTEE("x86_sgx")
            if cls._check_sev_snp():
                return PlatformTEE("amd_sev")
            return PlatformTEE("none")

        if machine in ('aarch64', 'arm64'):
            if cls._check_arm_cca():
                return PlatformTEE("arm_cca")
            if cls._check_optee():
                return PlatformTEE("optee")
            return PlatformTEE("none")
    ...
    def __init__(self, platform_name: str):
        ...
        self.available_backends = [self.platform]
