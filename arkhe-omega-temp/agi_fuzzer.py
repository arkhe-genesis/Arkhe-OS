...
def generate_seed_corpus(output_dir: str):
    ...
    partial = json.dumps({"name": "test"}).encode()
...
class AGIFuzzer:
    ...
    def test_property_header_malformed(self, iterations: int = 10000):
        ...
        for i in range(iterations):
            ...
            magic = os.urandom(4)
            version = random.randint(0, 65535)
    ...
    def test_property_signature_malleability(self, iterations: int = 5000):
        ...
        for i in range(iterations):
            ...
            mutations = [
                lambda d: (d.__setitem__(slice(-256, None), os.urandom(256)), d)[1],
                lambda d: (d.__setitem__(slice(0, 1280), os.urandom(1280)), d)[1],
                lambda d: d,  # No mutation
            ]
