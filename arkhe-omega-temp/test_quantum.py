...
def t1_detection():
    ...
    assert oracle._is_quantum_negative_time(1.0) == False
test("Limiares de detecção", t1_detection)
...
test("Tempo negativo quântico aceito", t2_quantum_acceptance)
...
test("Tempo positivo normal", t3_positive)
...
def t4_classical_penalized():
    ...
    assert not r.quantum_coherent
    assert r.score < 1.0
test("Tempo negativo clássico penalizado", t4_classical_penalized)
...
test("Paradoxo do avô detectado", t5_grandfather)
...
def t6_extreme():
    ...
    assert r.score < 0.5
    assert not r.quantum_coherent
test("Tempo negativo extremo", t6_extreme)
...
test("Janela configurável por instância", t8_configurable)
