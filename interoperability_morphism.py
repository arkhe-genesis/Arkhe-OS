...
class CasperEstimate:
    ...
    # Adding fields for arkhify_casper_consensus compatibility
    @property
    def hash(self):
    ...
    @property
    def payload(self):
        return {"epoch": self.justification_epoch, "score": self.score}

    @property
    def finalized_epoch(self):
        return self.justification_epoch
...
class ArkheToCasperMorphism:
    ...
    def direct_image_message(self, arkhe_msg: 'TemporalMessage') -> CasperEstimate:
        ...
        checkpoint = hashlib.sha3_256(
            f"{arkhe_msg.sender}:{arkhe_msg.content}".encode()
        ).hexdigest()[:16]
...
class CrossConsensusBridge:
    ...
    def import_external_message(self, external_msg) -> Optional['TemporalMessage']:
        ...
        if report.consistent and report.score >= 0.90:
    ...
    def export_arkhe_message(self, arkhe_msg: 'TemporalMessage'):
        ...
        if self.morphism.oracle_natural_transformation(
            arkhe_msg, arkhe_report.score, self.external_oracle
        ):
            return external_est
        return None
...
def arkhify_casper_consensus(casper_proposition, arkh_estimator):
    ...
    mapped_state = TemporalMessage(
        ...
        content=json.dumps(casper_proposition.payload),
        source_timestamp=time.time(),
        target_timestamp=casper_proposition.finalized_epoch,
