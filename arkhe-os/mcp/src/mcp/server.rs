...
async fn handle_execute_workload(
    ...
    // 2. Validação do provenance (deve ser um dos valores permitidos)
    ...
    // 3. Obter IdentityAttestation (com força de refresh se necessário)
    let identity = state
        .identity_provider
    ...
    // 4a. Verificar confiança mínima (0.7)
    ...
    // 4b. Verificar se a identidade está verificada
    ...
    // 4c. Verificar expiração do atestado (se tiver TTL definido)
    ...
    // 5. Verificar se a assinatura do Arquiteto é válida (se houver verifier configurado)
    ...
    // 6. Verificar políticas ativas (list_policies) e conferir compatibilidade
    ...
    // Se houver políticas que bloqueiam o workload, retorna erro
    ...
    // 7. Executa o workload com autorização
    ...
    // 8. Validação pós-execução (se a execução gerou um atestado válido)
    ...
    // 9. Registra o atestado no AttestationManager (persistência)
    state
        .attestation_manager
    ...
    // 10. Retorna o atestado completo (em produção, pode-se retornar apenas o ID)
...
pub async fn start_mcp_server(
    ...
    let app = Router::new()
        ...
        .route("/version", axum::routing::get(|| async { "2.0" }))
        .layer(axum::middleware::from_fn_with_state(
            auth_config.clone(),
            |State(config): State<Arc<AuthConfig>>, req: axum::extract::Request, next: axum::middleware::Next| async move {
