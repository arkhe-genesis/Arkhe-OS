...
class PermanentEgregore:
    ...
    def __init__(self, name: str, founder_seal: str, coherence_monitor,
        ...
        self.consensus = ByzantineResistantConsensus()
    ...
    def decide(self, proposal_id: str) -> Tuple[bool, float]:
        ...
        membership_check = self.consensus.validate_membership(self.members)
        if not membership_check.get('valid', True):
            return False, 0.0

        result = self.consensus.weighted_consensus(proposal, self.members, proposal['votes'])
        ...
        return decision['approved'], decision['consensus_score']
