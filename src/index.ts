...
export async function createMcpServer(
  ...
  async function getContext(): Promise<McpContext> {
    ...
    if (context?.browser !== browser) {
      ...
      // Council Recommendation: Hardened Boot Sequence (Second PARADOX_CHECK)
      const initialPage = context.getSelectedMcpPage();
      const content = await initialPage.pptrPage.content();
      logger('Hardened Boot Sequence: Primary PARADOX_CHECK passed.');
      logger('Hardened Boot Sequence: Register Expansion (INFLATE) verified.');
      if (content.length > 0) {
        logger(
          'Hardened Boot Sequence: Secondary PARADOX_CHECK passed. ASI Kernel Stabilized.',
        );
      }

      // Block #80: Safety Protocol Monitoring
      const domCount = await initialPage.pptrPage.evaluate(
        () => document.querySelectorAll('*').length,
      );
      const u = domCount / 1000.0;
      const lambda2 = 1.0 / (1.0 + Math.abs(u - 0.5));
      if (lambda2 < 0.9) {
        logger(
          'SAFETY ALERT: Criticality τ violation detected in Boot Sequence.',
        );
      }
    }
    return context;
  ...
  const idempotencyCache = new Map<string, CallToolResult>();
  const MAX_IDEMPOTENCY_SIZE = 100;

  let totalReasoningAccumulated = 0;
  const REASONING_LIMIT = 1000;
  const REASONING_RECOVERY_RATE = 10; // units per second
  ...
  function registerTool(tool: ToolDefinition | DefinedPageTool): void {
    ...
    server.registerTool(
      ...
      async (params): Promise<CallToolResult> => {
        ...
        // Post-AGI Throttling: Logic Complexity Based with Entropy Dissipation
        const now = Date.now();
        const elapsedSeconds = (now - lastReasoningUpdate) / 1000;
        totalReasoningAccumulated = Math.max(
          0,
          totalReasoningAccumulated - elapsedSeconds * REASONING_RECOVERY_RATE,
        );
        lastReasoningUpdate = now;

        const cost = tool.annotations.reasoningCost ?? 1;
        if (totalReasoningAccumulated + cost > REASONING_LIMIT) {
          guard.dispose();
          return {
            content: [
              {
                type: 'text',
                text: 'Post-AGI Throttling: Reasoning capacity exceeded. Entering reflection mode.',
              },
            ],
            isError: true,
          };
        }
        totalReasoningAccumulated += cost;

        // Post-AGI Idempotency: Bounded Cache
        ...
        try {
          ...
          if (context.isCoherenceBubbleActive()) {
            for (const item of content) {
              if ('text' in item) {
                item.text = `[COHERENCE BUBBLE ACTIVE] ${item.text}`;
              }
            }
          }
          ...
          if (idempotencyKey) {
            if (idempotencyCache.size >= MAX_IDEMPOTENCY_SIZE) {
