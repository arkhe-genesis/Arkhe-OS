...
export default function HybridNetworkPanel({
  state,
  onClose,
}: HybridNetworkPanelProps) {
  if (!state) {
    return null;
  }
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-black/40 p-3 rounded border border-white/5 space-y-1">
              ...
              <div className="text-lg font-bold text-arkhe-text font-mono">
                {state.sapphireNanoholes.toLocaleString()}
              </div>
            </div>
            <div className="bg-black/40 p-3 rounded border border-white/5 space-y-1">
              <div className="text-[9px] text-arkhe-muted uppercase">
              ...
              <div className="text-lg font-bold text-emerald-500 font-mono">
                {state.hybridCoherenceTimeMs} ms
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center p-12 bg-black/60 border border-arkhe-border rounded-lg relative overflow-hidden min-h-[300px]">
            ...
            <div className="relative w-64 h-64">
              ...
              <div className="absolute inset-12 border border-arkhe-cyan/50 rounded-lg flex flex-col gap-2 p-4 bg-arkhe-cyan/5">
                {Array.from({length: 5}).map((_, i) => (
                  <div
                    key={i}
                    className="h-2 w-full bg-arkhe-cyan/20 rounded-sm"
                  />
                ))}
              </div>
            ...
            <div className="mt-8 text-center">
              <div className="text-xs font-mono text-arkhe-text uppercase font-bold">
                Evanescent Coupling Interface
              </div>
              <div className="text-[10px] font-mono text-arkhe-muted">
                Coupling Efficiency:{' '}
                {(state.couplingEfficiency * 100).toFixed(2)}%
              </div>
            </div>
          </div>
