  ...
  useEffect(() => {
    ...
    const mockAlerts: Alert[] = [
      {
        ...
        message:
          'A rede está operando com alta coerência (0.98). O tempo de geração de provas ZK caiu 12% na última hora, indicando otimização no prover Plonky3. Nenhuma anomalia de consenso detectada.',
      },
      {
        id: '2',
        ...
        message:
          'A rede entrou na Fase Solar. O multiplicador de recompensa base foi ajustado.',
      },
      {
        id: '3',
        ...
        message:
          'Aumento de 300% no volume de transações zkERC detectado no último bloco.',
      },
    ];
    ...
    const interval = setInterval(() => {
      const newAlert: Alert = {
        ...
        message:
          'Verificação de integridade do bloco concluída. Prova STARK validada em 45ms.',
      };
      ...
      <div className="bg-[#111214] border border-[#1f2024] rounded-xl w-full max-w-4xl h-[80vh] flex flex-col shadow-2xl overflow-hidden relative">
        ...
        <div className="p-6 border-b border-[#1f2024] flex items-center justify-between bg-black/40">
          ...
          <div className="flex items-center space-x-2 mr-8">
            ...
            <div
              className={`flex items-center space-x-2 px-3 py-1 rounded-full text-xs font-medium border ${isLive ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-red-500/10 text-red-400 border-red-500/30'}`}
            >
              <div
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-black/20">
          {alerts.map(alert => (
            <div
              key={alert.id}
              className={`p-4 rounded-lg border ${getLevelColor(alert.level)} transition-all hover:bg-opacity-20`}
            >
