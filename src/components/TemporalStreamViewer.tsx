...
export default function TemporalStreamViewer({
  onClose,
}: TemporalStreamViewerProps) {
  ...
  useEffect(() => {
    ...
    if (shaka.Player.isBrowserSupported()) {
      ...
      newPlayer.addEventListener('error', (event: any) => {
        ...
        logger.error(
          `Error code ${detail.code} object ${JSON.stringify(detail)}`,
        );
  ...
  const toggleVrMode = () => {
    if (player) {
      ...
      setVrMode(newVrMode);
      logger.info(
        `🜏 [PERCEPTION] VR Exploration Mode ${newVrMode ? 'ENABLED' : 'DISABLED'}.`,
      );
    }
  };
              ...
              <>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/90 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="text-white hover:text-cyan-400 transition-colors"
                    title="Capture Frame for Ouroboros"
                  >
                    <Eye className="w-5 h-5" />
                  </button>
                  <button
                    onClick={toggleVrMode}
                    className={`transition-colors ${vrMode ? 'text-cyan-400' : 'text-white hover:text-cyan-400'}`}
                    title="Toggle VR Exploration Mode"
                  >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-[#111214] border border-arkhe-border rounded p-2">
                  ...
                  <div className="text-xs font-mono text-cyan-400">
                    {formatBitrate(stats.estimatedBandwidth as number)}
                  </div>
                </div>
                <div className="bg-[#111214] border border-arkhe-border rounded p-2">
                  <div className="text-[9px] font-mono text-arkhe-muted uppercase">
                  ...
                  <div className="text-xs font-mono text-cyan-400">
                    {(stats.width as number) || 0}x
                    {(stats.height as number) || 0}
                  </div>
                </div>
                <div className="bg-[#111214] border border-arkhe-border rounded p-2">
                  <div className="text-[9px] font-mono text-arkhe-muted uppercase">
              ...
              <div className="bg-[#111214] border border-arkhe-border rounded p-3 space-y-2">
                ...
                <div className="flex justify-between items-center border-b border-arkhe-border/50 pb-1">
                  <span className="text-[10px] font-mono text-arkhe-text">
                    DASH Stream
                  </span>
                  <span className="text-[10px] font-mono text-cyan-400">
                    {stats.streamBandwidth
                      ? formatBitrate(stats.streamBandwidth as number)
                      : 'Auto'}
                  </span>
                </div>
                <div className="flex justify-between items-center border-b border-arkhe-border/50 pb-1">
                  ...
                  <span
                    className={`text-[10px] font-mono ${vrMode ? 'text-emerald-400' : 'text-arkhe-muted'}`}
                  >
                    {vrMode ? 'ENABLED' : 'DISABLED'}
                  </span>
                </div>
              ...
              <div className="bg-cyan-500/5 border border-cyan-500/20 rounded p-3">
                ...
                <p className="text-[10px] font-mono text-arkhe-muted leading-relaxed">
                  Dimensional Adaptive Stream Handling (DASH) engaged. Adapting
                  reality stream quality based on Tzinor channel coherence.
                  Frames are actively captured for Ouroboros pattern
                  recognition.
                </p>
              </div>
            </div>
