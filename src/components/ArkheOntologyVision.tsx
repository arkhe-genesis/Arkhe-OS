...
export default function ArkheOntologyVision({onClose}: {onClose: () => void}) {
  ...
  const handleZKChallenge = async () => {
    ...
    setNotification({
      type: 'info',
      message: 'Iniciando Desafio ZK (Commit-and-Prove)...',
    });
    ...
    try {
      ...
      setNotification({
        type: 'error',
        message: 'Erro no protocolo de privacidade ZK.',
      });
    } finally {
      setIsProving(false);
  ...
  useEffect(() => {
    ...
    gl.attachShader(
      program,
      createShader(gl, gl.FRAGMENT_SHADER, FRAGMENT_SHADER),
    );
    ...
    const render = (time: number) => {
      ...
      if (nodes.length > 0) {
        ...
        gl.uniform3fv(
          nodePosLoc,
          new Float32Array(nodes.slice(0, nodeLimit).flatMap(n => n.position)),
        );
        gl.uniform4fv(
          nodeColLoc,
          new Float32Array(nodes.slice(0, nodeLimit).flatMap(n => n.color)),
        );
        gl.uniform1fv(
          nodeSizeLoc,
          new Float32Array(nodes.slice(0, nodeLimit).map(n => n.size)),
        );
        gl.uniform1iv(
          nodeSecLoc,
          new Int32Array(nodes.slice(0, nodeLimit).map(n => n.securityState)),
        );
      ...
      if (edges.length > 0) {
        ...
        gl.uniform2iv(
          ...
          new Int32Array(
            edges
              .slice(0, edgeLimit)
              .flatMap(e => [e.sourceIndex, e.targetIndex]),
          ),
        );
      ...
      <div className="w-full max-w-7xl bg-[#0a0a0c] border border-cyan-500/30 rounded-2xl shadow-[0_0_50px_rgba(0,255,170,0.2)] overflow-hidden flex flex-col h-[90vh]">
        <div className="flex items-center justify-between p-6 border-b border-cyan-500/20 bg-cyan-500/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-cyan-400 tracking-[0.2em] uppercase font-mono">
                Arkhe Immersive Defense
              </h2>
          ...
          <button
            onClick={onClose}
            className="group flex items-center gap-2 px-4 py-2 border border-white/10 rounded-lg text-neutral-500 hover:text-white hover:border-white transition-all"
          >
        ...
        <div className="flex-1 relative flex flex-col lg:flex-row overflow-hidden">
          <div className="flex-1 relative bg-black">
            ...
            {notification && (
              <div
                className={`absolute top-10 left-1/2 -translate-x-1/2 px-6 py-4 rounded-xl border flex items-center gap-4 animate-bounce z-50 backdrop-blur-xl ${
          ...
          <div className="w-full lg:w-96 bg-[#050507] border-l border-white/10 p-6 flex flex-col gap-6 overflow-y-auto">
            {selectedNode ? (
              <div className="space-y-6 animate-in slide-in-from-right">
                <div className="p-4 bg-white/5 border border-cyan-500/30 rounded-2xl space-y-4">
                  <h4 className="font-mono text-xs text-cyan-400 uppercase font-bold tracking-widest">
                    Inspeção de Objeto
                  </h4>
                  ...
                  <div className="flex items-center gap-2 text-[9px] text-neutral-400 uppercase font-mono">
                    <Lock className="w-3 h-3" /> URI Original Oculta por
                    Protocolo de Privacidade
                  </div>
                </div>

                <div className="space-y-3">
                  <h4 className="font-mono text-[10px] text-neutral-500 uppercase tracking-widest">
                    Ação: Auditoria de Integridade
                  </h4>
                  <button
                  ...
                  <p className="text-[8px] font-mono text-neutral-500 leading-tight">
                    A geração da prova ZK valida restrições SHACL (maxCount)
                    localmente sem expor a URI do recurso ao backend.
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-4 opacity-40">
                ...
                <p className="font-mono text-xs text-neutral-500 uppercase tracking-widest">
                  Selecione uma entidade no
                  <br />
                  universo 3D para interagir.
                </p>
              </div>
            )}
            ...
            <div className="mt-auto border-t border-white/5 pt-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="font-mono text-[10px] text-white uppercase font-bold">
                  Protocolo Commit-and-Prove
                </span>
              </div>
              <div className="p-3 bg-indigo-500/5 border border-indigo-500/20 rounded-lg text-[9px] font-mono text-neutral-400 leading-relaxed">
                As auditorias utilizam Compromissos de Pedersen e circuitos R1CS
                (Pinocchio) para garantir que apenas anomalias reais sejam
                reportadas, mantendo o sigilo da infraestrutura.
              </div>
            </div>
          </div>
        </div>
