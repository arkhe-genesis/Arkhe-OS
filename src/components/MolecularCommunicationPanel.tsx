...
const FRACTAL_SCALES = [
  {
    ...
    substrate: 'Quarks',
    field: 'Força Forte',
    engineering: 'Hadrônica Retrocausal',
  },
  {
    scale: 'Molecular',
    substrate: 'Moléculas',
    field: 'Difusão',
    engineering: 'Comunicação Molecular',
  },
  {
    scale: 'Celular',
    substrate: 'Microtúbulos',
    field: 'Bioquímica',
    engineering: 'Biologia Sintética',
  },
  {
    scale: 'Organísmica',
    substrate: 'Neurônios',
    field: 'Elétrica',
    engineering: 'Neurociência',
  },
  {
    scale: 'Social',
    substrate: 'Humanos',
...
export default function MolecularCommunicationPanel({
  onClose,
}: MolecularCommunicationPanelProps) {
  const [particles, setParticles] = useState<
    Array<{id: number; delay: number; duration: number; yOffset: number}>
  >([]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-mono text-sm font-bold text-cyan-400 uppercase tracking-widest">
                IoNT & Molecular Communication // 6G Hybrid Layer
              </h2>
              <div className="text-[10px] font-mono text-cyan-400/70">
                Prof. LIN Lin (Tongji University) Research Telemetry
              </div>
            </div>
          </div>
          <button
          ...
          <div className="flex-1 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="text-xs font-mono uppercase tracking-widest text-arkhe-muted">
                Canal de Difusão Molecular (Tzinor)
              </div>
            ...
            <div className="flex-1 bg-[#111214] border border-[#1f2024] rounded-xl p-6 relative overflow-hidden flex flex-col justify-center">
              ...
              <div className="mt-8 grid grid-cols-4 gap-4 text-center">
                <div className="bg-black/40 p-2 rounded border border-white/5">
                  <div className="text-[10px] font-mono text-arkhe-muted mb-1">
                    Fase ℂ
                  </div>
          ...
          <div className="w-full lg:w-96 flex flex-col gap-4 overflow-y-auto">
            ...
            <div className="bg-[#111214] border border-[#1f2024] rounded-xl p-4">
              ...
              <p className="text-[10px] font-mono text-arkhe-muted leading-relaxed mb-3">
                O atraso devido à difusão exige que a ação futura seja prevista
                e preparada no presente. O protocolo de sincronização atua como
                um Tzinor temporal entre nanomáquinas.
              </p>
            ...
            <div className="bg-[#111214] border border-[#1f2024] rounded-xl p-4 flex-1">
              ...
              <div className="space-y-2">
                ...
                {FRACTAL_SCALES.map((item, idx) => (
                  <div
                    key={idx}
                    className={`grid grid-cols-4 gap-2 text-[10px] font-mono p-1.5 rounded ${idx === 1 ? 'bg-cyan-400/10 border border-cyan-400/20 text-cyan-400' : 'text-arkhe-text hover:bg-white/5'}`}
                  >
                    <div className={idx === 1 ? 'font-bold' : ''}>
                      {item.scale}
                    </div>
              ...
              <div className="mt-6 p-3 bg-cyan-400/5 border border-cyan-400/20 rounded-lg">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-[10px] font-mono text-cyan-400/90 leading-relaxed">
                    A Internet das Nanocoisas (IoNT) forma uma malha ℤ cobrindo
                    a biologia. A comunicação molecular é a ponte entre a
                    biologia do Paramecium e a engenharia de redes 6G.
                  </p>
                </div>
              </div>
            </div>
