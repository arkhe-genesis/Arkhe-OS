...
export default function RiscViArchitecturePanel({
  state,
  onClose,
}: RiscViArchitecturePanelProps) {
  if (!state) {
    return null;
  }
      ...
      >
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-6 md:grid-cols-12 gap-1">
              {stages.map(s => (
                ...
                >
                  <div className="text-[7px] font-mono font-bold text-center leading-tight">
                    {s}
                  </div>
                  {state.pipelineStage === s && (
                    <motion.div
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(state.registers).map(([reg, val]) => (
                  <div
                    key={reg}
                    className="bg-white/5 border border-white/10 p-2 rounded flex justify-between items-center"
                  >
                    <span className="text-[10px] font-mono text-arkhe-cyan font-bold">
                      {reg}
                    </span>
                    <span className="text-[10px] font-mono text-arkhe-text truncate ml-2">
                      {val}
                    </span>
                  </div>
                ))}
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {state.activeIsaExtensions.map(ext => (
                  <div
                    key={ext}
                    className="w-8 h-8 rounded border border-arkhe-cyan/30 bg-arkhe-cyan/5 flex items-center justify-center font-mono text-xs font-bold text-arkhe-cyan"
                  >
                    {ext}
                  </div>
                ))}
              ...
              <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
                ...
                <div className="text-lg font-mono text-arkhe-text font-bold">
                  {state.lastOpcode}
                </div>
              </div>
            </div>
          ...
          <div className="flex gap-4">
            ...
            <button
              onClick={() =>
                fetch('/api/riscvi/execute', {
                  method: 'POST',
                  headers: {'Content-Type': 'application/json'},
                  body: JSON.stringify({opcode: 'QUBIT.GHZ'}),
                })
              }
        ...
        <div className="p-4 border-t border-white/5 bg-black/40 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-emerald-500 font-bold">
              {(state.invarianceMetric * 100).toFixed(6)}%
            </span>
          </div>
        </div>
      </motion.div>
