...
export default function NeuralMolecularBridgePanel({
  onClose,
}: NeuralMolecularBridgePanelProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const complexity =
        (lfPower + hfPower) / (Math.abs(lfPower - hfPower) + 1.0);
      ...
      const newPayload: ConsciousnessPayload = {
        ...
        timestamp: Date.now(),
        rawEntropy: Array.from({length: 8}, () =>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-rose-500/20 bg-rose-500/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-mono text-sm font-bold text-rose-400 uppercase tracking-widest">
                Neural-Molecular Bridge // Layer 7
              </h2>
              <div className="text-[10px] font-mono text-rose-400/70">
                MultiverseManager gRPC Ingestion Stream (iOS/watchOS)
              </div>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 flex flex-col lg:flex-row p-6 gap-6 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="text-xs font-mono uppercase tracking-widest text-arkhe-muted">
                Consciousness Payload Stream
              </div>
              <button
              ...
              >
                ...
                <span>
                  {isConnected ? 'gRPC STREAM ACTIVE' : 'gRPC STREAM PAUSED'}
                </span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-[#111214] border border-[#1f2024] rounded-xl p-4 relative overflow-hidden">
                ...
                <div className="text-[10px] font-mono text-arkhe-muted uppercase mb-2">
                  Heart Rate (Stochastic)
                </div>
                <div className="flex items-end gap-2">
                  <span className="text-4xl font-mono font-bold text-rose-400">
                    {latest ? latest.heartRate.toFixed(1) : '---'}
                  </span>
                  <span className="text-xs font-mono text-rose-400/50 mb-1">
                    BPM
                  </span>
                </div>
              </div>
              ...
              <div className="bg-[#111214] border border-[#1f2024] rounded-xl p-4 relative overflow-hidden">
                ...
                <div className="flex items-end gap-2">
                  <span
                    className={`text-3xl font-mono font-bold ${latest && latest.lfHfRatio > 1.5 ? 'text-orange-400' : 'text-emerald-400'}`}
                  >
                    {latest ? latest.lfHfRatio.toFixed(2) : '---'}
                  </span>
                  <span className="text-xs font-mono text-arkhe-muted mb-1">
                    Ratio
                  </span>
                </div>
                <div className="text-[9px] font-mono text-arkhe-muted mt-2">
                  {latest && latest.lfHfRatio > 1.5
              ...
              <div className="bg-[#111214] border border-[#1f2024] rounded-xl p-4 relative overflow-hidden">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <span className="text-xs font-mono text-cyan-400/50 mb-1">
                    IIT
                  </span>
                </div>
                <div className="text-[9px] font-mono text-arkhe-muted mt-2">
                ...
                {payloads.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="text-rose-400">
                      HR:{p.heartRate.toFixed(1)} | HRV:{p.hrvSdnn.toFixed(1)} |
                      LF/HF:{p.lfHfRatio.toFixed(2)}
                    </div>
