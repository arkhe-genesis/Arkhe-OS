...
export default function SecurityAdvancedPanel({
  onClose,
}: SecurityAdvancedPanelProps) {
  const state: SimulationState = useArkheSimulation();
  const [activeTab, setActiveTab] = useState<
    'l1' | 'l2' | 'l3' | 'l4' | 'l5' | 'qhttp'
  >('l1');
  ...
  const LayerStatus = ({
    ...
    return (
      <div className="flex items-center justify-between p-2 border-b border-arkhe-border/30">
        <span className="text-arkhe-muted uppercase tracking-tighter">
          {label}
        </span>
        <div className="flex items-center gap-2">
          <span
            className={`text-[10px] font-bold uppercase ${isValid ? 'text-arkhe-green' : 'text-arkhe-red'}`}
          >
            {typeof status === 'string'
              ? status
              : status
          ...
          {isValid ? (
            <CheckCircle2 className="w-3 h-3 text-arkhe-green" />
          ) : (
            <AlertTriangle className="w-3 h-3 text-arkhe-red" />
          )}
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-arkhe-cyan font-bold tracking-[0.2em] uppercase text-sm">
              Arquiteto da República HYDRO-Ω
            </h2>
            <p className="text-[10px] text-arkhe-muted">
              SISTEMA DE SEGURANÇA AVANÇADA // CAMADAS L1-L5
            </p>
          </div>
        </div>
        <button
          onClick={onClose}
          className="text-arkhe-muted hover:text-arkhe-cyan transition-colors text-xs"
        >
          [ FECHAR ]
        </button>
      </div>
          ...
          {(['l1', 'l2', 'l3', 'l4', 'l5', 'qhttp'] as const).map(tab => (
            ...
            >
              ...
              <div className="text-xs font-bold uppercase tracking-widest">
                {tab === 'qhttp' ? 'qhttp-Ω' : tab.toUpperCase()}
              </div>
            </button>
          ))}
          <div className="mt-auto p-2 border-t border-arkhe-border/30">
            <div className="text-[10px] text-arkhe-muted mb-1 uppercase">
              Coerência Kuramoto
            </div>
            <div className="h-1 bg-arkhe-border rounded-full overflow-hidden">
              <div
                className="h-full bg-arkhe-cyan transition-all duration-1000"
                style={{width: `${state.currentLambda * 100}%`}}
              ></div>
            </div>
            <div className="text-right text-[10px] text-arkhe-cyan mt-1">
              R(t): {state.currentLambda.toFixed(4)}
            </div>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg">
                  ...
                  <div className="space-y-2">
                    <LayerStatus
                      label="Status TEE (SGX)"
                      status={security.l1.teeStatus}
                    />
                    <LayerStatus
                      label="Sensor de Intrusão"
                      status={security.l1.intrusionSensor}
                    />
                    <LayerStatus
                      label="HSM Cloud Sync"
                      status={security.l1.hsmBackupSynced}
                    />
                  </div>
                </div>
                <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-arkhe-muted mb-4 leading-relaxed">
                      ATIVAÇÃO TÉRMICA DOS CENTROS NV (+600°C) EM CASO DE
                      VIOLAÇÃO FÍSICA DETECTADA PELO ENCLAVE.
                    </p>
                  </div>
                  <button
                  ...
                  >
                    {security.l1.thermalDestructionArmed
                      ? 'SISTEMA ARMADO'
                      : 'ARMAR DESTRUIÇÃO TÉRMICA'}
                  </button>
                </div>
              </div>

              <div className="bg-black/40 border border-arkhe-border p-4 rounded-lg">
                <h4 className="text-arkhe-cyan text-xs font-bold mb-4 uppercase tracking-widest">
                  Atestação Remota Periódica
                </h4>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <div className="text-[10px] text-arkhe-muted mb-1">
                      ÚLTIMO QUOTE VALIDADO
                    </div>
                    <div className="text-xs font-mono text-arkhe-text">
                      {security.l1.lastRemoteAttestation}
                    </div>
                  </div>
                  <button
                  ...
                  >
                    <RefreshCw
                      className={`w-3 h-3 ${attesting ? 'animate-spin' : ''}`}
                    />
            ...
            <div className="space-y-4">
              <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg">
                ...
                <div className="space-y-1">
                  <LayerStatus
                    label="Handshake EPR (Viol. Bell)"
                    status={security.l2.eprHandshake}
                  />
                  <LayerStatus
                    label="MuSig2 Heartbeat Sign"
                    status={security.l2.muSig2Heartbeat}
                  />
                  <LayerStatus
                    label="Pneuma Phase Outlier"
                    status={!security.l2.pneumaOutlierDetected}
                  />
                </div>
                <div className="mt-4 pt-4 border-t border-arkhe-border/30 grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="text-sm font-bold text-arkhe-text">
                      {security.l2.qrngJitterMs.toFixed(2)} ms
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] text-arkhe-muted mb-1">
          ...
          {activeTab === 'l3' && (
            <div className="space-y-4">
              <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg">
                ...
                <div className="space-y-1">
                  <LayerStatus
                    label="Nullifier Unique Status"
                    status={security.l3.nullifierVerified}
                  />
                  <LayerStatus
                    label="TTL Window (5m)"
                    status={security.l3.ttlValid}
                  />
                  <LayerStatus
                    label="Quantum Witness (T2*)"
                    status={security.l3.t2StarMicroseconds > 50}
                  />
                </div>
                <div className="mt-4 pt-4 border-t border-arkhe-border/30">
                  <div className="text-[10px] text-arkhe-muted mb-2 uppercase">
                    Integridade QRNG Timestamp
                  </div>
                ...
                <div className="mt-4 flex justify-between items-center">
                  <span className="text-[10px] text-arkhe-muted">
                    T2* COHERENCE
                  </span>
                  <span className="text-xs font-bold text-arkhe-green">
                    {security.l3.t2StarMicroseconds.toFixed(2)} μs
                  </span>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'l4' && (
            <div className="space-y-4">
              <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg">
                ...
                <div className="space-y-1">
                  <LayerStatus
                    label="Assinatura OWL (MuSig2)"
                    status={security.l4.owlSignatureValid}
                  />
                  <LayerStatus
                    label="Conformidade ZK-Ontology"
                    status={security.l4.zkOntologicalProof}
                  />
                </div>
                <div className="mt-4 space-y-2">
                  <div className="flex justify-between text-[10px] mb-1">
                    <span className="text-arkhe-muted uppercase">
                      Consistência LOGOS
                    </span>
                    <span className="text-arkhe-cyan">
                      {(security.l4.logosConsistency * 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="h-1 bg-arkhe-border rounded-full overflow-hidden">
                    <div
                      className="h-full bg-arkhe-cyan"
                      style={{width: `${security.l4.logosConsistency * 100}%`}}
                    ></div>
                  </div>
                </div>
              </div>
              <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg">
                <h4 className="text-arkhe-muted text-[10px] font-bold mb-3 uppercase">
                  Merkle-DAG Root Hash (Semanal)
                </h4>
              ...
              <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg">
                ...
                <div className="space-y-1">
                  <LayerStatus
                    ...
                    status={security.l5.cspStatus}
                  />
                  <LayerStatus
                    label="Asset Integrity (SRI)"
                    status={security.l5.sriVerified}
                  />
                  <LayerStatus
                    label="Anti-CSRF Session Token"
                    status={true}
                  />
                  <LayerStatus
                    label="ZK-Proof UI Verify"
                    status={security.l5.zkUiVerified}
                  />
                  <LayerStatus
                    label="Cache PWA Assinado"
                    status={security.l5.pwaCacheSigned}
                  />
                </div>
              </div>
            </div>
              ...
              <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center p-2 bg-black/40 rounded">
                    ...
                    <span className="text-[10px] font-bold text-arkhe-cyan">
                      {security.qhttp.pqTlsStatus}
                    </span>
                  </div>
                  <div className="flex justify-between items-center p-2 bg-black/40 rounded">
                    <span className="text-[10px] text-arkhe-muted uppercase">
                    ...
                    <span className="text-[10px] font-mono text-arkhe-muted">
                      {security.qhttp.xKuramotoHeader}
                    </span>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-arkhe-border/30">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span
                      className={`text-xs font-bold ${security.qhttp.bellViolationS > 2 ? 'text-arkhe-green' : 'text-arkhe-red'}`}
                    >
                  ...
                  <div className="h-1 bg-arkhe-border rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-500 ${security.qhttp.bellViolationS > 2 ? 'bg-arkhe-green' : 'bg-arkhe-red'}`}
                      style={{
                        width: `${(security.qhttp.bellViolationS / 2.82) * 100}%`,
                      }}
                    ></div>
                  </div>
