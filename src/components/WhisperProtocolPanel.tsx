...
export default function WhisperProtocolPanel({
  state,
  onClose,
}: WhisperProtocolPanelProps) {
                ...
                {state.calibrations.map(c => (
                  ...
                  >
                    ...
                    <div className="text-[9px] font-mono text-arkhe-muted">
                      AR {c.aspectRatio.toFixed(0)}:1
                    </div>
                  </button>
                ))}
                <button
                  onClick={() =>
                    fetch('/api/whisper/calibrate', {
                      method: 'POST',
              ...
              {state.calibrations.find(c => c.material === selectedMaterial) ? (
                <>
                  <div className="bg-black/40 p-6 rounded-lg border border-white/5 space-y-4">
                    <div className="flex items-center justify-between border-b border-white/10 pb-4">
                      <h3 className="text-xl font-bold text-amber-500 font-mono uppercase">
                        {selectedMaterial} Calibration
                      </h3>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-1">
                        ...
                        <div className="text-lg font-mono text-arkhe-text">
                          {state.calibrations
                            .find(c => c.material === selectedMaterial)
                            ?.pulseDurationFs.toFixed(1)}{' '}
                          fs
                        </div>
                      </div>
                      <div className="space-y-1">
                  ...
                  <div className="bg-amber-500/5 p-4 rounded border border-amber-500/20">
                    <div className="text-[10px] font-mono text-amber-500 uppercase mb-2">
                      Geometric Persuasion Logic
                    </div>
                    <div className="text-[9px] font-mono text-arkhe-muted leading-relaxed italic">
                      "The pulse profile is optimized to resound with{' '}
                      {selectedMaterial}'s intrinsic phonon dispersion modes. By
                      matching the spectral phase to the lattice anisotropy, we
                      induce spontaneous geometric reorganization."
                    </div>
                  </div>
                </>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-arkhe-muted space-y-4">
                  ...
                  <div className="text-xs font-mono uppercase">
                    Select a material to view whisper profile
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="p-4 border-t border-white/5 bg-black/40 flex justify-between items-center shrink-0">
          <div className="flex gap-4">
            <div className="text-[10px] font-mono">
              <span className="text-arkhe-muted uppercase mr-2">
                Total Whispers:
              </span>
              <span className="text-arkhe-text">
                {state.totalWhispers.toLocaleString()}
              </span>
            </div>
            <div className="text-[10px] font-mono">
              <span className="text-arkhe-muted uppercase mr-2">
              ...
              <span className="text-emerald-500">
                {(state.successRate * 100).toFixed(2)}%
              </span>
            </div>
          </div>
