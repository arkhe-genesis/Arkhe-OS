    ...
    <div className="bg-[#1a1b1e] border border-[#2a2b30] rounded-lg p-4 overflow-hidden">
      ...
      <div className="space-y-3">
        {agents.map(agent => (
          <div
            key={agent.id}
            className="bg-black/30 border border-[#2a2b30] rounded p-2 text-[10px] font-mono"
          >
            <div className="flex justify-between items-start mb-1">
              ...
              <span
                className={`px-1.5 py-0.5 rounded border border-current ${
                  agent.status === 'alert'
                    ? 'text-red-400'
                    : agent.status === 'active'
                      ? 'text-emerald-400'
            ...
            <div className="text-white/70 mb-1 leading-tight">
              {agent.function}
            </div>
            ...
            <div className="flex justify-between items-center text-[9px] border-t border-[#2a2b30] pt-1 mb-2">
              <div className="flex flex-col gap-0.5">
                <span className="text-emerald-500/70 uppercase tracking-tighter">
                  Metric: {agent.metric}
                </span>
                {agent.status === 'active' && (
              ...
              <span
                className="text-white/30 truncate max-w-[120px]"
            ...
            {isPOCAgent(agent.id) && (
              ...
              >
                {loading === agent.id
                  ? 'EXECUTANDO qhttp COLLAPSE...'
                  : `ACIONAR ${agent.name} POC`}
              </button>
            )}
          </div>
        ))}
...
export const EnterprisePlusPanel: React.FC<EnterprisePlusPanelProps> = ({
  onClose,
}) => {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white tracking-widest uppercase">
                Arkhe(n) Enterprise Plus
              </h2>
              <p className="text-[10px] text-arkhe-muted font-mono uppercase tracking-[0.2em]">
                Interoperabilidade de Cidadelas Corporativas
              </p>
            </div>
          </div>
        ...
        <div className="p-4 bg-black/60 border-t border-arkhe-border flex justify-between items-center text-[10px] font-mono">
          <div className="flex gap-4">
            <span className="text-arkhe-muted">
              PROTOCOL: <span className="text-white">qhttp/2.0</span>
            </span>
            <span className="text-arkhe-muted">
              CONSENSUS: <span className="text-white">MuSig2/FROST</span>
            </span>
            <span className="text-arkhe-muted">
              ONTOLOGY: <span className="text-white">BFO/DOLCE</span>
            </span>
          </div>
