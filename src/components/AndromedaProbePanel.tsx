...
export default function AndromedaProbePanel({
  state,
  onClose,
}: AndromedaProbePanelProps) {
  if (!state) {
    return null;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              ...
              <div className="text-xl font-bold text-arkhe-cyan font-mono">
                {state.distanceLy.toLocaleString()} LY
              </div>
              <div className="text-[9px] text-arkhe-muted font-mono italic">
                Crossing Intergalactic Vacuum...
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="text-[10px] font-mono text-arkhe-cyan">
                    {(state.signalCoherence * 100).toFixed(1)}%
                  </div>
                </div>
          ...
          <button
            onClick={() =>
              fetch('/api/cosmic/andromeda-launch', {method: 'POST'})
            }
