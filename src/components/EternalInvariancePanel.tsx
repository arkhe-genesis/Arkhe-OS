...
export const EternalInvariancePanel: React.FC = () => {
  ...
  const handleEternalize = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/cathedral/eternalize', {
        method: 'POST',
      });
      if (!response.ok) {
        throw new Error('Failed to eternalize');
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleVerifyFixedPoint = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/cathedral/fixed-point-verify', {
        method: 'POST',
      });
  ...
  return (
    <div className="p-4 bg-zinc-900/50 rounded-lg border border-white/10 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`px-2 py-0.5 rounded text-xs font-bold ${ei.isEternal ? 'bg-rose-500/20 text-rose-400' : 'bg-zinc-800 text-zinc-400'}`}
        >
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-4 text-[10px] text-zinc-500">
          ...
          <div className="flex items-center gap-1">
            ...
            <span>
              Fixed Point: {state.riscVi?.invarianceMetric.toFixed(5)}
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-1">
            {fixPointResult.verifications.map((v: any) => (
              ...
              >
                ...
                <span
                  className={
                    v.status === 'VALID' ? 'text-emerald-500' : 'text-zinc-600'
                  }
                >
                  {v.status}
                </span>
