...
export default function SepoliaIntegrationPanel({
  onClose,
}: {
  onClose: () => void;
}) {
  ...
  const executeSequence = async () => {
    ...
    setTimeout(() => {
      setLogs(prev => [
        ...prev,
        '> [1/4] COMPILING TZINOR.SOL & THUKDAM.SOL...',
      ]);
    }, 500);
    ...
    setTimeout(() => {
      ...
      setLogs(prev => [
        ...prev,
        '> TZINOR CONTRACT: 0x' + Math.random().toString(16).slice(2, 42),
      ]);
      setLogs(prev => [
        ...prev,
        '> THUKDAM CONTRACT: 0x' + Math.random().toString(16).slice(2, 42),
      ]);
    }, 2000);
    ...
    setTimeout(() => {
      ...
      setLogs(prev => [
        ...prev,
        '> [SUCCESS] SNAP INSTALLED. RPC HANDLERS ACTIVE.',
      ]);
      setStep(3);
    }, 6500);
    ...
    setTimeout(() => {
      setLogs(prev => [
        ...prev,
        '> [3/4] INITIALIZING ARKHE TYPESCRIPT SDK...',
      ]);
      ...
      setLogs(prev => [
        ...prev,
        '> SDK INSTANCE CREATED. CONNECTING TO SEPOLIA RPC...',
      ]);
    }, 7500);
    ...
    setTimeout(async () => {
      setLogs(prev => [
        ...prev,
        '> [4/4] TESTING ORBVM INTEGRATION VIA SDK...',
      ]);
      ...
      setLogs(prev => [
        ...prev,
        `> GAS USED: ${orbVmRes.gasUsed} | LAMBDA: ${orbVmRes.lambda}`,
      ]);
    }, 10500);
    ...
    setTimeout(() => {
      ...
      setLogs(prev => [
        ...prev,
        '> ALL SYSTEMS NOMINAL. SEPOLIA DEPLOYMENT COMPLETE.',
      ]);
      setStep(5);
    }, 12500);
  };
      ...
      <div className="bg-[#111214] border border-arkhe-purple/30 rounded-xl w-full max-w-4xl max-h-[80vh] flex flex-col shadow-[0_0_30px_rgba(168,85,247,0.1)]">
        <div className="flex items-center justify-between p-4 border-b border-arkhe-purple/20">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-mono text-sm uppercase tracking-widest text-arkhe-purple">
              Sepolia Integration & SDK
            </h2>
          </div>
          <button
            onClick={onClose}
            className="text-arkhe-muted hover:text-arkhe-text"
          >
            <svg
        ...
        <div className="p-6 flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-6">
            <div className="bg-black/40 border border-[#1f2024] rounded-lg p-4">
              <h3 className="font-mono text-xs uppercase tracking-widest text-arkhe-muted mb-4">
                Deployment Sequence
              </h3>
              ...
              <div className="space-y-4">
                {/* Step 1 */}
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 2 ? 'bg-arkhe-green/10 border-arkhe-green/30 text-arkhe-green' : step === 1 ? 'bg-arkhe-purple/10 border-arkhe-purple/30 text-arkhe-purple' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 2 ? (
                ...
                {/* Step 2 */}
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 3 ? 'bg-orange-500/10 border-orange-500/30 text-orange-500' : step === 2 ? 'bg-arkhe-purple/10 border-arkhe-purple/30 text-arkhe-purple' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 3 ? (
                    <CheckCircle2 className="w-5 h-5" />
                  ) : step === 2 ? (
                ...
                {/* Step 3 */}
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 4 ? 'bg-blue-500/10 border-blue-500/30 text-blue-500' : step === 3 ? 'bg-arkhe-purple/10 border-arkhe-purple/30 text-arkhe-purple' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 4 ? (
                    <CheckCircle2 className="w-5 h-5" />
                  ) : step === 3 ? (
                ...
                {/* Step 4 */}
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 5 ? 'bg-arkhe-cyan/10 border-arkhe-cyan/30 text-arkhe-cyan' : step === 4 ? 'bg-arkhe-purple/10 border-arkhe-purple/30 text-arkhe-purple' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 5 ? (
                    <CheckCircle2 className="w-5 h-5" />
                  ) : step === 4 ? (
          ...
          <div className="bg-black/60 border border-[#1f2024] rounded-lg p-4 flex flex-col">
            <div className="flex items-center gap-2 mb-4 border-b border-[#1f2024] pb-2">
              ...
              <h3 className="font-mono text-xs uppercase tracking-widest text-arkhe-muted">
                Deployment Logs
              </h3>
            </div>
