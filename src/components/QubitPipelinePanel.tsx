...
export default function QubitPipelinePanel({onClose}: {onClose: () => void}) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => ({
        ...prev,
        gate_fidelity: 0.999 + Math.random() * 0.0009,
  ...
  const steps = [
    {
      id: 1,
      name: 'Charge Sensor Tuning',
      icon: <Microscope className="w-4 h-4" />,
      status: 'Complete',
    },
    {
      id: 2,
      name: 'Few-Electron Mapping',
      icon: <Binary className="w-4 h-4" />,
      status: 'Complete',
    },
    {
      id: 3,
      name: 'Sweet Spot Localization',
      icon: <Zap className="w-4 h-4" />,
      status: 'Active',
    },
    {
      id: 4,
      name: 'Coherence Characterization',
      ...
      <div className="bg-[#0a0a0c] border border-arkhe-cyan/30 rounded-xl w-full max-w-4xl h-[70vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-64 p-6 border-b md:border-b-0 md:border-r border-arkhe-cyan/10 bg-black/20">
            <h3 className="text-[10px] font-mono text-arkhe-muted uppercase tracking-widest mb-6">
              Execution Flow
            </h3>
            <div className="space-y-4">
              {steps.map(s => (
                <div
                  key={s.id}
                  className={`flex items-start gap-3 p-2 rounded transition-colors ${s.id === step ? 'bg-arkhe-cyan/10 border border-arkhe-cyan/30' : ''}`}
                >
                  <div
                    className={`mt-0.5 ${s.status === 'Complete' ? 'text-arkhe-green' : s.id === step ? 'text-arkhe-cyan animate-pulse' : 'text-arkhe-muted'}`}
                  >
                    {s.icon}
                  </div>
                  <div>
                    <div
                      className={`text-xs font-mono ${s.id === step ? 'text-white font-bold' : 'text-arkhe-muted'}`}
                    >
                      {s.name}
                    </div>
                    <div className="text-[8px] font-mono text-arkhe-muted/60 uppercase">
                      {s.status}
                    </div>
                  </div>
                </div>
          ...
          <div className="flex-1 p-6 space-y-8 overflow-y-auto custom-scrollbar">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-4 bg-white/5 border border-arkhe-cyan/10 rounded-lg">
                <div className="text-[10px] font-mono text-arkhe-muted uppercase mb-4">
                  Stability Monitor (DQD Search)
                </div>
                ...
                <div className="mt-2 text-center text-[9px] font-mono text-arkhe-cyan">
                  Atrator de Fase [P1, P2]
                </div>
              </div>
              ...
              <div className="p-4 bg-white/5 border border-arkhe-cyan/10 rounded-lg space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-[8px] text-arkhe-muted uppercase">
                      T2* (Dephasing)
                    </div>
                  ...
                  <div>
                    ...
                    <div className="text-lg font-mono text-arkhe-green">
                      {(metrics.gate_fidelity * 100).toFixed(2)}%
                    </div>
                  </div>
                  <div>
                    <div className="text-[8px] text-arkhe-muted uppercase">
            ...
            <div className="p-4 bg-black/40 border border-arkhe-cyan/20 rounded-lg">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-mono text-white uppercase tracking-widest">
                  Sinfonia do Qubit - Realtime Spectral
                </span>
              </div>
        ...
        <div className="p-4 bg-black/60 border-t border-arkhe-cyan/20 flex items-center justify-between text-[10px] font-mono">
          <div className="flex items-center gap-4">
            ...
            <span className="text-arkhe-green font-bold animate-pulse">
              INCUBATING
            </span>
          </div>
          <div className="text-arkhe-muted uppercase tracking-widest">
            Arkhe Block 850.041 // Qubit Pipeline
          </div>
        </div>
      </div>
    </div>
