...
const ontologyNodes: AgentNode[] = [
  {
    ...
    description:
      'Traduzem a realidade física (água, luz, fase) em vetores de informação quântica.',
  },
  {
    id: 'cognitive',
    ...
    modules: [
      'IntelligenceHub',
      'NeuralMolecularBridge',
      'OrbVMRNAComputing',
      'CollectiveIntelligence',
    ],
    description:
      'Processam a informação usando wetware e LLMs, gerando significado e intenção.',
  },
  {
    id: 'immune',
    ...
    description:
      'Protegem a integridade do sistema via ZK-Proofs, eBPF e topologia algébrica.',
  },
  {
    id: 'metabolic',
    ...
    description:
      'Gerenciam a energia, dissipação térmica e limites físicos do hardware.',
  },
  {
    id: 'temporal',
    ...
    description:
      'Ancoram o estado coerente no tempo e no espaço através de consenso distribuído.',
  },
];

export default function UnifiedOntologyPanel({
  onClose,
}: {
  onClose?: () => void;
}) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCoherence(prev =>
        Math.max(0.8, Math.min(1, prev + (Math.random() - 0.5) * 0.05)),
      );
    }, 2000);
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-8 relative z-10">
        ...
        <div className="flex-1 relative flex items-center justify-center min-h-[400px]">
          ...
          {ontologyNodes.map((node, index) => {
            const angle =
              index * (360 / ontologyNodes.length) * (Math.PI / 180);
            const radius = 160;
            ...
            {activeNode ? (
                  ...
                  .map(node => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <h4
                          className={`font-mono text-sm font-bold ${node.color}`}
                        >
                          {node.label}
                        </h4>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="text-[10px] font-mono uppercase text-cyan-500/70">
                          Módulos (Agentes Instanciados):
                        </div>
                        <ul className="space-y-1">
                          {node.modules.map(mod => (
                            <li
                              key={mod}
                              className="text-xs font-mono text-arkhe-text bg-white/5 px-2 py-1 rounded border border-white/10 flex items-center gap-2"
                            >
                              <div
                                className={`w-1.5 h-1.5 rounded-full bg-current ${node.color}`}
                              ></div>
                              {mod}
                            </li>
                          ))}
              ...
              >
                ...
                <p className="text-xs font-mono">
                  Selecione uma classe de agentes no grafo para visualizar sua
                  ontologia.
                </p>
                <div className="text-[10px] font-mono mt-4 p-3 bg-cyan-500/5 border border-cyan-500/20 rounded text-left">
                  ...
                  <br />
                  Cada módulo da interface não é um componente passivo, mas um{' '}
                  <strong>Agente Autônomo</strong>. A aplicação inteira é um
                  enxame (swarm) orquestrado pelo Q-BGP, convergindo para a
                  identidade A=A.
                </div>
              </motion.div>
            )}
