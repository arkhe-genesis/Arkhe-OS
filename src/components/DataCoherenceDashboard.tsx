...
export default function DataCoherenceDashboard({
  onClose,
}: DataCoherenceDashboardProps) {
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="bg-white/5 border border-white/10 rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xs font-mono uppercase tracking-widest text-arkhe-muted">
                  Overall System Health (λ₂-data)
                </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
                <div className="bg-black/40 border border-white/5 p-3 rounded">
                  <div className="flex justify-between items-start mb-1">
                    <span className="text-[8px] font-mono text-arkhe-muted uppercase">
                      Tor Circuit
                    </span>
                    <Shield
                      className={`w-3 h-3 ${state.networkInfra.tor.status === 'CONNECTED' ? 'text-arkhe-green' : 'text-blue-400'}`}
                    />
                  </div>
                  <div className="text-xs font-mono font-bold">
                    {state.networkInfra.tor.status}
                  </div>
                  <div className="text-[7px] font-mono text-arkhe-muted mt-1">
                    Latency: {state.networkInfra.tor.latencyMs.toFixed(1)}ms
                  </div>
                </div>
                <div className="bg-black/40 border border-white/5 p-3 rounded">
                  <div className="flex justify-between items-start mb-1">
                  ...
                  <div className="text-xs font-mono font-bold">
                    {state.networkInfra.broker.status}
                  </div>
                  <div className="text-[7px] font-mono text-arkhe-muted mt-1">
                    Processed: {state.networkInfra.broker.messagesProcessed}
                  </div>
                </div>
                <div className="bg-black/40 border border-white/5 p-3 rounded">
                  <div className="flex justify-between items-start mb-1">
                  ...
                  <div className="text-xs font-mono font-bold">
                    {state.networkInfra.redis.status}
                  </div>
                  <div className="text-[7px] font-mono text-arkhe-muted mt-1">
                    Hits: {state.networkInfra.redis.cacheHits}
                  </div>
                </div>
              </div>
            </div>
              ...
              {domains.map(domain => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-2 rounded bg-black/40 ${domain.health === 'CRITICAL' ? 'text-arkhe-red' : 'text-blue-400'}`}
                    >
                  ...
                  <div className="text-right">
                    <div
                      className={`text-lg font-mono font-bold ${domain.health === 'CRITICAL' ? 'text-arkhe-red' : 'text-blue-400'}`}
                    >
                      {domain.lambda2.toFixed(3)}
                    </div>
                    <div
                      className={`text-[8px] font-mono uppercase ${domain.health === 'CRITICAL' ? 'text-arkhe-red/70' : 'text-arkhe-muted'}`}
                    >
                      {domain.health}
                    </div>
                  </div>
          ...
          <div className="space-y-4">
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-3 text-blue-400">
                ...
                <h4 className="text-[10px] font-mono uppercase font-bold text-blue-400">
                  Critical Alerts
                </h4>
              </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-lg p-4 space-y-3">
              <h4 className="text-[10px] font-mono uppercase font-bold text-arkhe-muted">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() =>
                    fetch('/api/sca-data/protocol', {
                      method: 'POST',
                      headers: {'Content-Type': 'application/json'},
                      body: JSON.stringify({protocol: 'BRAID'}),
                    })
                  }
                ...
                <button
                  onClick={() =>
                    fetch('/api/sca-data/protocol', {
                      method: 'POST',
                      headers: {'Content-Type': 'application/json'},
                      body: JSON.stringify({protocol: 'COMPUTE'}),
                    })
                  }
                  className="py-2 border border-blue-500/30 text-blue-400 font-mono text-[8px] uppercase rounded hover:bg-blue-500/10 transition-colors"
                >
                ...
                <button
                  onClick={() =>
                    fetch('/api/sca-data/protocol', {
                      method: 'POST',
                      headers: {'Content-Type': 'application/json'},
                      body: JSON.stringify({protocol: 'HEAL'}),
                    })
                  }
                ...
                <button
                  onClick={() =>
                    fetch('/api/sca-data/protocol', {
                      method: 'POST',
                      headers: {'Content-Type': 'application/json'},
                      body: JSON.stringify({protocol: 'SEAL'}),
                    })
                  }
                  className="py-2 border border-arkhe-green/30 text-arkhe-green font-mono text-[8px] uppercase rounded hover:bg-arkhe-green/10 transition-colors"
                >
              ...
              <div className="mt-4 space-y-1">
                <div className="flex justify-between text-[8px] font-mono">
                  ...
                  <span className="text-blue-400 font-bold">
                    {state.scaData.entanglementMode}
                  </span>
                </div>
                <div className="flex justify-between text-[8px] font-mono">
                  <span>R(t) Global:</span>
                  <span className="text-blue-400">
                    {state.scaData.globalOrderR.toFixed(3)}
                  </span>
                </div>
                <div className="flex justify-between text-[8px] font-mono">
                  <span>ATP Cons.:</span>
                  <span className="text-arkhe-green">
                    {state.scaData.atpConsumptionCps} cps
                  </span>
                </div>
                <div className="flex justify-between text-[8px] font-mono">
                  <span>Resiliência:</span>
                  <span className="text-arkhe-green font-bold">
                    TOPOLÓGICA (QSL)
                  </span>
                </div>
              </div>
            ...
            {state.scaData.protocolLogs.length > 0 && (
              <div className="p-2 bg-black/40 rounded border border-white/5">
                <h4 className="text-[8px] font-mono uppercase text-blue-400 mb-2">
                  Protocol Output: {state.scaData.activeProtocol}
                </h4>
                <div className="space-y-1 max-h-32 overflow-y-auto custom-scrollbar">
                  {state.scaData.protocolLogs.map((log, i) => (
                    <div
                      key={i}
                      className="text-[7px] font-mono text-arkhe-muted border-l border-white/10 pl-2 leading-tight"
                    >
                      {log}
                    </div>
                  ))}
            ...
            <div className="p-2">
              <h4 className="text-[8px] font-mono uppercase text-arkhe-muted mb-2">
                Strategy: Edge-of-Chaos (SBM)
              </h4>
              <p className="text-[8px] font-mono text-arkhe-muted leading-relaxed">
                O SCA-Data utiliza o controlador Stabilized by Memory para
                equilibrar throughput e latência. Circuit-breakers são ativados
                quando λ₂ cai abaixo de 0.90.
              </p>
            </div>
          </div>
        </div>
