...
export const ConsensusMeter: React.FC = () => {
  ...
  return (
    <div
      className={`p-4 rounded-lg border flex flex-col items-center justify-center space-y-4 ${getStatusBg()}`}
    >
      <div className="flex items-center space-x-2">
        ...
        {status === 'warning' && (
          <AlertTriangle className={`w-6 h-6 ${getStatusColor()}`} />
        )}
        {status === 'critical' && (
          <ShieldAlert className={`w-6 h-6 ${getStatusColor()}`} />
        )}
        <h3
          className={`text-lg font-bold uppercase tracking-wider ${getStatusColor()}`}
        >
          Consensus Health (Σ)
        </h3>
      </div>
      ...
      <div className="text-xs text-white/60 font-mono text-center">
        {status === 'healthy' &&
          'Network Coherence Optimal. No reorgs detected.'}
        {status === 'warning' &&
          'Entropy Detected. Hashrate drop or minor forks.'}
        {status === 'critical' &&
          'CRITICAL: Severe Decoherence! Potential chain split.'}
      </div>
    </div>
  );
};
