...
const CellularHealthPanel: React.FC<CellularHealthPanelProps> = ({state}) => {
  ...
  const metrics = [
    {
      label: 'Telômeros',
      value: `${(health.telomere_length * 100).toFixed(1)}%`,
      sub: 'Baseline Jovem',
    },
    {
      label: 'Estresse Oxid.',
      value: `${(health.oxidative_stress * 100).toFixed(0)}%`,
      sub: 'Meta < 30%',
    },
    {
      label: 'Efic. Mito.',
      value: `${(health.mitochondrial_efficiency * 100).toFixed(1)}%`,
      sub: 'Modo Otimizado',
    },
    {
      label: 'Inflamação',
      value: `${(health.inflammation_marker * 100).toFixed(1)}%`,
      sub: 'IL-6 / TNF-α',
    },
  ];
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {metrics.map(m => (
            <div
              key={m.label}
              className="bg-black/30 p-2 rounded border border-white/5"
            >
              <p className="text-[7px] text-white/40 uppercase mb-1 font-mono tracking-tighter">
                {m.label}
              </p>
              <p className="text-xs font-bold text-white/90">{m.value}</p>
              <p className="text-[6px] text-white/30 uppercase italic">
                {m.sub}
              </p>
            </div>
          ))}
