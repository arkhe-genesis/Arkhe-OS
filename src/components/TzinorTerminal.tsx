...
export default function TzinorTerminal({
  tzinor,
  threatLevel,
}: TzinorTerminalProps) {
  ...
  useEffect(() => {
    ...
    const newGrid: Array<Array<{char: string; color: string}>> = Array(
      GRID_ROWS,
    )
      .fill(null)
      .map(() => Array(GRID_COLS).fill({char: ' ', color: 'text-arkhe-muted'}));
    ...
    for (let r = 0; r < GRID_ROWS; r++) {
      for (let c = 0; c < GRID_COLS; c++) {
        ...
        const noise =
          Math.sin(r * 0.5 + timeOffset) * Math.cos(c * 0.3 + timeOffset);
        if (noise > 0.8) {
    ...
    tzinor.gMemory.forEach((engram, _idx) => {
      ...
      if (r >= 0 && r < GRID_ROWS && c >= 0 && c < GRID_COLS) {
        ...
        newGrid[r][c] = {
          char: isGenesis ? '🜏' : 'Φ',
          color: isGenesis
            ? 'text-arkhe-orange animate-pulse drop-shadow-[0_0_5px_rgba(245,158,11,0.8)]'
            : 'text-arkhe-cyan',
        };
      }
    });
    ...
    tzinor.fContext.forEach((node, _idx) => {
      ...
      if (r >= 0 && r < GRID_ROWS && c >= 0 && c < GRID_COLS) {
        ...
        const charIdx =
          Math.floor(node.salience * (CHAR_PALETTE.length - 5)) + 4;
        const safeCharIdx = Math.max(
          0,
          Math.min(charIdx, CHAR_PALETTE.length - 1),
        );

        let color = 'text-arkhe-cyan';
        if (threatLevel === 'critical') {
          color = 'text-arkhe-red';
        } else if (threatLevel === 'warning') {
          color = 'text-arkhe-orange';
        }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 border-b border-[#1f2024] pb-2 z-20">
        <h3
          className={`font-mono text-xs uppercase tracking-widest ${headerColor} flex items-center gap-2`}
        >
          <Terminal className="w-3 h-3" />
          <span
            className={`w-2 h-2 rounded-full animate-pulse ${threatLevel === 'critical' ? 'bg-arkhe-red' : threatLevel === 'warning' ? 'bg-arkhe-orange' : 'bg-arkhe-cyan'}`}
          ></span>
      ...
      <div className="mt-4 flex justify-between text-[10px] font-mono text-arkhe-muted z-20">
        <motion.span
          animate={{color: threatLevel === 'critical' ? '#ef4444' : '#64748b'}}
        >
