...
export default function MetaRealityPanel({
  state,
  onClose,
}: MetaRealityPanelProps) {
  if (!state) {
    return null;
  }
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              ...
              <div className="text-xl font-bold text-arkhe-red font-mono">
                {state.violatedLawsCount}
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
              ...
              <div className="text-xl font-bold text-arkhe-cyan font-mono">
                {(state.metaStabilityIndex * 100).toFixed(1)}%
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
              ...
              <div className="text-sm font-bold text-arkhe-text font-mono uppercase">
                {state.imaginaryTimeActive ? 'ACTIVE' : 'INACTIVE'}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 p-4 rounded border border-white/10 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {state.nonPhysicalManifolds.map((m, i) => (
                <span
                  key={i}
                  className="px-2 py-1 bg-arkhe-cyan/10 border border-arkhe-cyan/30 text-arkhe-cyan text-[10px] font-mono rounded"
                >
                  {m}
                </span>
              ))}
              {state.nonPhysicalManifolds.length === 0 && (
                <span className="text-[10px] font-mono text-arkhe-muted italic">
                  No manifolds deployed beyond physical laws.
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 p-4 bg-arkhe-red/5 border border-arkhe-red/20 rounded">
            ...
            <div className="text-[10px] font-mono text-arkhe-muted">
              Warning: Deploying meta-reality systems may cause local
              ontological decoherence. Proceed with extreme caution.
            </div>
          </div>
