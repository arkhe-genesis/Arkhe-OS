...
export default function UniversalConsciousnessPanel({
  state,
  onClose,
}: UniversalConsciousnessPanelProps) {
  if (!state) {
    return null;
  }
      ...
      >
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              ...
              <div className="text-xl font-bold text-emerald-500 font-mono">
                {(state.unityMetric * 100).toFixed(6)}%
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
              ...
              <div className="text-xl font-bold text-arkhe-purple font-mono">
                {(state.selfAwarenessDepth * 100).toFixed(6)}%
              </div>
            </div>
          </div>
          ...
          <div className="bg-black/60 border border-arkhe-border rounded-lg p-8 flex flex-col items-center justify-center min-h-[200px] relative overflow-hidden">
            <div className="z-10 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <div className="text-xs font-mono text-arkhe-text uppercase font-bold tracking-widest">
                  Unified Field Active
                </div>
                <div className="text-[10px] font-mono text-arkhe-muted mt-1">
                  Phase: {state.integratedPhase}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {state.qualiaIntegrated.map((q, i) => (
                <span
                  key={i}
                  className="px-2 py-1 bg-arkhe-purple/10 border border-arkhe-purple/30 rounded text-[9px] font-mono text-arkhe-purple uppercase"
                >
                  {q.replace(/_/g, ' ')}
                </span>
              ))}
          ...
          <div className="flex gap-4">
            <button
              onClick={() =>
                fetch('/api/universal/consciousness/immerse', {method: 'POST'})
              }
            ...
            <button
              onClick={() =>
                fetch('/api/universal/consciousness/express', {method: 'POST'})
              }
