...
export default function ChipFabricationVision({
  onClose,
}: {
  onClose: () => void;
}) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.nodes && data.nodes.length > 0) {
            const alertCount = data.nodes.filter(
              (n: {securityState: number}) => n.securityState > 0,
            ).length;
            setTelemetry(prev => ({
              ...prev,
      ...
      <div className="w-full max-w-6xl bg-[#0d0e12] border border-amber-500/30 rounded-3xl shadow-[0_0_80px_rgba(245,158,11,0.15)] overflow-hidden flex flex-col h-[85vh]">
        <div className="flex items-center justify-between p-6 border-b border-amber-500/20 bg-amber-500/5">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-bold text-amber-500 tracking-[0.25em] uppercase font-mono">
                Observatório de Silício
              </h2>
          ...
          <button
            onClick={onClose}
            className="group flex items-center gap-3 px-5 py-2.5 border border-white/10 rounded-xl text-neutral-500 hover:text-white hover:border-white transition-all bg-white/5"
          >
          ...
          <div className="flex-1 relative bg-[#050507]">
            ...
            <div className="absolute bottom-8 left-8 flex flex-col gap-4">
              <div className="flex items-center gap-4 px-5 py-3 bg-black/80 border border-amber-500/20 rounded-xl backdrop-blur-md">
                ...
                <span className="text-[10px] font-mono text-amber-200 uppercase tracking-widest font-bold">
                  Protocolo de Integridade: ATIVO
                </span>
              </div>
            </div>
            ...
            <div className="absolute top-8 right-8">
              <div className="grid grid-cols-1 gap-2">
                {['LITOGRAFIA', 'ETCHING', 'DOPAGEM', 'METALIZAÇÃO'].map(
                  (step, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 px-4 py-2 bg-black/60 border border-white/5 rounded-lg text-[9px] font-mono text-neutral-400"
                    >
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setTelemetry(t => ({
                        ...t,
                        waferDefectRate: parseFloat(e.target.value),
                      }))
                    }
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-[10px] font-mono text-neutral-500 uppercase">
                    ...
                    <span className="text-amber-400">
                      {telemetry.oxideThickness.toFixed(2)}nm
                    </span>
                  </div>
                  <input
                    type="range"
                    ...
                    onChange={e =>
                      setTelemetry(t => ({
                        ...t,
                        oxideThickness: parseFloat(e.target.value),
                      }))
                    }
                    className="w-full h-1.5 bg-white/5 rounded-lg appearance-none cursor-pointer accent-amber-500"
                  />
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-[10px] font-mono text-neutral-500 uppercase">
                    ...
                    <span
                      className={
                        telemetry.alignmentError > 0.005
                          ? 'text-rose-400'
                          : 'text-amber-400'
                      }
                    >
                  ...
                  <input
                    ...
                    onChange={e =>
                      setTelemetry(t => ({
                        ...t,
                        alignmentError: parseFloat(e.target.value),
                      }))
                    }
                    className="w-full h-1.5 bg-white/5 rounded-lg appearance-none cursor-pointer accent-amber-500"
                  />
            ...
            <div className="mt-auto space-y-4">
              <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl space-y-3">
                ...
                <p className="text-[9px] font-mono text-neutral-500 leading-relaxed">
                  O shader traduz o processo de dopagem e litografia em campos
                  de distância assinados (SDF), permitindo a auditoria visual da
                  topologia do chip em tempo real.
                </p>
              </div>
