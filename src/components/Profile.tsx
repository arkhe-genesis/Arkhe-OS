    ...
    <div className="min-h-screen bg-arkhe-bg text-arkhe-text font-sans selection:bg-arkhe-cyan selection:text-black p-4 md:p-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 border-b border-arkhe-border pb-4 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-widest uppercase">
              Arkhe-PNT{' '}
              <span className="text-arkhe-muted font-normal">// Operador</span>
            </h1>
            <div className="flex items-center gap-2 text-xs font-mono mt-1">
              ...
              <span className="text-arkhe-green">
                IDENTIDADE VERIFICADA // NÍVEL Ω-1
              </span>
            </div>
          </div>
        </div>
        <nav className="flex gap-6 font-mono text-sm uppercase tracking-widest">
          <Link
            to="/dashboard"
            className="text-arkhe-muted hover:text-arkhe-cyan transition-colors"
          >
            Dashboard
          </Link>
          <Link
            to="/profile"
            className="text-arkhe-cyan border-b border-arkhe-cyan pb-1"
          >
            Profile
          </Link>
        </nav>
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-arkhe-card border border-arkhe-border p-6 rounded-xl relative overflow-hidden">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <div className="text-xs font-mono text-arkhe-muted mt-1">
                ID: ARK-9942-Ω
              </div>
          ...
          <div className="bg-arkhe-card border border-arkhe-border p-6 rounded-xl">
            ...
            <div className="space-y-4">
              <div>
                <div className="text-xs text-arkhe-muted mb-1">
                  Chave Pública (Arkhe-Chain)
                </div>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-arkhe-card border border-arkhe-border p-6 rounded-xl">
            ...
            <div className="space-y-3">
              {[
                {
                  action: 'Sincronização de Bio-Nó',
                  target: 'Setor 7G',
                  time: '10 min atrás',
                  status: 'success',
                },
                {
                  action: 'Assinatura Bloco Gênese',
                  target: 'Arkhe-Chain (2147483647)',
                  time: '2 horas atrás',
                  status: 'success',
                },
                {
                  action: 'Varredura de Anomalia',
                  target: 'Triângulo das Bermudas',
                  time: '5 horas atrás',
                  status: 'success',
                },
                {
                  action: 'Mitigação de Ameaça',
                  target: 'Ataque DDoS (Camada 7)',
                  time: '1 dia atrás',
                  status: 'warning',
                },
                {
                  action: 'Calibração Kuramoto',
                  target: 'QPU Principal',
                  time: '2 dias atrás',
                  status: 'success',
                },
              ].map((log, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-3 bg-black/40 border border-arkhe-border/50 rounded"
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-2 h-2 rounded-full ${log.status === 'success' ? 'bg-arkhe-green' : 'bg-arkhe-orange'}`}
                    ></div>
                    <div>
