  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-2">
        <Icon
          className={`w-5 h-5 ${status === 'active' ? 'text-arkhe-cyan' : ''}`}
        />
...
export default function ResearchAgentsPanel() {
  ...
  const handleSearch = async () => {
    setIsSearching(true);
    setSearchLog(prev => [
      `> Iniciando consulta ao Banco de Dados de Conhecimento (Akasha)...`,
      ...prev,
    ]);
  ...
  return (
    <div className="bg-[#0a0a0c] border border-[#1f2024] rounded-xl p-6 flex flex-col gap-6 h-full overflow-hidden">
      ...
      <div className="flex-1 min-h-[120px] bg-black/40 rounded border border-[#1f2024] p-4 font-mono text-[10px] overflow-y-auto">
        <div className="text-arkhe-muted mb-2 uppercase tracking-widest border-b border-white/5 pb-1">
          Log de Pesquisa
        </div>
        <AnimatePresence>
          {searchLog.map((log, i) => (
        ...
        {searchLog.length === 0 && (
          <div className="text-white/10 italic">
            Aguardando comando de pesquisa...
          </div>
        )}
      </div>
    </div>
