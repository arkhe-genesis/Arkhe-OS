...
export default function CrystalComputationPanel({
  state,
  onClose,
}: CrystalComputationPanelProps) {
  if (!state) {
    return null;
  }
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              ...
              <div className="text-xl font-bold text-arkhe-cyan font-mono">
                {state.nanoholesCount.toLocaleString()}
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
              ...
              <div className="text-xl font-bold text-emerald-500 font-mono">
                {(state.opticalCoherence * 100).toFixed(6)}%
              </div>
            </div>
          </div>
          ...
          <div className="bg-black/60 border border-arkhe-border rounded-lg p-6 flex flex-col items-center justify-center space-y-4">
            ...
            <div className="text-center">
              ...
              <div className="text-[10px] font-mono text-arkhe-muted">
                Sapphire 3D Optical Logic Gate Matrix
              </div>
            </div>
