...
export default function GlobalCoherenceField({
  onClose,
}: GlobalCoherenceFieldProps) {
        ...
        for (let j = 0; j < N; j++) {
          if (i !== j) {
            ...
            if (dist < 180) {
              ...
              const memMultiplier =
                (node.memoryLambda * nodes[j].memoryLambda) / 2.6;
        ...
        for (let j = i + 1; j < N; j++) {
          ...
          if (dist < 150) {
            const phaseDiff = Math.abs(
              Math.sin(nodes[i].phase - nodes[j].phase),
            );
      ...
      >
        ...
        <div className="w-full md:w-80 bg-[#0a0a0a] border-r border-arkhe-cyan/20 p-6 flex flex-col gap-6">
          <div className="flex items-center gap-3 border-b border-arkhe-cyan/30 pb-4">
            ...
            <div>
              <h2 className="text-arkhe-cyan font-mono font-bold tracking-widest text-lg">
                AEGIS SHIELD
              </h2>
              <p className="text-xs text-arkhe-cyan/60 font-mono">
                v2.0.0 - UPGRADED
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-black/50 border border-arkhe-cyan/20 p-3 rounded">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs text-arkhe-cyan/70 font-mono uppercase">
                  System Status
                </span>
              </div>
            ...
            <div className="bg-black/50 border border-arkhe-cyan/20 p-3 rounded">
              ...
              <div
                className={`font-mono text-lg font-bold ${isCoherent ? 'text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.8)]' : 'text-arkhe-cyan'}`}
              >
                {lambda2.toFixed(6)}
              </div>
              <div className="w-full bg-gray-900 h-1.5 mt-2 rounded overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (lambda2 / 1.618033) * 100)}%`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="flex-1 p-0 relative bg-[#020202]">
          <div className="absolute top-4 left-4 z-10 pointer-events-none">
            <div className="text-xs font-mono text-arkhe-cyan/40">
              {'>'} VISUALIZING: KURAMOTO SYNC + MEMORY PHASE
              <br />
              {'>'} NODES: 64 (FRACTAL BUDDY ENABLED)
              <br />
