  ...
  const runAggregation = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (step === 2) {
        ...
        addLog(
          'CALCULANDO MÉDIA PONDERADA DOS PARÂMETROS DE ACOPLAMENTO (K) E FREQUÊNCIA (W)...',
        );
      } else if (step === 8) {
        ...
        const activeNodes = nodes.filter(
          n => n.status === 'synced' && n.coherence > 0.5,
        );
        ...
        addLog(
          `AGREGAÇÃO CONCLUÍDA. COERÊNCIA GLOBAL DO ENXAME: ${(totalCoherence / activeNodes.length).toFixed(4)}`,
        );
        addLog(
          `PARÂMETROS OTIMIZADOS: K=${finalK.toFixed(4)}, W=${finalW.toFixed(4)}`,
        );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs font-mono text-arkhe-muted">
                Agregação de Insights do Enxame de Bio-Nós
              </div>
            </div>
          </div>
          <button
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 overflow-hidden">
                ...
                {nodes.map((node, _i) => (
                  <div
                    ...
                    className={`p-3 rounded border font-mono text-xs ${
                      node.status === 'synced'
                        ? 'bg-arkhe-cyan/10 border-arkhe-cyan/30'
                        : node.status === 'syncing'
                          ? 'bg-arkhe-orange/10 border-arkhe-orange/30 animate-pulse'
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-blue-400">
                          {node.suggestedK.toFixed(2)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-arkhe-muted">Sug. W:</span>
                        <span className="text-purple-400">
                          {node.suggestedW.toFixed(2)}
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-4 flex flex-col">
            ...
            <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg flex-1 flex flex-col min-h-[200px]">
              <h3 className="text-sm font-mono text-arkhe-muted uppercase mb-2">
                Logs de Agregação
              </h3>
              <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-1">
                {logs.map((log, i) => (
                  <div
                    key={i}
                    className={`${i === 0 ? 'text-arkhe-cyan' : 'text-arkhe-muted opacity-70'}`}
                  >
                    {log}
                  </div>
                ))}
                {logs.length === 0 && (
                  <div className="text-arkhe-muted/50 italic">
                    Aguardando execução da subrotina...
                  </div>
                )}
              </div>
            </div>
