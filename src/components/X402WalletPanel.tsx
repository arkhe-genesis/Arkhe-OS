  ...
  const handleSimulatePayment = async () => {
    ...
    try {
      ...
      if (data.success) {
        setPaymentStatus({
          success: true,
          message: `Paid ${data.transaction.amount.toFixed(4)} USDC`,
        });
      } else {
        setPaymentStatus({
          success: false,
    ...
    <div className="bg-black/40 border border-arkhe-border/50 rounded-xl p-4 flex flex-col h-full">
      ...
      <div className="bg-[#1f2024]/50 p-4 rounded-lg border border-arkhe-purple/30 mb-4 flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-[10px] font-mono text-arkhe-purple hover:underline flex items-center gap-1"
          >
            <ExternalLink className="w-3 h-3" /> Etherscan
          </a>
        </div>
        <div className="text-xs font-mono text-arkhe-text break-all">
          {wallet.address}
        </div>
        ...
        <div className="mt-2 flex items-end justify-between">
          <div>
            ...
            <div className="text-2xl font-mono text-arkhe-purple flex items-center gap-2">
              ...
              {wallet.balanceUSDC.toFixed(4)}{' '}
              <span className="text-sm text-arkhe-muted">USDC</span>
            </div>
          </div>
      ...
      <div className="mb-4 space-y-2">
        ...
        {wallet.prometheusSync?.status === 'synced' ? (
          ...
          >
            {isPrometheusSyncing ||
            wallet.prometheusSync?.status === 'syncing' ? (
              <>
                <Loader2 className="w-3 h-3 animate-spin" />
              ...
              {wallet.transactions.map(tx => (
                ...
                >
                  <div className="flex justify-between items-start mb-1">
                    <div className="text-[10px] font-mono text-arkhe-cyan truncate flex-1 mr-2">
                      {tx.resource}
                    </div>
                    <div className="text-[10px] font-mono text-arkhe-purple shrink-0">
                      -{tx.amount.toFixed(4)} USDC
                    </div>
                  </div>
                  <div className="flex justify-between items-center">
                    <div className="text-[9px] font-mono text-arkhe-muted truncate max-w-[150px]">
                      {tx.provider}
                    </div>
                    <div className="text-[9px] font-mono text-arkhe-muted/50">
                      {tx.timestamp
