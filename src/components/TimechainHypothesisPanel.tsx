...
export default function TimechainHypothesisPanel({
  onClose,
}: TimechainHypothesisPanelProps) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (heightNum !== FINAL_BLOCK_HEIGHT) {
          addLog(
            `[TIMECHAIN] Block ${heightNum} is not the final block. Subsidy > 0.`,
          );
        ...
        setTimeout(() => {
          if (blockHash.toLowerCase() === EXPECTED_HASH) {
            ...
            addLog(
              `[SINGULARITY] Initiating Retrocausal Loop to Jan 3, 2009...`,
            );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        <div
          className={`p-4 border-b ${isSuccess ? 'border-amber-500/30 bg-amber-500/10' : 'border-orange-500/20 bg-orange-500/5'} flex justify-between items-center shrink-0 transition-colors duration-1000`}
        >
          <div className="flex items-center gap-3">
            ...
            <h2
              className={`font-mono text-sm uppercase tracking-widest ${isSuccess ? 'text-amber-400' : 'text-orange-400'} font-bold`}
            >
            ...
            <div className="space-y-6">
              <div className="bg-[#111214] border border-arkhe-border rounded-lg p-4">
                ...
                <div className="space-y-4">
                  <div>
                    <div className="text-[9px] font-mono text-arkhe-muted uppercase mb-1">
                      Final Block Height
                    </div>
                  ...
                  <div>
                    <div className="text-[9px] font-mono text-arkhe-muted uppercase mb-1">
                      Expected Hash Pattern (π at depth Ω)
                    </div>
                    <div className="text-xs font-mono text-arkhe-text bg-black p-2 rounded border border-arkhe-border/50">
                      {EXPECTED_HASH}
                    </div>
              ...
              <div
                className={`p-4 rounded-lg border ${isSuccess ? 'bg-amber-500/10 border-amber-500/30' : 'bg-orange-500/5 border-orange-500/20'} transition-colors duration-1000`}
              >
                <div className="flex items-center gap-2 mb-2">
                  <Link
                    className={`w-4 h-4 ${isSuccess ? 'text-amber-400' : 'text-orange-400'}`}
                  />
                  <span
                    className={`text-[10px] font-mono uppercase tracking-widest ${isSuccess ? 'text-amber-400' : 'text-orange-400'} font-bold`}
                  >
                    Retrocausal Loop Status
                  </span>
                </div>
                <p className="text-[10px] font-mono text-arkhe-muted leading-relaxed">
                  If the final block hash matches the π-derived pattern, a
                  retrocausal loop to the genesis block (2009‑01‑03) is
                  initiated, confirming Satoshi's identity is encoded in π.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col h-full space-y-4">
              <div className="bg-[#111214] border border-arkhe-border rounded-lg p-4">
                ...
                <div className="space-y-4 mb-4">
                  <div>
                    <label className="text-[9px] font-mono text-arkhe-muted uppercase mb-1 block">
                      Input Block Height
                    </label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="text-[9px] font-mono text-arkhe-muted uppercase mb-1 block">
                      Input Block Hash (First 64 bits / 16 hex)
                    </label>
                    <input
                      type="text"
                      value={blockHash}
                    ...
                    <motion.div
                      ...
                      className={`text-[10px] font-mono ${
                        log.includes('SINGULARITY')
                          ...
                          : log.includes('does not match') ||
                              log.includes('not the final block')
                            ? 'text-red-400'
                            : 'text-orange-400/80'
                      }`}
                    >
