      ...
      <div className="overflow-auto h-full pr-2">
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 bg-arkhe-card z-10">
            <tr>
              <th className="py-2 px-2 text-[10px] font-mono text-arkhe-muted uppercase border-b border-arkhe-border">
                Orb ID
              </th>
              <th className="py-2 px-2 text-[10px] font-mono text-arkhe-muted uppercase border-b border-arkhe-border">
                Origin Time
          ...
          <tbody className="font-mono text-xs">
            {logs.map((log, i) => {
              ...
              return (
                <tr
                  key={`${log.id}-${i}`}
                  className="border-b border-arkhe-border/50 hover:bg-[#151619] transition-colors"
                >
                  <td
                    className="py-2 px-2 text-arkhe-muted truncate max-w-[80px]"
                    title={log.id}
                  >
                    {log.id.substring(0, 8)}
                  </td>
                  <td
                    className={`py-2 px-2 ${isRejected ? 'text-arkhe-red' : 'text-arkhe-text'}`}
                  >
                  ...
                  <td
                    className={`py-2 px-2 ${log.coherence < 0.618 ? 'text-arkhe-red' : 'text-arkhe-cyan'}`}
                  >
                  ...
                  <td className="py-2 px-2">
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                        isRejected
                          ? 'bg-arkhe-red/20 text-arkhe-red'
                          : isMitigated
            ...
            {logs.length === 0 && (
              <tr>
                <td
                  colSpan={5}
                  className="py-4 text-center text-arkhe-muted text-xs font-mono"
                >
