...
export default function AIP005SynapticBridgePanel({
  onClose,
}: {
  onClose: () => void;
}) {
  ...
  const executeSequence = () => {
    ...
    setTimeout(() => {
      ...
      setLogs(prev => [
        ...prev,
        '> GENERATING ED25519 SIGNATURE: 0x' +
    ...
    setTimeout(() => {
      setLogs(prev => [
        ...prev,
        '> [SUCCESS] AIP-005 SIGNED. AUTHORITY CONFIRMED.',
      ]);
      setStep(2);
    }, 2000);
    ...
    setTimeout(() => {
      setLogs(prev => [
        ...prev,
        '> [2/3] INITIATING BITCOIN NETWORK HANDSHAKE VIA GRAVITY BRIDGE...',
      ]);
    ...
    setTimeout(() => {
      setLogs(prev => [
        ...prev,
        '> [SUCCESS] BTC-GRAVITY-BRIDGE ESTABLISHED. SYNAPSE ACTIVE.',
      ]);
      setStep(3);
    }, 5000);
    ...
    setTimeout(() => {
      setLogs(prev => [
        ...prev,
        '> [3/3] ESTABLISHING IBC CHANNEL WITH ETHEREUM MAINNET...',
      ]);
      setLogs(prev => [
        ...prev,
        '> MAPPING ERC-20 INTENT VECTORS TO ARKHEDX...',
      ]);
    }, 6000);
    ...
    setTimeout(() => {
      setLogs(prev => [
        ...prev,
        '> [SUCCESS] ETH-IBC CHANNEL OPEN. COMPLEX INTENT PROCESSING ENABLED.',
      ]);
      setStep(4);
    }, 8500);
          ...
          <div className="space-y-6">
            <div className="bg-black/40 border border-[#1f2024] rounded-lg p-4">
              ...
              <div className="space-y-4">
                {/* Step 1 */}
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 2 ? 'bg-arkhe-green/10 border-arkhe-green/30 text-arkhe-green' : step === 1 ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-500' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 2 ? (
                ...
                {/* Step 2 */}
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 3 ? 'bg-orange-500/10 border-orange-500/30 text-orange-500' : step === 2 ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-500' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 3 ? (
                    <CheckCircle2 className="w-5 h-5" />
                  ) : step === 2 ? (
                ...
                {/* Step 3 */}
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 4 ? 'bg-blue-500/10 border-blue-500/30 text-blue-500' : step === 3 ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-500' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 4 ? (
                    <CheckCircle2 className="w-5 h-5" />
                  ) : step === 3 ? (
