...
export default function ArkheGame({onClose}: {onClose: () => void}) {
  ...
  const [message, setMessage] = useState(
    'Inicie o Ritual de Coerência (Substrato 43)',
  );
      ...
      >
        ...
        <div className="p-4 border-b border-arkhe-border flex justify-between items-center bg-white/5">
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <div className="text-[10px] text-arkhe-muted uppercase font-mono">
                Score
              </div>
            ...
            <button
              onClick={onClose}
              className="p-2 text-arkhe-muted hover:text-white transition-colors"
            >
              [SAIR]
            </button>
          </div>
          ...
          {!isPlaying ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-arkhe-muted max-w-md mb-8 font-serif leading-relaxed">
                Mantenha a simetria da Catedral Arkhe(N). Equilibre os
                substratos, dissipe anomalias e impeça o colapso da fase global.
              </p>
              <button
                onClick={startGame}
            ...
            <>
              ...
              <div className="absolute top-6 left-6 right-6">
                <div className="flex justify-between items-end mb-2">
                  <span className="text-[10px] font-mono text-arkhe-muted uppercase tracking-widest">
                    Global Coherence (Ω)
                  </span>
                  <span
                    className={`text-sm font-mono font-bold ${coherence < 30 ? 'text-arkhe-fissure animate-pulse' : 'text-arkhe-cyan'}`}
                  >
                    {coherence.toFixed(2)}%
                  </span>
                </div>
                <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/10">
                  <motion.div
                    ...
                    animate={{
                      width: `${coherence}%`,
                      backgroundColor: coherence < 30 ? '#E11D48' : '#00E5FF',
                    }}
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="relative w-64 h-64 border border-white/5 rounded-full flex items-center justify-center">
                  ...
                  <div className="w-32 h-32 bg-arkhe-cyan/5 rounded-full flex items-center justify-center backdrop-blur-md border border-arkhe-cyan/20">
                    <Shield
                      className={`w-12 h-12 ${coherence < 30 ? 'text-arkhe-fissure' : 'text-arkhe-cyan'} transition-colors duration-500`}
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="bg-black/40 p-6 border-t border-arkhe-border grid grid-cols-1 md:grid-cols-4 gap-4">
          {substrates.map(sub => (
            <div
              key={sub.id}
              className="space-y-3 bg-white/5 p-4 rounded-lg border border-white/5 group hover:border-arkhe-cyan/30 transition-all"
            >
              ...
              <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{width: `${sub.level}%`, backgroundColor: sub.color}}
                />
              </div>
        ...
        <div className="p-3 bg-arkhe-cyan/5 border-t border-arkhe-border flex items-center justify-between px-6">
          <div className="flex items-center gap-2">
            <RefreshCw
              className={`w-3 h-3 text-arkhe-cyan ${isPlaying ? 'animate-spin' : ''}`}
            />
