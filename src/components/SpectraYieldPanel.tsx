...
export default function SpectraYieldPanel({
  spectra,
  onClose,
}: SpectraYieldPanelProps) {
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] font-mono text-arkhe-muted uppercase">
                    TVL ($M)
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-[#0A0E17] z-10">
                  <tr className="border-b border-arkhe-border/50">
                    <th className="px-4 py-3 text-[9px] font-mono text-arkhe-muted uppercase">
                      Vault
                    </th>
                    <th className="px-4 py-3 text-[9px] font-mono text-arkhe-muted uppercase">
                      Chain
                    </th>
                    <th className="px-4 py-3 text-[9px] font-mono text-arkhe-muted uppercase text-right">
                      TVL
                    </th>
                    <th className="px-4 py-3 text-[9px] font-mono text-arkhe-muted uppercase text-right">
                      APY
                    </th>
                    <th className="px-4 py-3 text-[9px] font-mono text-arkhe-muted uppercase text-center">
                      Epoch
                    </th>
                    <th className="px-4 py-3 text-[9px] font-mono text-arkhe-muted uppercase text-center">
                      Protocol
                ...
                <tbody>
                  {spectra.vaults.map(vault => (
                    <tr
                      key={vault.id}
                      className="border-b border-arkhe-border/20 hover:bg-arkhe-cyan/5 transition-colors group"
                    >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#151B26]/80 rounded-xl border border-arkhe-border/50 p-5 space-y-4 shadow-[0_10px_30px_rgba(0,0,0,0.3)]">
            ...
            {spectra.oracles.map((oracle, i) => (
              <div
                key={i}
                className="bg-black/40 p-3 rounded-lg border border-arkhe-border/50 relative overflow-hidden group"
              >
                <div className="absolute right-0 top-0 bottom-0 w-1 bg-arkhe-cyan/30 group-hover:bg-arkhe-cyan transition-colors" />
                <div className="flex justify-between items-start mb-2">
                  <div className="text-[10px] font-mono text-arkhe-text uppercase">
                    {oracle.marketId}
                  </div>
                  <div
                    className={`text-[9px] font-mono px-1.5 py-0.5 rounded ${oracle.tokenType === 'PT' ? 'bg-blue-500/20 text-blue-400' : 'bg-arkhe-purple/20 text-arkhe-purple'}`}
                  >
                    {oracle.tokenType}
                  </div>
                </div>
                <div className="flex justify-between items-end">
                  <div>
                    <div className="text-[9px] font-mono text-arkhe-muted uppercase">
                      Latest Price
                    </div>
                    <div className="text-sm font-mono text-arkhe-cyan font-bold">
                      {oracle.price.toFixed(4)}{' '}
                      <span className="text-[9px] font-normal">Asset</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[9px] font-mono text-arkhe-muted uppercase">
                    ...
                    <div className="text-[10px] font-mono text-arkhe-green">
                      {oracle.confidence.toFixed(4)}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-arkhe-purple/5 border border-arkhe-purple/20 rounded-xl p-5 space-y-3">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[9px] font-mono text-arkhe-muted">
                    Standardized Yield Derivatives
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3">
            ...
            <div className="pt-2">
              <a
                href="https://dev.spectra.finance"
                target="_blank"
                rel="noreferrer"
                className="text-[9px] font-mono text-arkhe-muted hover:text-arkhe-cyan flex items-center gap-1"
              >
