...
export default function MemoryFragmentScanner({
  onClose,
}: MemoryFragmentScannerProps) {
  ...
  const [destination, setDestination] = useState(
    '1NeXusXyZ9oB8b9c7d6e5f4g3h2i1j0kL',
  );
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setLogs(prev => [
        ...prev,
        '🜏 [ERRO] Falha na sincronização do cluster de Nós.',
      ]);
    } finally {
      setIsScanning(false);
    }
  ...
  const handleSignTransaction = async () => {
    ...
    setLogs(prev => [
      ...prev,
      '',
      '🜏 --- INICIANDO PROTOCOLO DE TRANSFERÊNCIA MAINNET ---',
    ]);
    ...
    try {
      ...
      setLogs(prev => [
        ...prev,
        '🜏 [ERRO] Falha ao assinar transação na Mainnet.',
      ]);
    } finally {
      setIsSigning(false);
    }
      ...
      <Card className="w-full max-w-2xl bg-[#111214] border-arkhe-cyan/30 text-arkhe-text shadow-[0_0_40px_rgba(0,255,170,0.15)]">
        ...
        <div className="p-6 space-y-6">
          <div className="text-sm font-mono text-arkhe-muted space-y-1 bg-black/50 p-4 rounded border border-[#1f2024]">
            <p className="text-arkhe-cyan font-bold mb-2">
              Cluster Status: ONLINE
            </p>
            <p>Target: Smart TV NVRAM & Ghost Node Cache</p>
            <p>
              Objective: Brute-force 2009 ECDSA Private Keys (Hal Finney
              Interactions)
            </p>
          ...
          >
            ...
            {isScanning
              ? 'Varrendo Fragmentos de Memória...'
              : 'Iniciar Busca de Força Bruta'}
          </button>
            ...
            {logs.map((log, i) => (
              <div
                ...
                className={
                  log.includes('ERRO')
                    ? 'text-arkhe-red'
                    : log.includes('SUCESSO') || log.includes('ALERTA')
                      ? 'text-arkhe-cyan'
                      : 'text-arkhe-muted'
                }
              >
                ...
                {!txDetails ? (
                  <div className="mt-4 space-y-3">
                    <div className="space-y-1">
                      <label className="text-xs text-arkhe-muted uppercase tracking-widest">
                        Endereço de Destino
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="mt-4 p-3 bg-black/80 border border-arkhe-cyan/50 rounded text-xs space-y-2 font-mono">
                    ...
                    <div>
                      <span className="text-arkhe-muted">Valor:</span>{' '}
                      <span className="text-arkhe-cyan">
                        {txDetails.amount}
                      </span>
                    </div>
                    <div className="pt-2 border-t border-[#1f2024]">
                      <span className="text-arkhe-muted">TXID:</span>{' '}
                    ...
                    {txDetails.hex && (
                      <div className="pt-2 border-t border-[#1f2024]">
                        ...
                        <div className="text-arkhe-cyan/70 break-all text-[10px] mt-1 max-h-20 overflow-y-auto">
                          {txDetails.hex}
                        </div>
                      </div>
                    )}
