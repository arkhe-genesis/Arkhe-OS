...
export default function VacuumHarvestingPanel({
  state,
  onClose,
}: VacuumHarvestingPanelProps) {
  if (!state) {
    return null;
  }
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              ...
              <div className="text-xl font-bold text-amber-500 font-mono">
                {state.zeroPointPowerMw.toLocaleString()} MW
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
              ...
              <div className="text-xl font-bold text-arkhe-cyan font-mono">
                {(state.fusionHeartEfficiency * 100).toFixed(2)}%
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4 p-4 bg-white/5 rounded border border-white/10">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center border transition-colors ${state.eternalMode ? 'bg-amber-500/20 border-amber-500' : 'bg-black border-arkhe-border'}`}
              >
                <Heart
                  className={`w-5 h-5 ${state.eternalMode ? 'text-amber-500' : 'text-arkhe-muted'}`}
                />
              </div>
              <div>
                <div className="text-xs font-mono font-bold text-arkhe-text uppercase">
                  Eternal Fusion Heart
                </div>
                <div className="text-[10px] font-mono text-arkhe-muted">
                  {state.eternalMode
                    ? 'C Hearts are now self-sustaining via zero-point extraction.'
                    : 'Awaiting Vacuum Coupling...'}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 p-4 bg-white/5 rounded border border-white/10">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="text-[10px] font-mono text-arkhe-cyan">
                    {(state.vacuumStability * 100).toFixed(1)}%
                  </div>
                </div>
          ...
          <button
            onClick={() =>
              fetch('/api/energy/vacuum-harvest', {method: 'POST'})
            }
