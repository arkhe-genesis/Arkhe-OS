...
export default function ArkheVisionPanel({onClose}: ArkheVisionPanelProps) {
  ...
  const triggerScan = () => {
    ...
    setTimeout(() => {
      ...
      const mockResponses: MinerResponse[] = Array.from({length: 5}).map(
        (_, _i) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          // Aggregate
          const intruderCount = mockResponses.filter(
            r => r.result === 'intruder_detected',
          ).length;
          const finalClass =
            intruderCount > 2 ? 'intruder_detected' : 'authorized_personnel';
          const avgConfidence =
            mockResponses.reduce((acc, curr) => acc + curr.confidence, 0) /
            mockResponses.length;

          // Boost coherence if consensus is strong
          const finalCoherence =
            intruderCount === 5 || intruderCount === 0
              ? Math.min(0.999, avgConfidence + 0.05)
              : avgConfidence;

          const invariant = Array.from({length: 64}, () =>
          ...
          addLog(
            `CONSENSO ATINGIDO: ${finalClass.toUpperCase()} (λ₂ = ${finalCoherence.toFixed(4)})`,
          );
          addLog(
            `INVARIANTE TOPOLÓGICO GERADO: ${invariant.substring(0, 16)}...`,
          );
          addLog(
            'EVENTO Z REGISTRADO NO PARSEABLE (Stream: arkhen-security-logs).',
          );
          ...
          if (finalCoherence > 0.95 && finalClass === 'intruder_detected') {
            setTimeout(() => {
              ...
              addLog(
                'PLAYBOOK: Bloqueio de acesso físico e alerta para equipe de segurança.',
              );
            }, 1000);
          }
      ...
      <div className="w-full max-w-5xl bg-arkhe-card border border-arkhe-cyan/30 rounded-xl shadow-[0_0_30px_rgba(0,255,170,0.1)] overflow-hidden flex flex-col h-[85vh]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-arkhe-cyan/20 bg-arkhe-cyan/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-arkhe-cyan tracking-widest uppercase">
                Arkhe-Vision (Subnet 44)
              </h2>
              <div className="text-xs font-mono text-arkhe-muted">
                Integração Bittensor - Inteligência Visual Descentralizada
              </div>
            </div>
          </div>
          <button
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-black/40 border border-arkhe-border p-4 rounded-lg flex flex-col items-center justify-center relative overflow-hidden aspect-video">
              ...
              <div className="relative z-10 text-center">
                <Eye
                  className={`w-12 h-12 mx-auto mb-2 ${isScanning ? 'text-arkhe-cyan animate-pulse' : 'text-arkhe-muted'}`}
                />
                <div className="text-xs font-mono text-arkhe-muted uppercase">
                  Câmera Setor 7G
                </div>
                {finalResult && (
                  <div
                    className={`mt-2 px-3 py-1 text-xs font-bold uppercase border rounded ${
            ...
            {playbookTriggered && (
              <div className="mt-auto bg-arkhe-red/10 border border-arkhe-red/50 p-4 rounded-lg animate-pulse">
                ...
                <div className="text-xs text-arkhe-red/80 font-mono">
                  Bloqueio de acesso físico ativado. Equipe de segurança
                  notificada. Invariante registrado na Arkhe-Chain.
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-black/40 border border-arkhe-border p-4 rounded-lg flex flex-col overflow-hidden">
              ...
              {scanStep >= 2 ? (
                minerResponses.length > 0 ? (
                  minerResponses.map((miner, i) => (
                    <div
                      key={i}
                      className="p-3 bg-arkhe-card border border-arkhe-border rounded flex justify-between items-center"
                    >
                      <div>
                        <div className="text-xs font-mono text-arkhe-muted">
                          {miner.minerId}
                        ...
                        <div
                          className={`text-sm font-bold uppercase ${miner.result === 'intruder_detected' ? 'text-arkhe-red' : 'text-arkhe-green'}`}
                        >
                          {miner.result.replace('_', ' ')}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs text-arkhe-muted">
                          Confiança
                        </div>
                        <div className="text-sm font-mono text-arkhe-cyan">
                          {(miner.confidence * 100).toFixed(1)}%
                        </div>
                      </div>
                    </div>
                  ))
            ...
            {finalResult && (
              <div className="mt-4 p-4 bg-arkhe-cyan/10 border border-arkhe-cyan/30 rounded-lg">
                <div className="text-xs text-arkhe-muted uppercase mb-1">
                  Coerência Global (λ₂)
                </div>
            ...
            <div className="flex-1 overflow-y-auto font-mono text-[10px] space-y-2">
              {logs.map((log, i) => (
                <div
                  key={i}
                  className={`${
                    log.includes('ALTA COERÊNCIA') || log.includes('PLAYBOOK')
                      ? 'text-arkhe-red font-bold'
                      : log.includes('CONSENSO')
                        ? 'text-arkhe-cyan'
