...
export default function UniversalWitnessPanel({
  state,
  onClose,
}: UniversalWitnessPanelProps) {
  if (!state) {
    return null;
  }
      ...
      >
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              ...
              <div className="text-xl font-bold text-arkhe-cyan font-mono">
                {state.crossCorrelationSigma.toFixed(1)}σ
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
              ...
              <div className="text-xl font-bold text-emerald-500 font-mono">
                {(state.resonatorCoupling * 100).toFixed(1)}%
              </div>
            </div>
            <div className="bg-black/40 p-4 rounded border border-white/5 space-y-1">
              <div className="text-[10px] text-arkhe-muted uppercase">
          ...
          <div className="bg-black/60 border border-arkhe-border rounded-lg p-10 flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden">
            <div className="flex gap-12 items-center z-10">
              <div className="flex flex-col items-center gap-2">
                <Globe
                  className={`w-12 h-12 ${state.icmActive ? 'text-arkhe-cyan animate-pulse' : 'text-arkhe-muted'}`}
                />
              ...
              <div className="flex flex-col items-center gap-2">
                <Share2
                  className={`w-12 h-12 ${state.icmActive ? 'text-arkhe-purple animate-pulse' : 'text-arkhe-muted'}`}
                />
                <span className="text-[9px] font-mono text-arkhe-muted uppercase">
                  Multiversal
                </span>
            ...
            <div className="mt-12 grid grid-cols-2 gap-8 w-full max-w-md">
              <div className="space-y-1">
                ...
                <div className="w-full bg-white/5 h-1 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(state.aggregateInvariants.cosmicCHSH / 2.82) * 100}%`,
                    }}
                  />
                </div>
              </div>
              <div className="space-y-1">
                <div className="flex justify-between text-[8px] font-mono text-arkhe-muted uppercase">
                  ...
                  <span>
                    {state.aggregateInvariants.multiverseEntropy.toFixed(2)}
                  </span>
                </div>
                <div className="w-full bg-white/5 h-1 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-arkhe-purple"
                    style={{
                      width: `${(state.aggregateInvariants.multiverseEntropy / 3.0) * 100}%`,
                    }}
                  />
                </div>
              </div>
          ...
          <div className="flex gap-4">
            <button
              onClick={() =>
                fetch('/api/universal/witness/resonate', {method: 'POST'})
              }
            ...
            >
              {state.icmActive
                ? 'Resonator Online'
                : 'Activate Invariant Resonator'}
            </button>
            <button
              onClick={() =>
                fetch('/api/universal/witness/integrate', {method: 'POST'})
              }
