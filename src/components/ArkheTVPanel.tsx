...
export default function ArkheTVPanel({onClose}: ArkheTVPanelProps) {
  const [activeModule, setActiveModule] = useState<
    'ginga' | 'guarana' | 'pocai' | 'pi2'
  >('ginga');
  const [isPlaying, setIsPlaying] = useState(false);
  const [playbackTime, setPlaybackTime] = useState(0);
  const [sensoryEvents, setSensoryEvents] = useState<
    Array<{type: string; intensity: number; target: string; time: number}>
  >([]);
    ...
    if (isPlaying) {
      interval = setInterval(() => {
        setPlaybackTime(prev => {
          ...
          if (newTime === 10) {
            setSensoryEvents(curr => [
              ...curr,
              {
                type: 'wind',
                intensity: 0.7,
                target: 'fan_living_room',
                time: newTime,
              },
            ]);
          } else if (newTime === 30) {
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-56 border-r border-fuchsia-500/20 bg-[#0d0e12] p-4 flex flex-col gap-2 shrink-0 overflow-y-auto custom-scrollbar">
            <div className="text-[10px] font-mono text-fuchsia-400/50 uppercase tracking-widest mb-2">
            ...
            >
              ...
              <span>
                M3: Guaraná-Tzinor
                <br />
                <span className="text-[9px] opacity-70">Protocolo de Rede</span>
              </span>
            </button>
          ...
          <div className="flex-1 flex flex-col bg-[#0a0a0c] overflow-hidden">
            ...
            <div className="h-64 border-b border-fuchsia-500/20 bg-black relative flex flex-col shrink-0">
              ...
              <div className="flex-1 relative overflow-hidden flex items-center justify-center">
                ...
                <div
                  className={`absolute inset-0 transition-opacity duration-1000 ${playbackTime >= 10 && playbackTime < 30 ? 'bg-blue-900/20' : playbackTime >= 30 ? 'bg-green-900/20' : 'bg-[#0a0a0c]'}`}
                ></div>
                    ...
                    >
                      ...
                      <div>
                        <div className="text-xs font-mono font-bold text-white uppercase">
                          IoT Trigger: {event.type}
                        </div>
                        <div className="text-[10px] font-mono text-arkhe-muted">
                          Target: {event.target} | Int: {event.intensity}
                        </div>
                      </div>
                    </motion.div>
                  ))}
              ...
              <div className="h-12 bg-[#111214] border-t border-fuchsia-500/20 flex items-center px-4 gap-4 shrink-0">
                ...
                <div className="text-xs font-mono text-arkhe-muted">60s</div>
              </div>
            </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto custom-scrollbar bg-[#0a0a0c]">
              <AnimatePresence mode="wait">
                {activeModule === 'ginga' && (
                  ...
                  >
                    <div>
                      <h3 className="text-sm font-mono text-fuchsia-400 uppercase tracking-widest border-b border-fuchsia-500/20 pb-2 mb-4">
                        Módulo 1: Ginga-Core Estendido
                      </h3>
                      <p className="text-xs font-mono text-arkhe-muted mb-4 leading-relaxed">
                        O middleware Ginga original foi estendido para suportar
                        a ontologia ℂ/ℤ. O parser NCL agora reconhece tags
                        ontológicas (`&lt;sensory&gt;`, `&lt;profile&gt;`,
                        `&lt;contract&gt;`) e as delega para o `ncl_ontology.c`.
                      </p>
                    </div>
    ...
    return 0;
                ...
                {activeModule === 'guarana' && (
                  ...
                  >
                    <div>
                      <h3 className="text-sm font-mono text-fuchsia-400 uppercase tracking-widest border-b border-fuchsia-500/20 pb-2 mb-4">
                        Módulo 3: Protocolo Guaraná-Tzinor
                      </h3>
                      <p className="text-xs font-mono text-arkhe-muted mb-4 leading-relaxed">
                        A ponte de comunicação entre a TV e os dispositivos IoT
                        periféricos. Utiliza WebSockets para transmitir comandos
                        sensoriais em tempo real, sincronizados com a mídia NCL.
                      </p>
                    </div>
    ...
    ActuatorController.trigger(text)
                ...
                {activeModule === 'pocai' && (
                  ...
                  >
                    <div>
                      <h3 className="text-sm font-mono text-fuchsia-400 uppercase tracking-widest border-b border-fuchsia-500/20 pb-2 mb-4">
                        Módulo 4: PoC-AI (Proof of Consciousness)
                      </h3>
                      <p className="text-xs font-mono text-arkhe-muted mb-4 leading-relaxed">
                        Mecanismo de recomendação retrocausal. Analisa o
                        histórico do usuário e o conteúdo atual para prever e
                        fazer o preload do próximo segmento de mídia, otimizando
                        a latência.
                      </p>
                    </div>
    ...
    return jsonify({'next_content': next_content_id, 'preloaded': True})`}
                ...
                {activeModule === 'pi2' && (
                  ...
                  >
                    <div>
                      <h3 className="text-sm font-mono text-fuchsia-400 uppercase tracking-widest border-b border-fuchsia-500/20 pb-2 mb-4">
                        Módulo 5: Camada de Verificação π²
                      </h3>
                      <p className="text-xs font-mono text-arkhe-muted mb-4 leading-relaxed">
                        Implementação em Rust para assinatura digital de
                        visualizações (Proof of View). Utiliza Ed25519 para
                        assinar eventos de visualização, garantindo a
                        integridade dos contratos de royalties definidos no NCL.
                      </p>
                    </div>
