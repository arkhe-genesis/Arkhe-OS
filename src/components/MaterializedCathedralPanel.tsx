...
export default function MaterializedCathedralPanel({
  state,
  onClose,
}: MaterializedCathedralPanelProps) {
  if (!state) {
    return null;
  }
      ...
      >
        <div className="p-4 border-b border-emerald-500/20 flex justify-between items-center bg-emerald-500/5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-mono text-sm uppercase tracking-widest text-emerald-500 font-bold">
              Catedral Materializada (Atómos Neutros)
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-black/40 p-3 rounded border border-white/5 space-y-1">
              <div className="text-[9px] text-arkhe-muted uppercase">
                Physical Qubits
              </div>
              <div className="text-lg font-bold text-emerald-400 font-mono">
                {state.totalPhysicalQubits.toLocaleString()}
              </div>
            </div>
            <div className="bg-black/40 p-3 rounded border border-white/5 space-y-1">
              <div className="text-[9px] text-arkhe-muted uppercase">
              ...
              <div className="text-lg font-bold text-arkhe-text font-mono">
                {state.logicalQubits.toLocaleString()}
              </div>
            </div>
            <div className="bg-black/40 p-3 rounded border border-white/5 space-y-1">
              <div className="text-[9px] text-arkhe-muted uppercase">
              ...
              <div className="text-lg font-bold text-arkhe-text font-mono truncate">
                {state.memoryCode}
              </div>
            </div>
            <div className="bg-black/40 p-3 rounded border border-white/5 space-y-1">
              ...
              <div className="text-lg font-bold text-arkhe-cyan font-mono">
                {state.stabilizerCycleMs.toFixed(1)} ms
              </div>
            </div>
          </div>
            ...
            <div className="lg:col-span-1 space-y-4">
              ...
              <div className="space-y-2">
                {state.zones.map(z => (
                  <div
                    key={z.name}
                    className="bg-white/5 border border-white/10 p-3 rounded space-y-2"
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-mono font-bold text-arkhe-text uppercase">
                        {z.name}
                      </span>
                      <span
                        className={`text-[8px] font-mono font-bold ${z.status === 'ACTIVE' ? 'text-emerald-500' : 'text-amber-500 animate-pulse'}`}
                      >
                        {z.status}
                      </span>
                    </div>
            ...
            <div className="lg:col-span-2 bg-black/60 border border-arkhe-border rounded-lg p-8 flex flex-col items-center justify-center relative min-h-[300px]">
              ...
              <div className="mt-8 text-center z-10 bg-black/80 p-2 rounded border border-white/5">
                <div className="text-xs font-mono text-emerald-400 font-bold uppercase tracking-widest">
                  qLDPC Parallel Surgery Online
                </div>
                <div className="text-[9px] font-mono text-arkhe-muted mt-1 italic">
                  Extrapolated Error Rate: ~10⁻¹¹ / cycle
                </div>
              </div>
            </div>
          </div>
          ...
          <button
            onClick={() =>
              fetch('/api/cathedral/materialize', {method: 'POST'})
            }
