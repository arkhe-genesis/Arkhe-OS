...
export default function PluralityMCPPanel({onClose}: PluralityMCPPanelProps) {
  const [step, setStep] = useState<'intro' | 'authorizing' | 'connected'>(
    'intro',
  );
  const [activeTab, setActiveTab] = useState<'status' | 'tools' | 'logs'>(
    'status',
  );
  ...
  const handleConnect = async () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(async () => {
        ...
        try {
          const res = await fetch('/api/mcp/connect-plurality', {
            method: 'POST',
          });
          ...
          if (data.success) {
            ...
            addLog(
              'Plurality Tools discovered: get_user_memory_buckets, query_social_graph',
            );
            setStep('connected');
          } else {
          ...
          addLog(
            'ERR_RES: 522 Connection Timed Out - Failed to reach backend substrate.',
          );
          setStep('intro');
        }
      }, 1500);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold uppercase tracking-widest text-arkhe-text">
                Plurality Network // MCP Bridge
              </h2>
              <p className="text-[10px] font-mono text-arkhe-cyan/60 uppercase">
                Sovereign Social Graph & Memory Buckets
              </p>
            </div>
          </div>
          <button
              ...
              >
                <div className="bg-white/5 border border-white/10 p-4 rounded-lg space-y-3">
                  <p className="text-arkhe-text leading-relaxed">
                    Integrate your{' '}
                    <span className="text-arkhe-cyan">Plurality Network</span>{' '}
                    identity and data buckets directly into the Arkhe ecosystem
            ...
            {step === 'authorizing' && (
              ...
              >
                ...
                <div className="text-center space-y-2">
                  <h3 className="text-arkhe-cyan font-bold uppercase tracking-tighter text-lg">
                    Authorizing Handshake
                  </h3>
                  <p className="text-arkhe-muted animate-pulse">
                    Waiting for Plurality OAuth response...
                  </p>
                </div>
              </motion.div>
            )}
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-3 border border-arkhe-border rounded bg-black/20">
                          <div className="text-[10px] text-arkhe-muted uppercase mb-1">
                            Server URL
                          </div>
                          <div className="truncate text-arkhe-cyan">
                  ...
                  {activeTab === 'tools' && (
                    <div className="space-y-2 pt-2">
                      {[
                        {
                          name: 'get_user_memory_buckets',
                          desc: 'Retrieve encrypted memory buckets for the authorized user.',
                        },
                        {
                          name: 'query_social_graph',
                          desc: 'Map social relations across decentralised identifiers.',
                        },
                        {
                          name: 'write_context_fragment',
                          desc: 'Commit a new memory fragment to the Plurality storage layer.',
                        },
                      ].map(tool => (
                        <div
                          key={tool.name}
                          className="p-3 border border-arkhe-border rounded bg-black/20 hover:border-arkhe-cyan/30 transition-colors group"
                        >
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-arkhe-cyan font-bold">
                              {tool.name}
                            </span>
                  ...
                  {activeTab === 'logs' && (
                    <div className="space-y-1 pt-2 font-mono text-[10px]">
                      {logs.map((log, i) => (
                        <div
                          key={i}
                          className="text-arkhe-muted border-l border-arkhe-cyan/30 pl-2"
                        >
                          {log}
                        </div>
                      ))}
