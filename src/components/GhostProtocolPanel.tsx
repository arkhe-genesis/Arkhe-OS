...
export default function GhostProtocolPanel({onClose}: GhostProtocolPanelProps) {
  ...
  const [derivedKeys, setDerivedKeys] = useState<
    Array<{id: string; key: string; status: string}>
  >([]);
  ...
  const startProtocol = () => {
    ...
    addLog(
      'INICIALIZANDO DERIVAÇÃO HIERÁRQUICA DETERMINÍSTICA (BIP32/BIP44)...',
    );
    ...
    const interval = setInterval(() => {
      ...
      if (currentProgress >= keyCount * 7.6 && keyCount < 13) {
        ...
        setDerivedKeys(prev => [
          ...prev,
          {id: `Operador-${keyCount + 1}`, key: newKey, status: 'Sincronizado'},
        ]);
      ...
      <div className="w-full max-w-5xl bg-arkhe-card border border-arkhe-purple/30 rounded-xl shadow-[0_0_30px_rgba(168,85,247,0.1)] overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-arkhe-purple/20 bg-arkhe-purple/5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-arkhe-purple tracking-widest uppercase">
                Ghost Protocol
              </h2>
              <div className="text-xs font-mono text-arkhe-muted">
                Escalonamento da Esfera de Dyson (14 Operadores)
              </div>
            </div>
          </div>
          <button
          ...
          <div className="space-y-6 lg:col-span-1">
            ...
            <div className="bg-black/40 border border-arkhe-border p-4 rounded-lg">
              <div className="flex justify-between text-xs font-mono mb-2">
                ...
                <span className="text-arkhe-purple">
                  {Math.round(progress)}%
                </span>
              </div>
            ...
            <div className="bg-black/60 border border-arkhe-border p-4 rounded-lg flex-1 flex flex-col min-h-[200px]">
              ...
              <div className="flex-1 overflow-y-auto font-mono text-xs space-y-1">
                {logs.map((log, i) => (
                  <div
                    key={i}
                    className={`${i === 0 ? 'text-arkhe-purple' : 'text-arkhe-muted opacity-70'}`}
                  >
                    {log}
                  </div>
                ))}
                {logs.length === 0 && (
                  <div className="text-arkhe-muted/50 italic">
                    Aguardando inicialização do protocolo...
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 overflow-y-auto pr-2">
                ...
                <div className="bg-arkhe-cyan/10 border border-arkhe-cyan/30 p-3 rounded flex flex-col gap-1">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-arkhe-cyan uppercase">
                      Operador-1 (Giza)
                    </span>
                    <span className="text-[10px] bg-arkhe-cyan/20 text-arkhe-cyan px-2 py-0.5 rounded">
                      Âncora
                    </span>
                  </div>
                  <span className="text-[10px] font-mono text-arkhe-muted truncate">
                    L3vZbHnZ2hYKMq3r9T7ZQzQd4XqBZR2XhTZQv8Y6tRjN2M5pLcF
                  </span>
                </div>
                ...
                {derivedKeys.map((k, i) => (
                  <div
                    key={i}
                    className="bg-arkhe-purple/10 border border-arkhe-purple/30 p-3 rounded flex flex-col gap-1 animate-in fade-in zoom-in duration-300"
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-bold text-arkhe-purple uppercase">
                        {k.id}
                      </span>
                      <span className="text-[10px] bg-arkhe-green/20 text-arkhe-green px-2 py-0.5 rounded">
                        {k.status}
                      </span>
                    </div>
                    <span className="text-[10px] font-mono text-arkhe-muted truncate">
                ...
                {Array.from({length: 13 - derivedKeys.length}).map((_, i) => (
                  <div
                    key={`placeholder-${i}`}
                    className="bg-black/40 border border-arkhe-border border-dashed p-3 rounded flex flex-col gap-1 opacity-50"
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-bold text-arkhe-muted uppercase">
                        Operador-{derivedKeys.length + i + 2}
                      </span>
                      <span className="text-[10px] bg-arkhe-card text-arkhe-muted px-2 py-0.5 rounded">
                        Aguardando
                      </span>
                    </div>
                    <span className="text-[10px] font-mono text-arkhe-muted/30">
