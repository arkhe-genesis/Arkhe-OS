...
export default function WhisperLibraryPanel({
  state,
  onClose,
}: WhisperLibraryPanelProps) {
  if (!state) {
    return null;
  }
      ...
      >
        <div className="p-4 border-b border-amber-500/20 flex justify-between items-center bg-amber-500/5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-mono text-sm uppercase tracking-widest text-amber-500 font-bold">
              Whisper Library (Multi-Material)
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {state.materials.map(m => (
              <div
                key={m.name}
                className="bg-black/40 p-4 rounded border border-white/10 space-y-3"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-sm font-mono font-bold text-arkhe-text uppercase">
                      {m.name}
                    </h3>
                    <div className="text-[9px] font-mono text-arkhe-muted italic">
                      Hardness: {m.mohsHardness} Mohs
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {m.phononPeaksTHz.map((p, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 rounded text-[8px] font-mono text-amber-400"
                      >
                        {p} THz
                      </span>
                    ))}
            ...
            <button
              onClick={() =>
                fetch('/api/whisper/library/register', {
                  method: 'POST',
                  headers: {'Content-Type': 'application/json'},
                  body: JSON.stringify({
                    name: 'Silicon Carbide',
                    hardness: 9.5,
                    phononPeaks: [20, 40, 60],
                    chirp: 1400,
                  }),
                })
              }
            ...
            >
              ...
              <span className="text-[10px] font-mono text-arkhe-muted group-hover:text-arkhe-text uppercase tracking-widest">
        ...
        <div className="p-4 border-t border-white/5 bg-black/40 flex items-center justify-between">
          <div className="text-[10px] font-mono text-arkhe-muted uppercase">
            Total Materials Persuaded:{' '}
            <span className="text-amber-500">{state.materials.length}</span>
          </div>
          <div className="flex items-center gap-2">
            <Zap className="w-3 h-3 text-amber-500 animate-pulse" />
            <span className="text-[9px] font-mono text-arkhe-muted italic italic">
              "The Invariance Symphony"
            </span>
          </div>
        </div>
      </motion.div>
