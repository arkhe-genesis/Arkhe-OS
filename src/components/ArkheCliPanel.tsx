...
export default function ArkheCliPanel({onClose}: {onClose: () => void}) {
  ...
  const executeSequence = async () => {
    ...
    await addLog(
      'Mnemonic: quantum void coherence tensor entropy flux manifold nexus prism echo vector sync',
      1500,
    );
    setStep(3);
    ...
    await addLog(
      '$ arkhe-cli config set node https://rpc.testnet.arkhe.network:26657',
      1000,
    );
  ...
  "ValidatorInfo": {
    ...
    await addLog(
      '\n[SUCCESS] Arkhe CLI configured for testnet (arkhe-asi-1).',
      1000,
    );
        ...
        <div className="p-6 flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-6 md:col-span-1">
            <div className="bg-black/40 border border-[#1f2024] rounded-lg p-4">
              ...
              <div className="space-y-4">
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 2 ? 'bg-arkhe-green/10 border-arkhe-green/30 text-arkhe-green' : step === 1 ? 'bg-arkhe-cyan/10 border-arkhe-cyan/30 text-arkhe-cyan' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 2 ? (
                ...
                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 3 ? 'bg-arkhe-green/10 border-arkhe-green/30 text-arkhe-green' : step === 2 ? 'bg-arkhe-cyan/10 border-arkhe-cyan/30 text-arkhe-cyan' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 3 ? (
                    <CheckCircle2 className="w-5 h-5" />
                  ) : step === 2 ? (
                  ...
                  <div>
                    ...
                    <div className="text-xs opacity-80 font-mono">
                      arkhe-cli wallet create
                    </div>
                  </div>
                </div>

                <div
                  className={`flex items-center gap-3 p-3 rounded border ${step >= 4 ? 'bg-arkhe-green/10 border-arkhe-green/30 text-arkhe-green' : step === 3 ? 'bg-arkhe-cyan/10 border-arkhe-cyan/30 text-arkhe-cyan' : 'bg-[#1a1b1e] border-[#2a2b2e] text-arkhe-muted'}`}
                >
                  {step >= 4 ? (
                    <CheckCircle2 className="w-5 h-5" />
                  ) : step === 3 ? (
            ...
            <div className="bg-arkhe-cyan/5 border border-arkhe-cyan/20 rounded-lg p-4">
              <h3 className="font-mono text-[10px] uppercase tracking-widest text-arkhe-cyan mb-2">
                Android Node Bootstrap
              </h3>
              <p className="text-[10px] text-arkhe-muted mb-3 font-mono">
                Run Arkhe(n) on your mobile device via Termux.
              </p>
              <button
                onClick={async () => {
                  ...
                  await addLog(
                    'curl -O https://raw.githubusercontent.com/Arkhe-Network/Arkhe-PNT/main/scripts/arkhe-android-bootstrap.sh && bash arkhe-android-bootstrap.sh',
                    400,
                  );
                }}
          ...
          <div className="bg-black border border-[#1f2024] rounded-lg p-4 flex flex-col md:col-span-2 shadow-inner">
            <div className="flex items-center gap-2 mb-4 border-b border-[#1f2024] pb-2">
              ...
              <h3 className="font-mono text-xs uppercase tracking-widest text-arkhe-muted ml-2">
                arkhe-cli terminal
              </h3>
            </div>
            <div className="flex-1 font-mono text-sm text-gray-300 space-y-1 overflow-y-auto whitespace-pre-wrap">
              {logs.map((log, i) => (
                <div
                  key={i}
                  className={`${log.startsWith('$') ? 'text-arkhe-cyan font-bold mt-2' : ''} ${log.includes('[SUCCESS]') ? 'text-arkhe-green font-bold' : ''} ${log.includes('**IMPORTANT**') ? 'text-arkhe-red font-bold' : ''}`}
                >
                  {log}
                </div>
              ))}
