...
export default function P2PNetworkPanel({onClose}: {onClose: () => void}) {
  ...
  const connectNodes = async () => {
    if (connectedNodes.length > 0 || connectingTo) {
      return;
    }
    ...
    for (const node of NODES) {
      ...
      setLogs(prev => [
        ...prev,
        `> DIALING ${node.name.toUpperCase()} PEERS ON ${node.protocol}...`,
      ]);
      ...
      try {
        ...
        if (data.success) {
          ...
          setLogs(prev => [
            ...prev,
            `> [ERROR] FAILED TO CONNECT TO ${node.name.toUpperCase()}`,
          ]);
        }
      } catch (_error) {
        setLogs(prev => [
          ...prev,
          `> [ERROR] CONNECTION TIMEOUT FOR ${node.name.toUpperCase()}`,
        ]);
      }
    }
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-black/40 border border-[#1f2024] rounded-lg p-6 relative min-h-[400px] flex items-center justify-center overflow-hidden">
              ...
              <div className="absolute z-10 flex flex-col items-center">
                <div
                  className={`w-20 h-20 rounded-full border-2 flex items-center justify-center bg-black ${connectedNodes.length === NODES.length ? 'border-arkhe-cyan shadow-[0_0_30px_rgba(0,255,170,0.5)]' : 'border-[#2a2b2e]'}`}
                >
                  <Zap
                    className={`w-10 h-10 ${connectedNodes.length === NODES.length ? 'text-arkhe-cyan animate-pulse' : 'text-arkhe-muted'}`}
                  />
                </div>
                <div className="mt-2 font-mono text-xs font-bold text-arkhe-cyan uppercase tracking-widest">
                  Arkhe Core
                </div>
              </div>
              ...
              {NODES.map((node, index) => {
                ...
                return (
                  <React.Fragment key={node.id}>
                    ...
                    >
                      <div
                        className={`w-14 h-14 rounded-full border flex items-center justify-center ${isConnected ? `${node.bg} ${node.border} ${node.color} shadow-[0_0_15px_currentColor]` : isConnecting ? 'bg-[#1a1b1e] border-arkhe-muted text-arkhe-muted animate-pulse' : 'bg-[#111214] border-[#2a2b2e] text-[#2a2b2e]'}`}
                      >
                        <Server className="w-6 h-6" />
                      </div>
                      <div
                        className={`mt-2 font-mono text-[10px] font-bold uppercase tracking-wider ${isConnected ? node.color : 'text-arkhe-muted'}`}
                      >
                        {node.name}
                      </div>
                      <div className="font-mono text-[8px] text-arkhe-muted">
                        {node.protocol}
                      </div>
                    </div>
                  </React.Fragment>
            ...
            >
              ...
              {connectedNodes.length === NODES.length
                ? 'OMNICAIN TOPOLOGY ACTIVE'
                : connectingTo
            ...
            <div className="flex-1 font-mono text-xs text-gray-300 space-y-2 overflow-y-auto whitespace-pre-wrap">
              {logs.map((log, i) => {
                ...
                if (log.includes('[SUCCESS]')) {
                  colorClass = 'text-arkhe-green font-bold';
                }
                if (log.includes('DIALING')) {
                ...
                return (
                  <div
                    key={i}
                    className={`animate-fade-in ${colorClass}`}
                  >
                    {log}
                  </div>
                );
