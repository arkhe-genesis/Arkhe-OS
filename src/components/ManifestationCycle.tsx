...
export default function ManifestationCycle({
  manifestation,
}: ManifestationCycleProps) {
  const stages = [
          ...
          {stages.map((stage, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Icon
                    className={`w-4 h-4 ${isActive ? 'text-arkhe-cyan' : isPast ? 'text-arkhe-green' : 'text-arkhe-muted'}`}
                  />
                </motion.div>
                <div className="text-center w-full">
                  <div
                    className={`text-[9px] font-mono font-bold leading-tight mb-1 ${isActive ? 'text-arkhe-cyan' : isPast ? 'text-arkhe-green' : 'text-arkhe-muted'}`}
                  >
                    {stage.label}
                  </div>
