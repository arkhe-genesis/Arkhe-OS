...
export default function QuantumMemoryPanel({
  state,
  onClose,
}: QuantumMemoryPanelProps) {
  if (!state) {
    return null;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 p-4 rounded-lg space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-lg font-mono text-emerald-400 font-bold">
                {state.coherenceTimeSeconds.toFixed(2)} s
              </span>
            </div>
          ...
          <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2">
              {Array.from({length: 16}).map((_, i) => (
                <div
                  key={i}
                  className={`h-8 rounded border flex items-center justify-center text-[10px] font-mono ${i < state.activeRegisters ? 'bg-amber-500/20 border-amber-500 text-amber-400 animate-pulse' : 'bg-black border-white/5 text-arkhe-muted'}`}
                >
          ...
          <button
            onClick={() =>
              fetch('/api/quantum/memory/store', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({material: 'MoS2'}),
              })
            }
