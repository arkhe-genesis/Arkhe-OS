...
export default function ExoticMaterialPanel({
  state,
  onClose,
}: ExoticMaterialPanelProps) {
  if (!state) {
    return null;
  }
      ...
      >
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {state.scaffolds.map(s => (
              <div
                key={s.name}
                className="bg-white/5 border border-white/10 p-4 rounded-lg space-y-3 group hover:border-amber-500/30 transition-all"
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-[8px] font-mono text-amber-500/70 font-bold uppercase">
                      {s.type}
                    </div>
                  </div>
            ...
            <button
              onClick={() =>
                fetch('/api/whisper/library/exotic', {
                  method: 'POST',
        ...
        <div className="p-4 border-t border-white/5 bg-black/40 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-arkhe-muted uppercase tracking-widest">
              Universal Resonance Database Active
            </span>
          </div>
