...
use crate::recorder::success_recorder_hybrid::SuccessRecorder as HybridRecorder;
...
pub struct EmbodiedCognitiveCore {
    ...
    // === In-memory recommendation tracking ===
    recommendation_outcomes: Vec<(crate::picoads::PicoAdsRecommendation, bool)>,
}
...
impl EmbodiedCognitiveCore {
    pub fn new(
        ...
        _recorder_path: Option<&str>, // Ignorado — o HybridRecorder decide sozinho
    ) -> Self {
        // Cria o recorder híbrido (tenta SQLite primeiro, cai para JSON automaticamente)
        let success_recorder = Some(HybridRecorder::new());
    ...
    fn collect_hub_performance(&self) -> HashMap<String, HubPerformance> {
        ...
        if let Some(recorder) = &self.success_recorder {
            let stats = recorder.recent_hub_stats(20);
    ...
    /// Process recommendations to be tracked
