...
impl SuccessRecorder {
    ...
    pub fn recent_hub_stats(&self, last_rounds: u32) -> Vec<(String, f32, u32, f32)> {
        match &self.backend {
            RecorderBackend::Sqlite(r) => r.recent_hub_stats(last_rounds).unwrap_or_default(),
            RecorderBackend::Json(_) => Vec::new(), // JSON não tem essa consulta
        }
    }
