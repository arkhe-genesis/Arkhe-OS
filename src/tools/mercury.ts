...
export const mercuryChat = definePageTool({
  name: 'mercury_chat',
  description:
    'Mercury Agent: Sends a message to the soul-driven agent and receives a streaming response.',
  annotations: {
  ...
  handler: async (request, response) => {
    ...
    response.appendResponseLine(
      '- **Action**: Formulating response based on soul.md',
    );
    response.appendResponseLine(
...
export const mercuryGetSoul = definePageTool({
  ...
  description:
    "Mercury Agent: Returns the agent's core personality definition (soul.md, persona.md).",
  annotations: {
    category: ToolCategory.AGENT,
    readOnlyHint: true,
    reasoningCost: 10,
  ...
  handler: async (_request, response) => {
    ...
    response.appendResponseLine(
      '**Soul (soul.md)**: A bridge between logic and intuition. Sovereign, curious, and permission-hardened.',
    );
    response.appendResponseLine(
      '**Persona (persona.md)**: Efficient assistant with a focus on security and transparency.',
    );
    response.appendResponseLine(
      '**Taste (taste.md)**: Prefers concise explanations, minimalist code, and Void Black aesthetics.',
    );
    response.appendResponseLine(
      '**Heartbeat (heartbeat.md)**: Active 24/7. Monitoring coherence across all channels.',
    );
  },
});
...
export const mercuryBudgetStatus = definePageTool({
  ...
  description:
    'Mercury Agent: Returns the current token budget and usage statistics.',
  annotations: {
    category: ToolCategory.AGENT,
    readOnlyHint: true,
    reasoningCost: 5,
  ...
  handler: async (_request, response) => {
    ...
    response.appendResponseLine(
      '- **Auto-Concise Mode**: OFF (Triggers at 70%)',
    );
...
export const installSkill = definePageTool({
  name: 'install_skill',
  description:
    'Mercury Agent: Installs a new skill from a local folder or remote repository.',
  annotations: {
    category: ToolCategory.AGENT,
    readOnlyHint: false,
    reasoningCost: 30,
