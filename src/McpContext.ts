...
export class McpContext implements Context {
  ...
  resolveCdpElementId(
    ...
    if (snapshot.backendNodeIdToId) {
      return snapshot.backendNodeIdToId.get(cdpBackendNodeId);
    }
  ...
  async createTextSnapshot(
    ...
    const assignIds = (node: SerializedAXNode): TextSnapshotNode => {
      ...
      if (nodeWithId.backendNodeId !== undefined) {
  ...
  isCoherenceBubbleActive(): boolean {
    return this.#coherenceBubbleActive;
  }

  setCoherenceBubbleActive(active: boolean): void {
    this.#coherenceBubbleActive = active;
  }
