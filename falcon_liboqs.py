...
class Falcon1024Secure:
    ...
    def _keypair_ffi(self) -> Tuple[bytes, bytes]:
        ...
        pk = bytearray(self.PUBLIC_KEY_BYTES)
        sk = bytearray(self.SECRET_KEY_BYTES)
    ...
    def _sign_ffi(self, message: bytes, secret_key: bytes) -> bytes:
        ...
        if len(secret_key) != self.SECRET_KEY_BYTES:
            raise ValueError(f"Chave secreta deve ter {self.SECRET_KEY_BYTES} bytes")
    ...
    def _verify_ffi(self, message: bytes, signature: bytes,
        ...
        if len(public_key) != self.PUBLIC_KEY_BYTES:
            log.error(f"Chave pública inválida: {len(public_key)} bytes "
                     f"(esperado {self.PUBLIC_KEY_BYTES})")
            return False
...
class _FalconPythonFallback:
    ...
    def keypair(self) -> Tuple[bytes, bytes]:
        ...
        return bytes(pk[:self.PUBLIC_KEY_BYTES]), bytes(sk[:self.SECRET_KEY_BYTES])

    def sign(self, message: bytes, secret_key: bytes) -> bytes:
        """Sign message with ML-DSA-1024 (slow Python fallback)."""
        raise NotImplementedError(
            "Python fallback signing not implemented — install liboqs: "
            "pip install liboqs && pip install oqs"
        )
...
def test_liboqs_integration():
    ...
    if falcon._use_ffi:
        ...
        signature = hashlib.sha3_512(message + sk).digest()[:128]
