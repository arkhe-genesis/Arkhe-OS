...
def validate_filename(filename: str) -> Tuple[bool, Optional[str]]:
    ...
    if not m:
        return False, "Nomenclatura inválida: '{0}' não corresponde a ATSEFCCNNNAAAAMMDDSSSSSS.FCC".format(base)
    nnn, ano, mes, dia, seq = m.groups()
    if not validate_date(ano + mes + dia):
        return False, "Data no nome do arquivo inválida: {0}{1}{2}".format(ano, mes, dia)
    return True, None
...
class FCCParser:
        ...
        for idx, line in enumerate(self.lines, start=1):
            if len(line) != LINE_LENGTH:
                self.result.errors.append(
                    "Linha {0}: tamanho {1} caracteres, esperado {2}".format(idx, len(line), LINE_LENGTH)
                )
                continue
            ...
            if tipo == REGISTRO_HEADER:
                ...
                self.result.errors.append("Linha {0}: tipo de registro desconhecido '{1}'".format(idx, tipo))
    ...
    def _parse_header(self, line: str, idx: int):
        ...
        if versao != LAYOUT_VERSION:
            self.result.warnings.append("Linha {0}: versão layout '{1}' != '{2}'".format(idx, versao, LAYOUT_VERSION))
        if nome_layout != LAYOUT_NAME:
            self.result.warnings.append("Linha {0}: nome layout '{1}' != '{2}'".format(idx, nome_layout, LAYOUT_NAME))
        if not validate_cnpj(cnpj):
            self.result.errors.append("Linha {0} (HEADER): CNPJ inválido '{1}'".format(idx, cnpj))
    ...
    def _parse_detalhe1(self, line: str, idx: int):
        ...
        if not validate_cnpj(cnpj):
            self.result.errors.append("Linha {0} (DETALHE 1): CNPJ prestador inválido '{1}'".format(idx, cnpj))
        if not validate_date(data_credito):
            self.result.errors.append("Linha {0} (DETALHE 1): data crédito inválida '{1}'".format(idx, data_credito))
        if especie not in ESPECIES_RECURSO:
            self.result.warnings.append("Linha {0} (DETALHE 1): espécie recurso desconhecida '{1}'".format(idx, especie))
        if valor_total != (valor_credito + valor_taxa):
            self.result.errors.append(
                "Linha {0} (DETALHE 1): valor_total({1}) != credito({2}) + taxa({3})".format(idx, valor_total, valor_credito, valor_taxa)
            )
    ...
    def _parse_detalhe2(self, line: str, idx: int):
        ...
        if not validate_cpf(cpf):
            self.result.errors.append("Linha {0} (DETALHE 2): CPF inválido '{1}'".format(idx, cpf))
        if not validate_date(data_doacao):
            self.result.errors.append("Linha {0} (DETALHE 2): data doação inválida '{1}'".format(idx, data_doacao))
        if meio not in MEIOS_PAGAMENTO:
            self.result.warnings.append("Linha {0} (DETALHE 2): meio pagamento desconhecido '{1}'".format(idx, meio))
        ...
        if self.current_d1 is None:
            self.result.errors.append("Linha {0} (DETALHE 2): orfão — sem DETALHE 1 precedente".format(idx))
        else:
    ...
    def _cross_validate(self):
        ...
        if self.result.trailer:
            if self.result.trailer.total_doacoes != self.d1_count:
                self.result.errors.append(
                    "TRAILER: total_doacoes({0}) != "
                    "quantidade DETALHE 1({1})".format(self.result.trailer.total_doacoes, self.d1_count)
                )
        else:
        ...
        for i, d1 in enumerate(self.result.detalhes1, start=1):
            if len(d1.detalhes2) != d1.total_individuais:
                self.result.errors.append(
                    "DETALHE 1 #{0}: total_individuais({1}) != "
                    "doadores encontrados({2})".format(i, d1.total_individuais, len(d1.detalhes2))
                )
...
def format_report(result: FCCFile) -> str:
    lines = []
    lines.append("=" * 70)
    lines.append("RELATÓRIO DE VALIDAÇÃO FCC — {0}".format(result.filename))
    lines.append("SHA3-256: {0}".format(result.sha3_256))
    lines.append("=" * 70)

    if result.header:
        lines.append("\n📋 HEADER:")
        lines.append("   Software: {0} v{1}".format(result.header.nome_fantasia, result.header.versao_layout))
        lines.append("   EF Type: {0}".format(result.header.nome_layout))
    ...
    lines.append("\n📊 Resumo:")
    lines.append("   HEADER:        {0}".format('OK' if result.header else 'AUSENTE'))
    lines.append("   DETALHE 1:     {0} doações".format(len(result.detalhes1)))
    lines.append("   DETALHE 2:     {0} doadores".format(sum(len(d1.detalhes2) for d1 in result.detalhes1)))
    lines.append("   TRAILER:       {0}".format('OK' if result.trailer else 'AUSENTE'))
    lines.append("   Valor total:   R$ {0:.2f}".format(sum(d1.valor_total for d1 in result.detalhes1)/100))
