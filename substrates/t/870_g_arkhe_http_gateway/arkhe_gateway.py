...
class CanonicalRegistry:
    ...
    def _compute_tx_hash(self, seal: str, nonce: int) -> str:
        return "0x" + hashlib.sha3_256(("{}:{}:{}").format(seal, nonce, time.time()).encode()).hexdigest()
    ...
    def publish(self, substrate: str, action: str, sequence: Optional[str],
        ...
        if phi_c < GHOST_THRESHOLD:
            raise HTTPException(status_code=503, detail=("Gateway abaixo do Ghost Threshold (Φ_C={:.4f} < γ={})").format(phi_c, GHOST_THRESHOLD))
        ...
        return PublishReceipt(
            ...
            verification_url=("/verify/{}").format(seal)
        )
    ...
    return HealthResponse(
        ...
        substrates={
            k: {
                ...
                "phi_c": v["phi_c"],
                "status": v["status"],
                "seal": hashlib.sha3_256(("{}-{}").format(k, v['name']).encode()).hexdigest(),
...
if __name__ == "__main__":
    print(("""
╔══════════════════════════════════════════════════════════════════════════════╗
║           ARKHE HTTP GATEWAY — Substrato {0}                          ║
║           Bridge Real: /publish + /verify                                     ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Arquiteto: {1:<20} ORCID: {2:<26} ║
║  Keeper: {3:<22} Royalties: 2% → ORCID                           ║
║  Ghost Threshold: γ = {4:<46.9f} ║
╚══════════════════════════════════════════════════════════════════════════════╝
    """).format(GATEWAY_ID, ARCHITECT, ORCID, KEEPER, GHOST_THRESHOLD))
