...
class K8sCoherenceSimulator:
    ...
    def __init__(self, n_pods: int = 100, base_latency_ms: float = 50.0):
        self.pods: List[PodState] = [
            PodState(name="pod-{0:03d}".format(i), latency_ms=base_latency_ms)
    ...
    def report(self) -> str:
        ...
        lines = [
            ...
            "╚════════════════════════════════════════════════════════════╝",
            "",
            "{0:>4} | {1:>6} | {2:>6} | {3:>8} | {4:>6} | {5:>7} | Status".format("Step", "Load", "Fail%", "r", "CPU%", "Healthy"),
            "-" * 70,
        ]
        for rec in self.history:
            status = "💥 COLAPSO" if rec["collapsed"] else "✓ COERENTE"
            lines.append(
                "{0:4d} | {1:6d} | {2:6.1%} | {3:8.4f} | {4:6.1f} | {5:7d} | {6}".format(
                    rec['step'], rec['load'], rec['failure_rate'],
                    rec['r'], rec['cpu_avg'], rec['healthy_pods'], status
                )
            )
        lines.append("")
        lines.append("Ghost Threshold (γ): {0:.6f}".format(self.GHOST_THRESHOLD))
        if any(r["collapsed"] for r in self.history):
            first = next(r for r in self.history if r["collapsed"])
            lines.append("Colapso detectado no step {0} (load={1}, fail={2:.1%})".format(first['step'], first['load'], first['failure_rate']))
        else:
...
def main():
    ...
    if any(r["collapsed"] for r in experiment):
        ...
        print("\n[VALIDAÇÃO] Threshold γ={0:.4f} cruzado em load={1}".format(sim.GHOST_THRESHOLD, first_colapse['load']))
        print("[VALIDAÇÃO] Healthy pods no colapso: {0}/{1}".format(first_colapse['healthy_pods'], first_colapse['total_pods']))
