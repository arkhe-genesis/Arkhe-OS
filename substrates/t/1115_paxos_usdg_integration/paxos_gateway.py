...
class PaxosAPIClient:
    ...
    def _generate_signature(self, method: str, path: str, body: str = "", timestamp: int = None) -> str:
        ...
        # Fix f-string format per rule
        message = "{0}\n{1}\n{2}\n{3}".format(method, path, nonce, body)
        signature = hmac.new(
    ...
    async def _request(self, method: str, endpoint: str, data: Dict = None) -> Dict:
        ...
        async with session.request(method, url, headers=headers, data=body) as resp:
            if resp.status >= 400:
                ...
                raise RuntimeError("Paxos API error {0}: {1}".format(resp.status, text))
    ...
    async def get_transaction(self, tx_id: str) -> PaxosTransaction:
        data = await self._request("GET", "orchestrations/{0}".format(tx_id))
        return PaxosTransaction(
            id=data["id"],
            type=data["type"],
            status=data["status"],
...
class OnChainUSDGClient:
    ...
    def __init__(self, credentials: PaxosCredentials):
        ...
        if self.w3.is_connected() is False:
            raise ConnectionError("Web3 nao conseguiu conectar a rede")
    ...
    async def increase_allowance(self, spender: str, added_value: Decimal, gas_limit: int = None) -> str:
        """Aumenta allowance usando `increaseAllowance` (seguranca contra race condition)."""
        if not self.account:
            raise ValueError("Chave privada nao configurada para transacoes on-chain")
        spender = Web3.to_checksum_address(spender)
    ...
    async def transfer(self, to_address: str, amount: Decimal, gas_limit: int = None) -> str:
        """Transfere USDG para outro endereco."""
        if not self.account:
            raise ValueError("Chave privada nao configurada para transacoes on-chain")
...
class PaxosUSDGGateway:
    """Fachada unificada para operacoes custodiais (API Paxos) e on-chain."""
    ...
    def set_alert_callback(self, callback: Callable):
        """Registra funcao para receber alertas (ex: enviar notificacao)."""
    ...
    async def _send_alert(self, title: str, message: str, severity: str = "warning"):
        for cb in self._alert_callbacks:
            try:
                ...
                logger.error("Erro no callback de alerta: {0}".format(e))
    ...
    async def mint(self, amount: Decimal, currency: str = "USD") -> PaxosTransaction:
        ...
        try:
            ...
            if amount > 100000:
                await self._send_alert("Mint Grande", "Montante: {0} {1}".format(amount, currency), "info")
            return tx
        except Exception as e:
    ...
    async def transfer_on_chain(self, to_address: str, amount: Decimal) -> str:
        ...
        await self._send_alert("Transferencia On-chain", "Para {0}, valor {1} USDG".format(to_address, amount), "info")
        return tx_hash
    ...
    async def get_onchain_balance(self, address: str = None) -> Decimal:
        if address is None:
            if not self.onchain.from_address:
                raise ValueError("Endereco nao fornecido e nenhum endereco configurado")
    ...
    async def start_monitoring(self, interval_seconds: int = 30):
        """Inicia loop de monitoramento continuo de saldos e allowances."""
        self._stop_monitoring = False
            ...
            try:
                ...
                for bal in balances:
                    if bal.currency == "USDG" and bal.available < 1000:
                        await self._send_alert("Saldo USDG Baixo", "Saldo disponivel: {0}".format(bal.available), "warning")

                # Verifica saldo on-chain
                if self.onchain.from_address:
                    onchain_bal = await self.get_onchain_balance()
                    if onchain_bal < 500:
                        await self._send_alert("Saldo On-chain Baixo", "Saldo: {0} USDG".format(onchain_bal), "warning")

                # Monitora allowances para spender critico (ex: contrato de swap)
                # (exemplo: verificar allowance para 0x...)
                # ...
            except Exception as e:
                logger.error("Erro no monitoramento: {0}".format(e))
