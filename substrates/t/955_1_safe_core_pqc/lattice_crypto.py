...
def _cbd(seed: bytes, eta: int, n: int = 256) -> List[int]:
    ...
    """
    coeffs = []
    bits = ''.join(f'{byte:08b}' for byte in seed)  # noqa: FS002
    for i in range(n):
        start = i * 2 * eta
...
class Kyber768:
    ...
    def encapsulate(self, pk: bytes) -> Tuple[bytes, bytes]:
        ...
        for i in range(self.k):
            ...
            for j in range(self.k):
                # A^T: swap indices
                prod = self.ntt.ntt_mul(A[j][i], r_ntt[j])
                poly = self.ntt.ntt_add(poly, prod)
    ...
    def decapsulate(self, sk: bytes, ct: bytes) -> bytes:
        ...
        for i in range(self.k):
            ...
            prod = self.ntt.ntt_mul(s_ntt[i], u_ntt)
...
class Dilithium3:
    ...
    def sign(self, sk: bytes, msg: bytes) -> bytes:
        ...
        while True:
            ...
            z = []
            for i in range(self.l):
                cs1 = self.ntt.intt(self.ntt.ntt_mul(c_ntt, self.ntt.ntt(s1[i])))
                z.append(_poly_add(y[i], cs1, self.q))

            # Compute r0 = low_bits(w - c*s2)
            cs2 = []
            for i in range(self.k):
                cs2_poly = self.ntt.intt(self.ntt.ntt_mul(c_ntt, self.ntt.ntt(s2[i])))
                cs2.append(cs2_poly)
            ...
            if z_norm < self.gamma1 - self.beta and r0_norm < self.gamma2 - self.beta:
                ...
                ct0 = []
                for i in range(self.k):
                    ct0_poly = self.ntt.intt(self.ntt.ntt_mul(c_ntt, self.ntt.ntt(t0[i])))
                    ct0.append(ct0_poly)
    ...
    def verify(self, pk: bytes, msg: bytes, sig: bytes) -> bool:
        ...
        ct1 = []
        for i in range(self.k):
            ct1_poly = self.ntt.intt(self.ntt.ntt_mul(c_ntt, self.ntt.ntt(t1_shifted[i])))
            ct1.append(ct1_poly)
...
if __name__ == "__main__":
    ...
    print(f"  ✓ Kyber KEM OK (ss = {ss_enc.hex()[:16]}...)")  # noqa: FS002
    ...
    print(f"  ✓ Dilithium DSA OK (sig len = {len(sig)} bytes)")  # noqa: FS002
