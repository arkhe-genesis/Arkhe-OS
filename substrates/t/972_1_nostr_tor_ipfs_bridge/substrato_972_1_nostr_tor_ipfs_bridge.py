...
def get_b64_artifacts():
    return {
        "substrate.toml": base64.b64encode(open("substrates/t/972_1_nostr_tor_ipfs_bridge/substrate.toml", "rb").read()).decode('utf-8'),
        "bridge_nostr_tor_ipfs.py": base64.b64encode(open("substrates/t/972_1_nostr_tor_ipfs_bridge/bridge_nostr_tor_ipfs.py", "rb").read()).decode('utf-8')
    }
...
def main():
    ...
    fd, path = tempfile.mkstemp(suffix=".json", prefix="substrate_972_1_")
    with os.fdopen(fd, "w") as f:
        json.dump(payload, f, indent=2)

    print("Substrate 972.1 canonized at:", path)
    print("Seal:", seal)

    if len(sys.argv) > 1 and sys.argv[1] == "--extract":
        extract_dir = sys.argv[2] if len(sys.argv) > 2 else "output_972_1"
