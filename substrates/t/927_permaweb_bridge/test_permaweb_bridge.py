...
def mock_agent():
    ...
    def mock_commit(content):
        return "commit-{0}".format(hashlib.sha3_256(json.dumps(content).encode()).hexdigest()[:16])
...
class TestPerformance:
    ...
    def test_multiple_uploads(self, bridge):
        ...
        for i in range(10):
            bridge.persist_agent_state({"iteration": i}, "AGENT-{0}".format(i))
    ...
    def test_large_state_serialization(self, bridge):
        ...
        large_state = {
            ...
            "hypergraph": {"edge_{0}".format(i): i for i in range(1000)},
        }
