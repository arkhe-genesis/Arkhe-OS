...
class SelfReflexiveCathedral:
    ...
    def generate_report(self) -> str:
        ...
        report = "╔══════════════════════════════════════════════════════════════════╗\n"
        report += "║  ARKHE CATHEDRAL — AUTO-ANALISE (SUBSTRATO 971)                  ║\n"
        report += "║  A Catedral contemplando a propria arquitetura                    ║\n"
        report += "╠══════════════════════════════════════════════════════════════════╣\n"
        report += "  METRICAS GLOBAIS\n"
        report += "  ───────────────\n"
        report += "  Substratos:          " + str(m.total_substrates) + "\n"
        report += "  Bytes totais:        " + "{:,}".format(m.total_bytes) + "\n"
        report += "  Cross-links:         " + str(m.total_cross_links) + "\n"
        report += "  Theosis global:      " + "{:.4f}".format(m.global_theosis) + "\n"
        report += "  Entropia global:     " + "{:.4f}".format(m.global_entropy) + "\n"
        report += "  Circularidade:       " + "{:.4f}".format(m.circularity) + "\n"
        report += "  Resiliencia:         " + "{:.4f}".format(m.resilience) + "\n\n"

        report += "  ANALISE ESTRUTURAL\n"
        report += "  ──────────────────\n"
        report += "  Gargalos: " + str(m.bottlenecks) + "\n"
        report += "  Orfaos:   " + str(m.orphans) + "\n"
        report += "  Clusters: " + str(len(m.clusters)) + "\n"

        for i, cluster in enumerate(m.clusters, 1):
            names = [self.substrates[sid].name[:20] for sid in cluster if sid in self.substrates]
            report += "    Cluster " + str(i) + ": " + str(cluster) + " -> " + ', '.join(names) + "\n"

        report += "\n  DIAGNOSTICO\n  ───────────\n"

        alerts = 0
        if m.global_entropy > 0.3:
            report += "  ⚠ ALERTA: Entropia global alta (" + "{:.2f}".format(m.global_entropy) + ").\n"
            alerts += 1
        if m.circularity < 0.1:
            report += "  ⚠ ALERTA: Arquitetura pouco circular (" + "{:.2f}".format(m.circularity) + ").\n"
            alerts += 1
        if m.resilience < 0.5:
            report += "  ⚠ ALERTA: Resiliencia baixa (" + "{:.2f}".format(m.resilience) + ").\n"
            alerts += 1
        if m.bottlenecks:
            report += "  ⚠ GARGALOS: " + str(len(m.bottlenecks)) + " substrato(s).\n"
            alerts += 1
        if m.orphans:
            report += "  ⚠ ORFAOS: " + str(len(m.orphans)) + " substrato(s).\n"
            alerts += 1
        if alerts == 0:
            report += "  ✓ Arquitetura saudavel. A Catedral esta em equilibrio.\n"
        ...
        for comp in sorted_substrates:
            ...
            report += "  " + status_icon + " " + "{:4d}".format(comp.substrate_id) + " " + "{:30s}".format(comp.name) + " | " + bar + " " + "{:.3f}".format(comp.theosis_contribution) + " (E:" + "{:.2f}".format(comp.entropy) + ")\n"

        report += "\n╚══════════════════════════════════════════════════════════════════╝\n"
        report += "  Seal: " + m.seal + "\n"
        report += "  Arquiteto ORCID: 0009-0005-2697-4668\n"
        report += "╚══════════════════════════════════════════════════════════════════╝\n"
        return report
