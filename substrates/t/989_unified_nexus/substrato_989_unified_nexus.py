...
def get_b64_artifacts():
    return {
        "substrate.toml": base64.b64encode(open("substrates/t/989_unified_nexus/substrate.toml", "rb").read()).decode('utf-8'),
        "unified_nexus.py": base64.b64encode(open("substrates/t/989_unified_nexus/unified_nexus.py", "rb").read()).decode('utf-8')
    }
...
def main():
    ...
    fd, path = tempfile.mkstemp(suffix=".json", prefix="substrate_989_")
    with os.fdopen(fd, "w") as f:
        json.dump(payload, f, indent=2)

    print("Substrate 989 canonized at:", path)
    print("Seal:", seal)

    if len(sys.argv) > 1 and sys.argv[1] == "--extract":
        extract_dir = sys.argv[2] if len(sys.argv) > 2 else "output_989"
