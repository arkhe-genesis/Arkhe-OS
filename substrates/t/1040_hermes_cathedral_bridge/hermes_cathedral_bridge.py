...
class HermesCathedralBridge:
    ...
    def sync_to_cathedral(self) -> Dict:
        ...
        sync_payload = {
            ...
            "theosis_summary": {
                "skills": float(np.mean([s.theosis for s in self.skills.values()])) if self.skills else 0.0,
                "memories": float(np.mean([m.theosis for m in self.memories])) if self.memories else 0.0,
                "gateways": float(np.mean([g.theosis for g in self.gateways.values()])) if self.gateways else 0.0,
                "crons": float(np.mean([c.theosis for c in self.crons.values()])) if self.crons else 0.0,
                "subagents": float(np.mean([a.theosis for a in self.subagents.values()])) if self.subagents else 0.0
            }
        }
