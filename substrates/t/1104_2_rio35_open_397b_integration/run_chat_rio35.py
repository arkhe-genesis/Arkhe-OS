...
class Rio35InferenceEngine:
    ...
    def _load_model(self):
        ...
        print("[Rio35Engine] Loading {0}...".format(self.MODEL_ID))
        ...
        print("[Rio35Engine] Model loaded on {0}".format(self.model.device))
    ...
    def _should_switch(self,
        ...
        if len(entropy_history) >= 2:
            ...
            if trend > 0.15 and entropy_current > ref:
                return True, "entropy_rising({0:.3f})".format(trend)

        # Absolute threshold
        if entropy_current > ref * 1.3:
            return True, "entropy_high({0:.3f}>{1:.3f})".format(entropy_current, ref*1.3)
        ...
        if len(entropy_history) >= 3:
            if all(entropy_history[i] > entropy_history[i+1]
                ...
                if entropy_current < ref * 0.8:
                    return True, "confidence_recovered({0:.3f})".format(entropy_current)
    ...
    def generate_with_swir(self,
        ...
        run_id = hashlib.sha256(
            "{0}{1}".format(prompt, time.time()).encode()
        ).hexdigest()[:16]
            ...
            for step_idx, logits in enumerate(outputs.scores):
                ...
                if step_idx > 0 and step_idx % self.config.block_size == 0:
                    ...
                    if should_switch and switch_count < self.config.max_switches:
                        ...
                        print("  [SWIR] Switch #{0} at t={1}: {2} -> {3}".format(switch_count, step_idx, reason, current_mode))
...
class SwiReasoningCalibrator:
    ...
    def calibrate(self,
        ...
        print("[Calibrator] Starting grid search: {0} refs × {1} switches".format(len(entropy_refs), len(max_switches_opts)))
        print("[Calibrator] Samples: {0} prompts".format(len(samples)))

        for ref in entropy_refs:
            for max_sw in max_switches_opts:
                print("\n[Calibrator] Testing ref={0}, max_switches={1}".format(ref, max_sw))
                ...
                for i, prompt in enumerate(samples):
                    ...
                    print("  Sample {0}/{1}: {2} switches, entropy_mean={3:.3f}, latency={4:.1f}ms".format(
                          i+1, len(samples), metrics.switch_count, metrics.entropy_mean, metrics.latency_total_us/1000))
...
def main():
    ...
    if args.calibrate:
        ...
        out_file = "calibration_{0}.json".format(datetime.now().strftime('%Y%m%d_%H%M%S'))
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump(calibration, f, indent=2, ensure_ascii=False)

        print("\n[Calibrator] Results saved to {0}".format(out_file))
        print("[Calibrator] BEST CONFIG: entropy_ref_x1000={0}, max_switches={1}".format(
              calibration['best_config']['entropy_ref_x1000'], calibration['best_config']['max_switches']))
        return
    ...
    print("Engine: {0} | Method: {1}".format(args.engine, args.method))
    print("Prompt hash: {0}".format(hashlib.sha256(prompt.encode()).hexdigest()[:16]))
    print("=" * 60)
    ...
    print("\n[Metrics appended to {0}]".format(args.output_metrics))
