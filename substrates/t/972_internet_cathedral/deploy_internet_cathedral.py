...
class InternetCathedralDeployer:
    ...
    async def inoculate(self, node: Dict[str, Any]) -> str:
        ...
        if node["arch"] == "wasm":
            print("  🧬 Inoculando WASM no navegador " + node['ip'] + ".")
        else:
            print("  🐳 Inoculando container OCI em " + node['ip'] + " (" + node['arch'] + ").")
