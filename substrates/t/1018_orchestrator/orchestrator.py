...
class CathedralOrchestrator:
    ...
    def initialize(self, node_id: str, region: str, orcid: str):
        ...
        print(f"[ORCHESTRATOR] Inicializando substratos para {node_id}...")  # noqa: FS002
        ...
        print(f"[ORCHESTRATOR] Todos os substratos inicializados.")  # noqa: FS002
        print(f"[ORCHESTRATOR] Seal: {self.config.seal}")  # noqa: FS002
...
class CathedralTelemetry:
    ...
    def generate_report(self) -> str:
        ...
        report.append("=" * 60)
        report.append(f"Total events: {self.event_count}")  # noqa: FS002
        report.append(f"Buffer size: {len(self.events)}")  # noqa: FS002
        report.append("")
        report.append("Substrate Metrics:")
        for substrate, metrics in self.substrate_metrics.items():
            report.append(f"  {substrate}: {metrics}")  # noqa: FS002
        report.append("")
        report.append("Recent Events (last 10):")
        for event in list(self.events)[-10:]:
            report.append(f"  [{event['timestamp']:.3f}] {event['type']}: {event['data']}")  # noqa: FS002
        report.append("=" * 60)
        return "\n".join(report)
...
def run_integration_test():
    ...
    print(f"  ✓ Inicializado: {status['initialized']}")  # noqa: FS002
    print(f"  ✓ Substratos ativos: {list(status['substrates'].keys())}")  # noqa: FS002
    ...
    print(f"  ✓ Kyber encaps/decaps: OK")  # noqa: FS002
    print(f"  ✓ Dilithium sign/verify: OK")  # noqa: FS002
    print(f"  ✓ Shared secret: {ss_enc.hex()[:16]}...")  # noqa: FS002
    ...
    print(f"  ✓ Session ID: {result['session_id']}")  # noqa: FS002
    print(f"  ✓ Handshake PQ estabelecido")  # noqa: FS002
    ...
    print(f"  ✓ Stamp issued and verified")  # noqa: FS002
    print(f"  ✓ Passport humanity: {passport_result['is_human']} "  # noqa: FS002
          f"(confidence: {passport_result['confidence']:.2%})")  # noqa: FS002
    ...
    print(f"  ✓ Percepção multimodal: {list(cognitive_result['percepts'].keys())}")  # noqa: FS002
    print(f"  ✓ Global coherence: {cognitive_result['global_coherence']:.3f}")  # noqa: FS002
    print(f"  ✓ Bindu state (unified): {cognitive_result['bindu_state']}")  # noqa: FS002
    ...
    print(f"  ✓ {len(memories)} memórias consolidadas em {len(consolidated)} vetores")  # noqa: FS002

    # 7. Atenção Profunda
    print("\n[7] Teste Atenção Profunda (952)")
    attended, coherence = orch.cognitive_attend(consolidated)
    print(f"  ✓ Coerência média: {np.mean(coherence):.3f}")  # noqa: FS002
    print(f"  ✓ Estado Bindu: {orch.cognitive_pipeline.global_coherence > 0.7}")  # noqa: FS002
    ...
    print(f"  Sessions ativas: {final_status['active_sessions']}")  # noqa: FS002
    print(f"  Eventos registrados: {final_status['telemetry_events']}")  # noqa: FS002
    print(f"  Coerência global: {final_status['global_coherence']:.3f}")  # noqa: FS002

    print("\n" + "=" * 70)
    print(" TESTE DE INTEGRAÇÃO COMPLETO — PASS")
    print(f" SEAL: {config.seal}")  # noqa: FS002
    print("=" * 70)
