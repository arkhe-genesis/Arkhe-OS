...
def get_b64_artifacts():
    return {
        "p1_transparency_v1.0.0_policy.yaml": "",
        "p2_privacy_v1.3.0_policy.yaml": "",
        "p3_sovereignty_v1.0.0_policy.yaml": "",
        "p4_verifiability_v1.0.0_policy.yaml": "",
        "p5_continuity_v1.0.0_policy.yaml": "",
        "p6_security_v1.0.0_policy.yaml": "",
        "p7_alignment_v1.0.0_policy.yaml": "",
    }
...
def main():
    ...
    fd, path = tempfile.mkstemp(suffix=".json", prefix="substrate_996_")
    with os.fdopen(fd, "w") as f:
        json.dump(payload, f, indent=2)

    print("Substrate 996 canonized at:", path)
    print("Seal:", seal)

    if len(sys.argv) > 1 and sys.argv[1] == "--extract":
        extract_dir = sys.argv[2] if len(sys.argv) > 2 else "output_996"
