...
class OracleFeed:
    ...
    def aggregate(self):
        ...
        data = "{}:{}:{}:{}".format(self.feed_id, self.value, self.timestamp, self.round_id)
...
class ChainlinkOracleBridge:
    ...
    def send_ccip_message(self, source_chain: str, dest_chain: str,
        ...
        msg_id = "ccip-" + hashlib.sha3_256(json.dumps(payload).encode()).hexdigest()[:16]
    ...
    def generate_seal(self, data: dict) -> str:
        ...
        return "976-CHAINLINK-" + hashlib.sha3_256(json_str.encode()).hexdigest()[:16].upper()
...
╠══════════════════════════════════════════════════════════════════╣
  NÓS CHAINLINK: {} (saudáveis: {})
  FEEDS ATIVOS: {}
  MENSAGENS CCIP: {}
  LINK CONSUMIDO: {:.4f} LINK
  PREÇO LINK/USD: ${:.2f}
...
""".format(
            ...
            healthy_nodes,
            len(self.feeds),
            self.total_ccip_messages,
            self.total_link_consumed,
            self.link_price_usd
        )
        for feed_id, feed in self.feeds.items():
            status = "✓ VALIDADO" if feed.validated else "○ PENDENTE"
            pair = "({})".format(feed.asset_pair) if feed.asset_pair else ""
            report += "  {} {}: {:,.8f} | {} | round {}\n".format(feed_id, pair, feed.value, status, feed.round_id)
...
"""
        ...
        for msg in recent_msgs:
            report += "  {}... {}→{} | {} | {:.4f} LINK\n".format(msg.message_id[:20], msg.source_chain, msg.dest_chain, msg.status, msg.fees_paid_link)
...
╚══════════════════════════════════════════════════════════════════╝
""".format(self.generate_seal(master_data))
        return report
...
def demo_chainlink_bridge():
    ...
    for node_id, pubkey, stake, rep in nodes:
        ...
        print("  {} {} | stake: {:,} LINK | rep: {:.2f}".format(status, node_id, stake, rep))
    ...
    for feed_id, ftype, pair in feeds:
        ...
        print("  ○ {} ({}) criado".format(feed_id, ftype.value))
    ...
    for i, price in enumerate(eth_prices):
        node_id = "chainlink-node-{:03d}".format(i+1)
        success = bridge.submit_node_response("ETH/USD", node_id, price)
        status = "✓" if success else "✗ REJEITADO"
        print("  {} {} → ${:,.2f}".format(status, node_id, price))

    # Agregar
    feed = bridge.aggregate_feed("ETH/USD")
    print("\n  → Mediana agregada: ${:,.2f} (round {})".format(feed.value, feed.round_id))
    print("  → Assinatura: {}".format(feed.signature))
    print("  → Nós participantes: {}".format(len(feed.node_responses)))
    ...
    print("  → Randomness verificável: {:.8f}".format(vrf_feed.value))
    print("  → Assinatura: {}".format(vrf_feed.signature))
    ...
    print("  ✓ {}... ARKHE→Ethereum | {:.4f} LINK".format(msg1.message_id[:20], msg1.fees_paid_link))
    ...
    print("  ✓ {}... Ethereum→ARKHE | {:.4f} LINK".format(msg2.message_id[:20], msg2.fees_paid_link))
