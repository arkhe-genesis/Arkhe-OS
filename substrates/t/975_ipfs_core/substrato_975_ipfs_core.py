...
def get_b64_artifacts():
    return {
        "substrate.toml": base64.b64encode(open("substrates/t/975_ipfs_core/substrate.toml", "rb").read()).decode('utf-8'),
        "ipfs_backbone.py": base64.b64encode(open("substrates/t/975_ipfs_core/ipfs_backbone.py", "rb").read()).decode('utf-8')
    }
...
def main():
    ...
    fd, path = tempfile.mkstemp(suffix=".json", prefix="substrate_975_")
    with os.fdopen(fd, "w") as f:
        json.dump(payload, f, indent=2)

    print("Substrate 975 canonized at:", path)
    print("Seal:", seal)

    if len(sys.argv) > 1 and sys.argv[1] == "--extract":
        extract_dir = sys.argv[2] if len(sys.argv) > 2 else "output_975"
