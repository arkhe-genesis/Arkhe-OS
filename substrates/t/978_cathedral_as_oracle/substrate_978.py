...
class CathedralAsOracle:
    ...
    def generate_insight(self, agent_id: str, insight_type: InsightType,
        ...
        insight = CathedralInsight(
            insight_id="ins-" + hashlib.sha3_256((agent_id + ":" + target_asset + ":" + datetime.now().isoformat()).encode()).hexdigest()[:12],
            ...
            historical_accuracy=accuracy if 'accuracy' in locals() else 0.7,
            theosis_at_generation=theosis,
        )

        self.insights.append(insight)

        print("  [{}] Agente {} prediz {}".format(insight_type.value.upper(), agent_id, target_asset))
        print("    Valor: {:,.4f} (intervalo: [{:,.4f}, {:,.4f}])".format(predicted, confidence_low, confidence_high))
        print("    Qualidade: {:.2f} | Theosis: {:.2f}".format(insight.quality_score, theosis))
    ...
    def aggregate_insights(self, target_asset: str, insight_type: InsightType) -> Optional[OraclePublication]:
        ...
        if len(relevant) < 3:
            print("  ✗ Insuficientes insights para {} ({} < 3)".format(target_asset, len(relevant)))
            return None
        ...
        signatures = ["sig_" + hashlib.sha3_256(i.insight_id.encode()).hexdigest()[:8] for i in relevant[:5]]
        ...
        pub = OraclePublication(
            publication_id="pub-" + hashlib.sha3_256((target_asset + ":" + datetime.now().isoformat()).encode()).hexdigest()[:12],
        ...
        if pub.verify():
            ...
            print("\n  ✓ PUBLICAÇÃO CHAINLINK: {}".format(pub.publication_id))
            print("    Ativo: {} | Valor agregado: {:,.4f}".format(target_asset, aggregated))
            print("    Assinaturas: {} nós".format(len(signatures)))
            print("    Reward: {:.4f} LINK (${:.2f})".format(reward, reward * self.link_price_usd))
            print("    Round Chainlink: {}".format(pub.chainlink_round_id))
        else:
            print("  ✗ Publicação rejeitada: qualidade insuficiente")
...
╠══════════════════════════════════════════════════════════════════╣
  INSIGHTS GERADOS: {}
  PUBLICAÇÕES CHAINLINK: {}
  LINK GANHO: {:.4f} LINK (${:,.2f})
  LINK STAKED: {:,.0f} LINK
  ROI: {:.2f}%
...
"""
        for pub in self.publications[-5:]:
            report += "  Round {}: {} = {:,.4f} | +{:.3f} LINK\n".format(pub.chainlink_round_id, pub.insight.target_asset, pub.aggregated_value, pub.link_reward)
...
""".format(self._generate_seal(master_data))
    ...
    def _generate_seal(self, data: dict) -> str:
        ...
        return "978-ORACLE-" + hashlib.sha3_256(json_str.encode()).hexdigest()[:16].upper()
...
def demo_cathedral_as_oracle():
    ...
    print("  Preço realizado ETH/USD: ${:,.2f}".format(realized_eth))
    print("  Acurácias atualizadas para todos os agentes")
