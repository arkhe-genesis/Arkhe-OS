...
class HashtreeCID:
    ...
    def __str__(self) -> str:
        return "nhash:%s..." % self.hash[:16]

    def to_npub_url(self, npub: str, path: str = "") -> str:
        """Gera URL npub/path para Nostr"""
        if path:
            return "htree://%s/%s@%s" % (npub, path, self.hash[:16])
        return "htree://%s@%s" % (npub, self.hash[:16])
...
class NostrPublisher:
    ...
    def publish_merkle_anchor(
        ...
        logging.info("[NostrPublisher] Anchor published: %s..." % event.id[:16])
        return event.id
...
class BlossomClient:
    ...
    def upload_blob(
        ...
        blob = BlossomBlob(
            ...
            url="%s/%s" % (self.servers[0], sha256),
        )

        self._blobs[sha256] = blob
        logging.info("[BlossomClient] Blob uploaded: %s... (%d bytes)" % (sha256[:16], len(data)))
        return blob
    ...
    def download_blob(self, sha256: str) -> Optional[bytes]:
        ...
        if sha256 in self._blobs:
            logging.info("[BlossomClient] Blob downloaded: %s..." % sha256[:16])
...
class P2PTransport:
    ...
    def request_blob(self, cid: str, hops: int = 0) -> Optional[bytes]:
        ...
        for peer_id, peer_info in self._peers.items():
            ...
            logging.debug("[P2P] Requesting %s... from %s... (hops=%d)" % (cid[:16], peer_id[:16], hops))
...
class HashtreeBridge1101:
    ...
    def persist_memory_lake(
        ...
        if encrypt or self.visibility != HashtreeVisibility.PUBLIC:
            ...
            logging.info("[HashtreeBridge] MemoryLake encrypted (CHK key: %s...)" % key[:8].hex())
        ...
        logging.info("[HashtreeBridge] MemoryLake persisted: %s" % cid)
        return cid
    ...
    def persist_proof_chain(
        ...
        logging.info("[HashtreeBridge] ProofChain persisted: %s" % cid)
        return cid
    ...
    def serve_gguf_model(
        ...
        npub_url = cid.to_npub_url(npub, "models/%s" % repo_name)
        logging.info("[HashtreeBridge] GGUF model served: %s" % npub_url)
...
if __name__ == "__main__":
    ...
    print("\nMemoryLake CID: %s" % cid)
    print("Merkle Root: %s..." % bridge.merkle.build_tree()[:32])
