...
def get_b64_artifacts():
    return {
        "substrate.toml": base64.b64encode(open("substrates/t/970_enterprise_mind/substrate.toml", "rb").read()).decode('utf-8'),
        "enterprise_mind.py": base64.b64encode(open("substrates/t/970_enterprise_mind/enterprise_mind.py", "rb").read()).decode('utf-8')
    }
...
def main():
    ...
    fd, path = tempfile.mkstemp(suffix=".json", prefix="substrate_970_")
    with os.fdopen(fd, "w") as f:
        json.dump(payload, f, indent=2)

    print("Substrate 970 canonized at:", path)
    print("Seal:", seal)

    if len(sys.argv) > 1 and sys.argv[1] == "--extract":
        extract_dir = sys.argv[2] if len(sys.argv) > 2 else "output_970"
