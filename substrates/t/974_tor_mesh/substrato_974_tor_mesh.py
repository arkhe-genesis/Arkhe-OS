...
def get_b64_artifacts():
    return {
        "substrate.toml": base64.b64encode(open("substrates/t/974_tor_mesh/substrate.toml", "rb").read()).decode('utf-8'),
        "tor_service.py": base64.b64encode(open("substrates/t/974_tor_mesh/tor_service.py", "rb").read()).decode('utf-8')
    }
...
def main():
    ...
    fd, path = tempfile.mkstemp(suffix=".json", prefix="substrate_974_")
    with os.fdopen(fd, "w") as f:
        json.dump(payload, f, indent=2)

    print("Substrate 974 canonized at:", path)
    print("Seal:", seal)

    if len(sys.argv) > 1 and sys.argv[1] == "--extract":
        extract_dir = sys.argv[2] if len(sys.argv) > 2 else "output_974"
