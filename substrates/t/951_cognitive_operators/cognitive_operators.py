...
if __name__ == "__main__":
    ...
    print(f"  Normas originais: {[f'{np.linalg.norm(v):.2f}' for v in mem_matrix]}")  # noqa: FS002

    reduced = lll.reduce(mem_matrix, track_history=True)
    print(f"  Normas reduzidas: {[f'{np.linalg.norm(v):.2f}' for v in reduced]}")  # noqa: FS002

    # Verificar propriedade LLL: primeiro vetor aproxima SVP
    sv = lll.get_shortest_vector()
    print(f"  Vetor mais curto (aprox SVP): norm={np.linalg.norm(sv):.4f}")  # noqa: FS002
    ...
    print(f"  Desvio de ortogonalidade: {orthogonality:.6f} (≈0 = bom)")  # noqa: FS002
    ...
    print(f"  Mapa de atenção (primeiros 3 blocos): "  # noqa: FS002
          f"{[(k, f'{v:.3f}') for k, v in list(attention_map.items())[:3]]}")  # noqa: FS002

    coherence_field = bkz.get_coherence_field(mem_bkz)
    print(f"  Campo de coerência: mean={np.mean(coherence_field):.3f}, "  # noqa: FS002
          f"std={np.std(coherence_field):.3f}")  # noqa: FS002
    ...
    print(f"  MSE reconstrução: {mse:.2e}")  # noqa: FS002
    assert mse < 1e-6, f"NTT roundtrip falhou: MSE={mse}"  # noqa: FS002

    # Cross-modal binding
    audio_signal = np.random.randint(0, 256, 256)
    audio_spec = ntt.sense(audio_signal)
    bound = ntt.bind(spectrum, audio_spec)
    print(f"  Energia do binding: {np.sum(bound**2):.2e}")  # noqa: FS002
    ...
    print(f"  Percepções geradas: {list(result['percepts'].keys())}")  # noqa: FS002
    print(f"  Memórias consolidadas: {len(result['consolidated'])}")  # noqa: FS002
    print(f"  Coerência global: {result['global_coherence']:.3f}")  # noqa: FS002
    print(f"  Estado Bindu (unificado): {result['bindu_state']}")  # noqa: FS002
    ...
    print(f"  Memórias de entrada: 90")  # noqa: FS002
    print(f"  Memórias consolidadas: {len(consolidated_dream)}")  # noqa: FS002
    print(f"  Fator de compressão: {90/len(consolidated_dream):.2f}x")  # noqa: FS002
