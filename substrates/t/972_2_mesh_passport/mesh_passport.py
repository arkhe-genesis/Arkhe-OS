...
class PQMeshProtocol:
    ...
    def encrypt_message(self, peer_id: str, plaintext: bytes) -> bytes:
        ...
        if not session_key:
            raise SecurityException(f"Sem sessão ativa com {peer_id}")  # noqa: FS002
...
class MeshNodeServer:
    ...
    def start(self):
        ...
        print(f"[MeshNode {self.protocol.node_id}] Listening on {self.host}:{self.port}")  # noqa: FS002
    ...
    def _handle_connection(self, conn: socket.socket, addr: Tuple[str, int]):
        try:
            ...
            print(f"[MeshNode {self.protocol.node_id}] Handshake OK with {peer_identity.node_id}")  # noqa: FS002
            ...
            while self.running:
                ...
                print(f"[MeshNode {self.protocol.node_id}] Received: {plaintext.decode('utf-8', errors='replace')[:100]}")  # noqa: FS002

        except Exception as e:
            print(f"[MeshNode {self.protocol.node_id}] Connection error: {e}")  # noqa: FS002
        finally:
            conn.close()
    ...
    def connect_to_peer(self, host: str, port: int):
        ...
        print(f"[MeshNode {self.protocol.node_id}] Connected to {peer_identity.node_id}")  # noqa: FS002
...
class PQPassportGateway:
    ...
    def issue_stamp(self, stamp: PassportStamp) -> bytes:
        ...
        # Criar registro
        stamp_key = f"{stamp.holder_orcid}:{stamp.stamp_type}:{stamp.stamp_id}"  # noqa: FS002
    ...
    def revoke_stamp(self, stamp_key: str) -> bytes:
        ...
        proof = hashlib.sha3_256(f"REVOKE:{stamp_key}:{time.time()}".encode()).digest()  # noqa: FS002
        return proof
...
class PQMeshConsensus:
    ...
    def propose(self, proposal: bytes) -> str:
        ...
        proposal_id = hashlib.sha3_256(proposal + secrets.token_bytes(16)).hex()
    ...
    def get_certificate(self, proposal_id: str) -> bytes:
        ...
        cert_data = b''.join(
            f"{node_id}:{sig.hex()}".encode()   # noqa: FS002
...
if __name__ == "__main__":
    ...
    print(f"  ✓ Mensagem criptografada: {decrypted.decode()}")  # noqa: FS002
    ...
    assert valid
    print(f"  ✓ Stamp emitido e verificado: {stamp.stamp_type}")  # noqa: FS002
    ...
    assert is_human
    print(f"  ✓ Passport verificado: human={is_human}, confidence={confidence:.2%}")  # noqa: FS002
    ...
    print(f"  ✓ Consenso atingido: cert={cert.hex()[:16]}...")  # noqa: FS002
