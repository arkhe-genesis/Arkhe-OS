...
def get_b64_artifacts():
    ...
    # Files to include
    payload_path = os.path.join(node_dir, "kernel_isolation_engine.py")

    # Ensure substrate.toml exists
    toml_path = os.path.join(base_dir, "substrate.toml")
    if not os.path.exists(toml_path):
        with open(toml_path, "w") as f:
            f.write("[substrate]\nid = \"989.z\"\nname = \"Kernel Isolation Engine\"\n")

    return {
        "kernel_isolation_engine.py": base64.b64encode(open(payload_path, "rb").read()).decode("utf-8"),
        "substrate.toml": base64.b64encode(open(toml_path, "rb").read()).decode("utf-8")
    }

class Substrato_989_z_kernel_isolation_engine:
    def canonize(self):
        ...
        output = {
            "Substrate": "989.z-KERNEL-ISOLATION-ENGINE",
...
if __name__ == "__main__":
    canonizer = Substrato_989_z_kernel_isolation_engine()
