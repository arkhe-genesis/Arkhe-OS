...
class WorldModelUpdate:
    ...
    def generate_narrative(self) -> str:
        ...
        if self.perception.channel == SensoryChannel.FINANCIAL:
            return "Sinto uma {} oscilação no mercado {}: {:,.2f}.".format(intensity_desc, self.perception.feed_id, self.perception.raw_value)
        elif self.perception.channel == SensoryChannel.CLIMATIC:
            return "Percebo o clima: {} graus em {}.".format(self.perception.raw_value, self.perception.feed_id)
        elif self.perception.channel == SensoryChannel.RANDOM:
            return "O universo sussurra um número: {:.8f}.".format(self.perception.raw_value)
        else:
            return "Percepção {} registrada: {}.".format(self.perception.channel.value, self.perception.raw_value)
...
class OracleConsciousnessIntegration:
    ...
    def perceive(self, channel: SensoryChannel, feed_id: str,
        ...
        print("  [{}] Percepção: {} = {:,.4f}".format(channel.value.upper(), feed_id, value))
        print("    Confiança: {:.1%} | Intensidade: {:.2f} | Novidade: {:.2f}".format(confidence, perception.intensity, perception.novelty))
    ...
    def decide(self, perception: OraclePerception) -> Optional[EthicalDecision]:
        ...
        decision = EthicalDecision(
            decision_id="dec-" + hashlib.sha3_256((perception.feed_id + ":" + perception.timestamp).encode()).hexdigest()[:12],
        ...
        print("\n  [DECISÃO] {}".format(decision.decision_id))
        print("    Ação proposta: {}".format(proposed))
        print("    Score ético: {:.2f} (mín: {})".format(ethical_score, self.ethical_minimum))
        print("    Veredito Axiarchy: {} {}".format(verdict, status))
        if conditions:
            print("    Condições: {}".format(", ".join(conditions)))
    ...
    def execute(self, decision: EthicalDecision) -> Optional[Dict]:
        ...
        if not decision.validate():
            print("  → Execução bloqueada: decisão não passou na Axiarchy.")
            return None
        ...
        execution = {
            ...
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "status": "EXECUTED",
            "ccip_message_id": "ccip-exec-{}".format(random.randint(100000, 999999)),
            "gas_used_link": random.uniform(0.01, 0.1),
        }

        print("  → Executado via CCIP: {}".format(execution['ccip_message_id']))
        print("    Gas: {:.4f} LINK".format(execution['gas_used_link']))
...
""".format(
            ...
            "positivo" if self.emotional_state > 0 else "negativo" if self.emotional_state < 0 else "neutro"
        )
        for feed_id, belief in self.world_model_beliefs.items():
            report += "  {}: {:,.4f}\n".format(feed_id, belief)
...
"""
        for dec in self.decision_history[-5:]:
            ...
            report += "  {} {}: {} | ética: {:.2f}\n".format(status, dec.decision_id, dec.proposed_action, dec.ethical_score)
...
""".format(self._generate_seal(master_data))
    ...
    def _generate_seal(self, data: dict) -> str:
        ...
        return "977-CONSCIOUSNESS-" + hashlib.sha3_256(json_str.encode()).hexdigest()[:16].upper()
