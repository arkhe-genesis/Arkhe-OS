...
class FECParser:
    ...
    def load(self) -> bool:
        try:
            ...
            self.result.errors.append("Erro ao ler arquivo: {0}".format(e))
            return False
    ...
    def parse(self) -> FECFile:
        ...
        for idx, fields in enumerate(self.lines, start=1):
            ...
            if record_type == "HDR":
                ...
                self.result.warnings.append("Linha {0}: Schedule C (loans) — parsing básico".format(idx))
            elif record_type.startswith("SD"):
                self.result.warnings.append("Linha {0}: Schedule D (debts) — parsing básico".format(idx))
            elif record_type.startswith("SE"):
                self.result.warnings.append("Linha {0}: Schedule E (independent expenditures) — parsing básico".format(idx))
            else:
                self.result.warnings.append("Linha {0}: tipo de registro desconhecido '{1}'".format(idx, record_type))
    ...
    def _parse_header(self, fields: List[str], idx: int):
        if len(fields) < 4:
            self.result.errors.append("Linha {0} (HDR): campos insuficientes".format(idx))
            return
    ...
    def _parse_form(self, fields: List[str], idx: int):
        ...
        if form_type not in FORM_TYPES:
            self.result.warnings.append("Linha {0}: form type '{1}' não reconhecido".format(idx, form_type))

        # Form types have variable lengths; we handle common ones
        if form_type in ("F3", "F3X", "F3P"):
            if len(fields) < 40:
                self.result.errors.append("Linha {0} ({1}): campos insuficientes ({2})".format(idx, form_type, len(fields)))
                return

            committee_id = fields[1].strip()
            if not validate_committee_id(committee_id):
                self.result.errors.append("Linha {0} ({1}): committee ID inválido '{2}'".format(idx, form_type, committee_id))

            report_code = fields[25].strip() if len(fields) > 25 else ""
            if report_code and not validate_report_code(report_code):
                self.result.warnings.append("Linha {0} ({1}): report code desconhecido '{2}'".format(idx, form_type, report_code))
            ...
            if coverage_from and not validate_date(coverage_from):
                self.result.errors.append("Linha {0} ({1}): coverage from date inválida '{2}'".format(idx, form_type, coverage_from))
            if coverage_through and not validate_date(coverage_through):
                self.result.errors.append("Linha {0} ({1}): coverage through date inválida '{2}'".format(idx, form_type, coverage_through))
    ...
    def _parse_schedule_a(self, fields: List[str], idx: int):
        if len(fields) < 20:
            self.result.errors.append("Linha {0} (SA): campos insuficientes ({1})".format(idx, len(fields)))
            return

        committee_id = fields[1].strip()
        if not validate_committee_id(committee_id):
            self.result.warnings.append("Linha {0} (SA): committee ID '{1}' formato não padrão".format(idx, committee_id))

        date_str = fields[13].strip() if len(fields) > 13 else ""
        if date_str and not validate_date(date_str):
            self.result.errors.append("Linha {0} (SA): contribution date inválida '{1}'".format(idx, date_str))

        amount_ok, amount = validate_amount(fields[14]) if len(fields) > 14 else (False, 0.0)
        if not amount_ok:
            self.result.errors.append("Linha {0} (SA): contribution amount inválido".format(idx))
        ...
        if state and not validate_state(state):
            self.result.warnings.append("Linha {0} (SA): state '{1}' não reconhecido".format(idx, state))

        zip_code = fields[11].strip() if len(fields) > 11 else ""
        if zip_code and not validate_zip(zip_code):
            self.result.warnings.append("Linha {0} (SA): ZIP '{1}' formato inválido".format(idx, zip_code))
    ...
    def _parse_schedule_b(self, fields: List[str], idx: int):
        if len(fields) < 20:
            self.result.errors.append("Linha {0} (SB): campos insuficientes ({1})".format(idx, len(fields)))
            return

        committee_id = fields[1].strip()
        if not validate_committee_id(committee_id):
            self.result.warnings.append("Linha {0} (SB): committee ID '{1}' formato não padrão".format(idx, committee_id))

        date_str = fields[13].strip() if len(fields) > 13 else ""
        if date_str and not validate_date(date_str):
            self.result.errors.append("Linha {0} (SB): disbursement date inválida '{1}'".format(idx, date_str))

        amount_ok, amount = validate_amount(fields[14]) if len(fields) > 14 else (False, 0.0)
        if not amount_ok:
            self.result.errors.append("Linha {0} (SB): disbursement amount inválido".format(idx))

        state = fields[10].strip() if len(fields) > 10 else ""
        if state and not validate_state(state):
            self.result.warnings.append("Linha {0} (SB): state '{1}' não reconhecido".format(idx, state))
        ...
        for sa in self.result.schedules_a:
            if sa.filer_committee_id not in committee_ids:
                self.result.warnings.append(
                    "Schedule A committee ID '{0}' não encontrado nos forms".format(sa.filer_committee_id)
                )
        ...
        for sa in self.result.schedules_a:
            if sa.contribution_amount > 200.0:
                if not sa.employer or not sa.occupation:
                    self.result.warnings.append(
                        "SA txid={0}: contribuição > $200 sem employer/occupation".format(sa.transaction_id)
                    )
...
def format_report(result: FECFile) -> str:
    lines = []
    lines.append("=" * 70)
    lines.append("RELATÓRIO DE VALIDAÇÃO FEC — {0}".format(result.filename))
    lines.append("SHA3-256: {0}".format(result.sha3_256))
    lines.append("=" * 70)

    if result.header:
        lines.append("\n📋 HEADER:")
        lines.append("   Software: {0} v{1}".format(result.header.software_name, result.header.software_version))
        lines.append("   EF Type: {0}".format(result.header.ef_type))

    if result.errors:
        lines.append("\n❌ ERROS ({0}):".format(len(result.errors)))
    ...
    if result.warnings:
        lines.append("\n⚠️  AVISOS ({0}):".format(len(result.warnings)))
    ...
    lines.append("\n📊 Resumo:")
    lines.append("   Forms:         {0}".format(len(result.forms)))
    lines.append("   Schedule A:    {0} transações | ${1:,.2f}".format(len(result.schedules_a), total_receipts))
    lines.append("   Schedule B:    {0} transações | ${1:,.2f}".format(len(result.schedules_b), total_disbursements))
    lines.append("   Outros:        {0} registros".format(len(result.raw_lines) - len(result.forms) - len(result.schedules_a) - len(result.schedules_b)))
            ...
            "forms": [
                {
                    ...
                    "coverage": "{0} - {1}".format(f.coverage_from_date, f.coverage_through_date),
                }
