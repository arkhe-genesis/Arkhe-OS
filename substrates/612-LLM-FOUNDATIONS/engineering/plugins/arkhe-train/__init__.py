...
for pillar, topics in CURRICULUM.items():
    ...
    for i, topic in enumerate(topics, 1):
        sub = (i - 1) // 10 + 1
        t_num = ((i - 1) % 10) + 1
        _TOPIC_ID_MAP[topic] = "612.%s.%d.%d" % (p_num, sub, t_num)
...
def list_units(pillar, trained):
    ...
    for p_name, topics in CURRICULUM.items():
        if pillar and not p_name.startswith(pillar):
            continue
        click.echo("\n\033[1;36m%s\033[0m (%d CKUs)" % (p_name, len(topics)))
        for t in topics:
            tid = _TOPIC_ID_MAP.get(t, "?")
            status = "✓" if _is_trained(tid) else "○"
            if trained and not _is_trained(tid):
                continue
            click.echo("  %s \033[90m%s\033[0m %s" % (status, tid, t))
            total += 1
    click.echo("\nTotal: %d CKUs disponíveis para treinamento" % total)
...
def search_unit(query, fuzzy):
    ...
    if found:
        click.echo("\n\033[1;32mCKUs encontradas para '%s' (%d matches):\033[0m" % (query, len(found)))
        for p, t in found:
            tid = _TOPIC_ID_MAP.get(t, "?")
            click.echo("  \033[90m[%s]\033[0m \033[1m%s\033[0m %s" % (p, tid, t))
    else:
        click.echo("\033[1;31mNenhuma CKU encontrada para '%s'.\033[0m" % query)
...
def show_unit(topic):
    ...
    if topic.startswith("612."):
        ...
        if t_name:
            ...
            click.echo("\033[1;31mCKU ID '%s' não encontrada.\033[0m" % topic)
    else:
        if topic in _TOPIC_ID_MAP:
            ...
            if len(matches) == 1:
                ...
                for m in matches:
                    click.echo("  • %s %s" % (_TOPIC_ID_MAP[m], m))
            else:
                click.echo("\033[1;31mCKU '%s' não encontrada no cânone.\033[0m" % topic)
...
def show_progress(ia, format):
    ...
    if format == "json":
        ...
        click.echo("\n\033[1;36mARKHE TRAIN — Progresso da IA: %s\033[0m" % ia)
        click.echo("CKUs treinadas: %d/%d (%.1f%%)" % (len(trained), total, pct))

        for p_name, topics in CURRICULUM.items():
            p_trained = sum(1 for t in topics if _TOPIC_ID_MAP.get(t) in trained)
            bar = "█" * p_trained + "░" * (len(topics) - p_trained)
            click.echo("  %-24s %s %d/%d" % (p_name, bar, p_trained, len(topics)))
...
def certify_ia(ia, level, architect):
    ...
    if engine.is_ia_certified(level):
        ...
        click.echo("  Modelo:    %s" % badge['ia_model_id'])
        click.echo("  Nível:     %s" % badge['level_name'])
        click.echo("  Classe:    %s" % badge['intelligence_class'])
        click.echo("  Arquiteto: %s" % badge['architect_orcid'])
        click.echo("  Badge:     %s" % badge['cert_id'])
        click.echo("  Seal:      %s..." % badge['seal_sha256'][:16])
        click.echo("  Temporal:  %s" % badge['temporalchain_anchor'])
        click.echo("\n  \033[90m%s...\033[0m" % badge['class_description'][:80])
    else:
        click.echo("\n\033[1;31m✗ IA não atinge critérios para %s\033[0m" % level)
...
def audit_ia(ia, architect):
    ...
    click.echo("\n\033[1;36m[612↔604-CAI] Iniciando auditoria canônica da IA: %s\033[0m" % ia)
    click.echo("  Arquiteto: %s" % architect)
...
def _get_ia_training_state(ia_model_id):
    ...
    state_file = Path.home() / ".arkhe" / ("ia_training_%s.json" % ia_model_id.replace('/', '_'))
