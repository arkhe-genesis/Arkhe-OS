...
class CurriculumRepoIndexer:
    ...
    def clone_and_index(self, topic_id, repo_url, force=False):
        ...
        if not target_dir.exists() or force:
            print("[612↔611] Cloning %s for %s..." % (repo_name, topic_id))
            subprocess.run(
        ...
        if not codegraph_dir.exists() or force:
            print("[612↔611] Indexing training repos with CodeGraph: %s" % repo_name)
            try:
                subprocess.run(
        ...
        if IPFS_PIN and codegraph_dir.exists():
            try:
                ...
                print("  📌 IPFS CID: %s" % cid)
            except Exception as e:
                print("  ⚠ IPFS upload failed: %s" % e)
    ...
    def index_all(self, force=False):
        ...
        print("[612↔611] Indexing training repos %d repositories across %d topics..." % (total, len(self.repo_map)))
        ...
        print("  Total indexed: %d repos" % len(self.index_log))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--topic":
        ...
        print("Training repos for CKU %s:" % topic)
        for e in entries:
            print("  • %s → %s" % (e['repo'], e['url']))
            if e.get("ipfs_cid"):
                print("    IPFS: %s" % e['ipfs_cid'])
    else:
        manifest = indexer.index_all()
        print("\nSeal: %s..." % manifest['seal'][:16])
