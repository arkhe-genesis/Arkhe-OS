...
class CurriculumDistiller(Distiller):
        ...
        for topic, freq in topic_frequency.items():
            if freq >= self.shallow_threshold and self._is_superficial(trajectory, topic):
                ...
                candidates.append({
                    ...
                    "content": (
                        "[Training 612] IA touched on CKU '%s' %dx " % (topic, freq) +
                        "but internalized superficially. Inject reinforcement with: %s. " % related_str +
                        "Next training unit: arkhe train --ia <model> --topic %s" % self._topic_to_id(topic)
                    ),
                    "orientation_score": self.orientation_score_base,
                    "transferability": "Any query about %s" % topic,
        ...
        for topic in all_seen:
            ...
            if missing:
                candidates.append({
                    ...
                    "content": (
                        "[Curriculum 612] Prerequisite gap detected for '%s': " % topic +
                        "missing foundations: %s. " % ', '.join(missing) +
                        "Recommended: arkhe learn --path '%s'" % self._topic_to_id(topic)
                    ),
                    "orientation_score": 0.85,
                    "transferability": "Any query requiring %s" % topic,
    ...
    def _topic_to_id(self, topic):
        ...
        return mapping.get(topic, "612.UNKNOWN.%s" % topic)
...
if __name__ == "__main__":
    ...
    print("Generated %d orientation candidates:" % len(candidates))
    for c in candidates:
        print("  [%s] %s..." % (c['section'], c['content'][:100]))
