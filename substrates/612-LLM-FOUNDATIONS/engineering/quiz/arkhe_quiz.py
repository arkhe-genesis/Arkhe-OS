...
class AI_CertificationEngine:
    ...
    def __init__(self, ia_model_id, architect_orcid, curriculum_registry=None):
        ...
        self.scores = {p: {} for p in ["P%d" % i for i in range(1, 12)]}
        self.ia_history = {}
    ...
    def _generate_generic_question(self, topic_id, difficulty):
        return {"qid": "%s.QGEN" % topic_id, "question": "Explain the core concept of %s." % topic_id, "type": "open", "difficulty": difficulty, "explanation": "[Generated by LLM judge]", "rubric": ["accuracy", "clarity", "depth"]}
    ...
    def issue_ia_badge(self, level="Nivel3"):
        ...
        badge = {
            "cert_id": "%s-%s" % (cfg['badge'], hashlib.sha256(self.ia_model_id.encode()).hexdigest()[:8]),
        ...
        badge["temporalchain_anchor"] = "9018.block#%d" % int(time.time() / 10)
        badge["verification_url"] = "https://arkhe.org/cert/verify/%s" % badge['cert_id']
    ...
    def save_training_state(self):
        ...
        state_file = Path.home() / ".arkhe" / ("ia_training_%s.json" % self.ia_model_id.replace('/', '_'))
...
if __name__ == "__main__":
    ...
    print("[TRAIN] Topic 612.P1.1.3 — Score: %.1f%%" % result['score'])

    if engine.is_ia_certified("Nivel1"):
        badge = engine.issue_ia_badge("Nivel1")
        print("[BADGE] %s" % badge['cert_id'])
        print("        Class: %s" % badge['intelligence_class'])
        print("        Architect: %s" % badge['architect_orcid'])
        print("        Seal: %s..." % badge['seal_sha256'][:16])
