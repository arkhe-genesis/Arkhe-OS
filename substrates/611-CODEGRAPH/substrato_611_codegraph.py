...
def canonize_611():
    # We will generate files in a safe temp directory
    temp_dir = tempfile.mkdtemp()

    plugin_base = os.path.join(temp_dir, "codegraph")
    os.makedirs(plugin_base, exist_ok=True)

    with open(os.path.join(plugin_base, "__init__.py"), "w") as f:
        f.write('"""ARKHE OS — Plugin codegraph (CodeGraph Knowledge Graph)"""\nfrom .codegraph_bridge import register_commands\n__all__ = ["register_commands"]\n')

    with open(os.path.join(plugin_base, "plugin.toml"), "w") as f:
    ...
    bridge_content = r'''#!/usr/bin/env python3
"""
...
'''
    ...
    with open(os.path.join(plugin_base, "codegraph_bridge.py"), "w") as f:
    ...
    canonical_dict = {
        "substrate": "611-CODEGRAPH",
        "description": "CodeGraph — Pre-indexed Code Knowledge Graph for AI Agents",
        "files": {
            "__init__.py": os.path.join(plugin_base, "__init__.py"),
            "plugin.toml": os.path.join(plugin_base, "plugin.toml"),
            "codegraph_bridge.py": os.path.join(plugin_base, "codegraph_bridge.py")
        },
