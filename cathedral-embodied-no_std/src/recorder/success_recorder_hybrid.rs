...
impl HybridRecorder {
    ...
    pub fn record_round(&mut self, round: u32, acceptance_rate: f32, memory_proof_used: bool) {
        match &mut self.backend {
            RecorderBackend::Sqlite(r) => {
                let _ = r.record_round(round, acceptance_rate, memory_proof_used);
            }
    ...
    pub fn average_acceptance_rate(&self, last_n: Option<u32>) -> Result<f32, rusqlite::Error> {
        match &self.backend {
            RecorderBackend::Sqlite(r) => r.average_acceptance_rate(last_n),
            RecorderBackend::Json(r) => Ok(r.average_acceptance_rate(last_n.map(|n| n as usize))),
        }
    }
    ...
    pub fn get_top_performing_hubs(&self, min_rounds: u32) -> Result<Vec<(String, f32, u32)>, rusqlite::Error> {
        match &self.backend {
            RecorderBackend::Sqlite(r) => r.get_top_performing_hubs(min_rounds),
            RecorderBackend::Json(_) => Ok(Vec::new()),
        }
    }
