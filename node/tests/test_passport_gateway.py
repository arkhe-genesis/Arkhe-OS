...
def mock_session():
    ...
    def side_effect_mock(*args, **kwargs):
        if hasattr(session.get, 'side_effect') and session.get.side_effect:
            val = session.get.side_effect.pop(0) if isinstance(session.get.side_effect, list) else session.get.side_effect(*args, **kwargs)
            return AsyncContextManagerMock(val)
        return AsyncContextManagerMock(session.get.return_value)

    session.get = side_effect_mock
    return session
...
class MockResponse:
    def __init__(self, status, json_data, text_data="Error"):
        self.status = status
...
async def test_is_human_high_score(gateway, mock_session):
    ...
    mock_session.get.side_effect = [score_resp, stamps_resp]
    gateway._session = mock_session
...
async def test_is_human_api_error_with_orcid(gateway, mock_session):
    ...
    score_resp._text_data = "Internal Server Error"

    # We must properly mock ORCID to return a valid 200 response when the passport scoring fails
    orcid_data = {
        "orcid-identifier": {"path": "0009-0005-2697-4668"},
        "person": {"name": {"given-names": {"value": "Arquiteto"}}}
    }
    orcid_resp = make_mock_response(200, orcid_data)

    mock_session.get.side_effect = [score_resp, orcid_resp]
    gateway._session = mock_session
