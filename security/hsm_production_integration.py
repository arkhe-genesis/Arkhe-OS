#!/usr/bin/env python3
"""
Substrato 224: HSM Production Integration
Integração com HSM real (Thales nCipher, Utimaco) para assinatura PQC
com chaves enraizadas em hardware, nunca exportáveis.
"""
import asyncio
import hashlib
import json
...
# Tentar importar bibliotecas de produção
try:
...
class HSMProductionConfig:
    ...
    provider: str  # "thales_ncipher", "utimaco", "aws_cloudhsm", "azure_dedicated"
    pkcs11_library: str  # Caminho para biblioteca PKCS#11
    slot_id: int
    token_label: str
    key_label: str
    ...
    fallback_classic: bool = True  # Fallback para SHA3-256 em caso de falha
...
class HSMProductionPQCSigner:
    ...
    async def _connect_to_hsm(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ Não foi possível carregar lib PKCS11 ({self.config.pkcs11_library}): {e}. Usando mock.")
                self._key_metadata = {
                    "key_label": self.config.key_label,
                    "algorithm": self.config.pqc_algorithm,
                    "security_level": self.SUPPORTED_ALGORITHMS.get(
                    ...
                    "created_at": time.time(),
                    "mode": "simulated_due_to_load_error"
                }
                return
    ...
    async def sign_data(
        ...
        try:
            ...
            if self.config.fallback_classic and context and context.get("allow_classic_fallback"):
    ...
    def get_signer_statistics(self) -> Dict:
        ...
        return {
            ...
            "mode": "production" if HSM_LIB_AVAILABLE else "simulated"
        }
