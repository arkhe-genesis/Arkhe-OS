...
class FederatedAuditLedger:
    ...
    async def append_entry(
        ...
        """
        # Covert string to enum if needed
        if isinstance(entry_type, str):
            try:
                entry_type = LedgerEntryType[entry_type]
            except KeyError:
                logging.warning(f"⚠️ Invalid entry_type string: {entry_type}. Falling back to CONSENSUS_ROUND")
                entry_type = LedgerEntryType.CONSENSUS_ROUND
