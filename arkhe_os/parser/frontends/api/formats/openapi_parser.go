// parser/frontends/api/formats/openapi_parser.go
package formats
...
func ParseOpenAPI(source []byte) (*models.APISpecification, error) {
	...
	if err := yaml.Unmarshal(source, &doc); err != nil {
		if err := json.Unmarshal(source, &doc); err != nil {
			return nil, fmt.Errorf("failed to parse OpenAPI (YAML/JSON): %w", err)
		}
	}

	spec := &models.APISpecification{
		Name:        extractString(doc, "info.title", "unnamed_api"),
		Version:     extractString(doc, "info.version", "0.0.0"),
