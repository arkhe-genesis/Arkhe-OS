...
// ContainerizationFrontend parser Containerization -> LFIR
type ContainerizationFrontend struct {
	mu      sync.Mutex
	metrics FrontendMetrics
}

func NewContainerizationFrontend() *ContainerizationFrontend { return &ContainerizationFrontend{} }

func (c *ContainerizationFrontend) GetLanguage() string { return "containerization" }
func (c *ContainerizationFrontend) GetExtensions() []string { return []string{"Dockerfile", ".yaml", ".yml", ".log"} }

func (c *ContainerizationFrontend) Parse(source string) (*LFIRGraph, error) {
	start := time.Now()
	graph := NewLFIRGraph()

	module := NewLFIRNode(LFIRModule, "container_orchestration", "containerization")
	graph.AddNode(module)
	graph.RootNodes = append(graph.RootNodes, module.ID)
	...
	for i, line := range lines {
		...
		if strings.HasPrefix(trimmed, "FROM ") {
			baseImage := strings.TrimSpace(strings.TrimPrefix(trimmed, "FROM "))
			node := NewLFIRNode(LFIRType, "container_base", "containerization")
			node.Attributes["line"] = i + 1
			node.Attributes["image"] = baseImage
			node.Attributes["kind"] = "docker_from"
			graph.AddNode(node)
			graph.Link(module.ID, node.ID)
		} else if strings.HasPrefix(trimmed, "RUN ") || strings.HasPrefix(trimmed, "CMD ") || strings.HasPrefix(trimmed, "ENTRYPOINT ") {
			cmd := strings.SplitN(trimmed, " ", 2)[0]
			node := NewLFIRNode(LFIROperation, cmd, "containerization")
			node.Attributes["line"] = i + 1
			node.Attributes["command"] = trimmed
			node.Attributes["kind"] = "docker_cmd"
			graph.AddNode(node)
			graph.Link(module.ID, node.ID)
		}

		// Kubernetes YAML (simplified heuristic)
		if strings.HasPrefix(trimmed, "apiVersion:") {
			node := NewLFIRNode(LFIRType, "k8s_manifest", "containerization")
			node.Attributes["line"] = i + 1
			node.Attributes["apiVersion"] = strings.TrimSpace(strings.Split(trimmed, ":")[1])
			node.Attributes["kind"] = "k8s_api"
			graph.AddNode(node)
			graph.Link(module.ID, node.ID)
		} else if strings.HasPrefix(trimmed, "kind:") {
			kind := strings.TrimSpace(strings.Split(trimmed, ":")[1])
			node := NewLFIRNode(LFIRType, kind, "containerization")
			node.Attributes["line"] = i + 1
			node.Attributes["kind"] = "k8s_kind"
			node.Attributes["k8s_type"] = kind
			graph.AddNode(node)
			graph.Link(module.ID, node.ID)
		}

		// Container logs
		if strings.Contains(trimmed, "INFO") || strings.Contains(trimmed, "ERROR") || strings.Contains(trimmed, "WARN") {
			node := NewLFIRNode(LFIROperation, "container_log", "containerization")
			node.Attributes["line"] = i + 1
			node.Attributes["log_entry"] = trimmed
			node.Attributes["kind"] = "log_event"
			graph.AddNode(node)
			graph.Link(module.ID, node.ID)
		}
	}
	...
	c.mu.Lock()
	c.metrics.FilesParsed++
	c.metrics.LinesParsed += int64(len(lines))
	c.metrics.AvgParseTimeMs = (c.metrics.AvgParseTimeMs*float64(c.metrics.FilesParsed-1) + float64(time.Since(start).Milliseconds())) / float64(c.metrics.FilesParsed)
	c.mu.Unlock()

	fmt.Printf("\n🐳 CONTAINERIZATION-FRONTEND: %d nós extraídos de %d linhas\n", len(graph.Nodes), len(lines))
	return graph, nil
}
...
func (r *RustBackend) Generate(graph *LFIRGraph) (string, error) {
	...
	for _, node := range graph.Nodes {
		if node.Type == LFIRType {
			if node.Attributes["kind"] == "struct" {
...
func (o *PhiOptimizer) Optimize(graph *LFIRGraph) {
	...
	for _, node := range graph.Nodes {
		...
		if len(node.Children) > int(math.Round(Phi*5.0)) {
...
func NewPolymathParser() *PolymathParser {
	...
	pp.RegisterFrontend(NewContainerizationFrontend())
	pp.RegisterFrontend(NewContainerizationFrontend())
