//! substrato-7001/src/webhooks/polar_handler.rs
//! Handler de webhooks Polar com retry exponencial + DLQ
//!
//! Correções v2.0.0:
//!   - Import Arc corretamente
//!   - Backoff exponencial com tokio-retry
//!   - Dead letter queue para eventos que falharam 3x
//!   - Conecta com OssDistributionEngine no fluxo order.paid
//!   - Assinatura verificada via header Polar-Signature (não x-polar-signature)
//!
//! Selo: CATHEDRAL-ARKHE-POLAR-WEBHOOKS-v2.0.0-2026-06-19
...
impl PolarWebhookHandler {
    ...
    pub async fn process_with_retry(&self, event_type: &str, data: Value) {
        let action = || async {
