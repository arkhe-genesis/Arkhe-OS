...
async fn main() -> anyhow::Result<()> {
    ...
    // 1. Configuração
    let webhook_config = app_webhooks::WebhookConfig::from_env()?;
    let dlq: app_webhooks::DeadLetterQueue = Arc::new(RwLock::new(Vec::new()));

    // 2. Inicializa handler
    let webhook_handler = Arc::new(
        app_webhooks::PolarWebhookHandler::new(webhook_config, Arc::clone(&dlq))
    );
    ...
    // 4. Router Axum
    let app = app_webhooks::create_webhook_router(webhook_handler, dlq);
