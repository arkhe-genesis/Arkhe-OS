...
impl EvoField {
    ...
    // ✅ Inicialização Harris Sheet (corrigida com random_harris integrado)
    ...
    pub fn random_harris(config: PlasmaConfig) -> Self {
        ...
        for i in 0..nx {
            ...
            for j in 0..ny {
                let x = (j as f64 / (ny - 1) as f64) * lx - lx / 2.0;
                let sech2 = 1.0 / (y.cosh() * y.cosh());
                field.omega_x[(i, j)] = y.tanh() + rng.gen::<f64>() * 0.01;
                field.omega_y[(i, j)] = rng.gen::<f64>() * 0.01;
                field.omega_z[(i, j)] = 0.1 * (PI * x / lx).sin() * sech2 + rng.gen::<f64>() * 0.01;
            }
        }
        field.nu_eff = config.nu_base;
        field
    ...
    // ✅ Potencial Vetor (Relaxação de Jacobi)
    ...
    // ✅ Evolução MHD Resistiva (Corrigida)
