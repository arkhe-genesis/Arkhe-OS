...
impl HierarchicalRouter {
    ...
    fn route_single(&self, token: &Tensor) -> Vec<RoutingIndex> {
        let group_logits = token.matmul(&Tensor::from(
            self.group_weights.to_ndarray().t().to_owned(),
        )); // [1, 64]
        ...
        for &(group_idx, _) in &top_groups {
            ...
            let expert_logits = token.matmul(&Tensor::from(
                group_expert_weights.to_ndarray().t().to_owned(),
            ));
            let expert_logits_vec = expert_logits.slice_row(0).to_vec();
            let top_experts = self.top_k_indices(&expert_logits_vec, experts_per_group);
    ...
    fn top_k_indices(&self, values: &[f32], k: usize) -> Vec<(usize, f32)> {
        ...
        indexed.into_iter().take(k).map(|(v, i)| (i, v)).collect()
    }
}
