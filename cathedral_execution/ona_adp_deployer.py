...
class ONAADPDeployer:
    def __init__(self, management_api_url: str, api_key: str):
        ...
        self.headers = {"Authorization": "Bearer {}".format(api_key), "Content-Type": "application/json"}
    ...
    def deploy_cathedral(self, adp_path: str, github_repo_url: str) -> dict:
        ...
        payload = {
            "name": "Cathedral-{}".format(int(time.time())),
        ...
        print("[ONA] Enviando manifesto Cathedral para o Management Plane...")
        response = requests.post("{}/api/v1/workspaces".format(self.url), headers=self.headers, json=payload)
        response.raise_for_status()
        workspace_id = response.json().get("id")

        print("[ONA] Workspace criado: {}. Aguardando provisionamento...".format(workspace_id))

        # Polling até o runner estar pronto
        status_url = "{}/api/v1/workspaces/{}/status".format(self.url, workspace_id)
        for _ in range(60): # Timeout de 5 minutos
            ...
            if status_res.get("status") == "ready":
                print("[ONA] Runner pronto! ID: {}".format(workspace_id))
                return status_res
    ...
    def trigger_task(self, workspace_id: str, task_command: str):
        ...
        requests.post("{}/api/v1/workspaces/{}/execute".format(self.url, workspace_id),
