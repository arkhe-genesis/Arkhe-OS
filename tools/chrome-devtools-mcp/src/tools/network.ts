...
export const listNetworkRequests = definePageTool({
  ...
  schema: {
    ...
    semanticPagination: zod
      .boolean()
      .default(false)
      .optional()
      .describe(
        'Post-AGI Semantic Pagination: Groups requests by domain (concept) instead of fixed size.',
      ),
  },
  handler: async (request, response, context) => {
    ...
    if (request.params.semanticPagination) {
      ...
      for (const req of allRequests) {
        ...
        groups.set(url.hostname, (groups.get(url.hostname) || 0) + 1);
      }
      response.appendResponseLine(
        '### Semantic Pagination (Grouped by Concept/Domain)',
      );
      for (const [domain, count] of groups) {
        response.appendResponseLine(`- **${domain}**: ${count} requests`);
      }
      response.appendResponseLine(
        '\nUse "list_network_requests" with "resourceTypes" filter to explore a specific concept.',
      );
    }
