...
impl EthicsEngineMock {
    ...
    ) -> Result<EthicsResultMock, safe_core_ethics::EthicsError> {
        if let Some(harm) = context.get("harm_to_humans").and_then(|v| v.as_bool()) {
            if harm {
                self.violations
                    .write()
                    .await
                    .push(crate::api::ViolationView {
                        constraint_id: "ETH-001".into(),
                    });
                return Ok(EthicsResultMock {
                    allowed: false,
                    type_: "blocked".into(),
                });
            }
