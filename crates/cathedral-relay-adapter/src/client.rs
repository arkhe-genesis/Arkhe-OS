    ...
    pub async fn list_vaults(&self) -> Result<Vec<VaultInfo>> {
        ...
        for v in vaults {
            let info = VaultInfo {
                ...
                total_supply: U256::from_str_radix(v["totalSupply"].as_str().unwrap_or("0"), 10).unwrap_or_default(),
                total_assets: U256::from_str_radix(v["totalAssets"].as_str().unwrap_or("0"), 10).unwrap_or_default(),
                apy: v["apy"].as_f64(),
            };
            result.push(info);
        }
