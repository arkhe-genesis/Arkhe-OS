//! Cathedral ARKHE v28.5.0 — Agente de Teste de Conformidade
//! Verifica se o sistema está em conformidade com políticas de segurança e regulamentações.
...
/// Agente que verifica conformidade com políticas e regulamentações.
...
impl ComplianceTestAgent {
    ...
    /// Verifica se todas as políticas obrigatórias estão ativas.
    ...
    /// Verifica se os atestados incluem as políticas obrigatórias.
    async fn verify_attestations_compliance(&self) -> Result<(usize, Vec<String>), String> {
        ...
        for traj in trajs.iter().filter(|t| t.goal.starts_with("attestation:")) {
            ...
            if let Some(att) = self.attestation_manager.get_attestation(&traj.id).await {
                // Verifica se todas as políticas obrigatórias estão marcadas
