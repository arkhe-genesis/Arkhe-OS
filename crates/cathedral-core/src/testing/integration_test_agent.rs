//! Cathedral ARKHE v28.5.0 — Agente de Teste de Integração
...
/// Agente que testa a integração entre os componentes principais.
...
impl IntegrationTestAgent {
    ...
    /// Testa o ciclo completo: spawn → execute → persist → retrieve.
    ...
    /// Testa a concorrência: múltiplos agentes em paralelo.
...
impl TestAgent for IntegrationTestAgent {
    ...
    async fn run_test(&self, _context: &TestContext) -> Result<TestResult, String> {
        ...
        let all_errors = [cycle_errors.clone(), concurrent_errors.clone()].concat();
        let passed = all_errors.is_empty();

        let details = json!({
            "test_count": self.test_count,
            "cycle_errors": cycle_errors.len(),
            "concurrent_errors": concurrent_errors.len(),
