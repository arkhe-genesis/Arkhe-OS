...
impl PluralityClient {
    ...
    async fn request(&mut self, method: reqwest::Method, path: &str, body: Option<serde_json::Value>) -> Result<reqwest::Response> {
        let url = format!("{}{}", self.base_url, path);
        let auth_header = self.auth.get_auth_header().await
            .map_err(|e| PluralityError::Auth(e))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(PluralityError::Other(format!("Erro HTTP {}: {}", status, text)));
        }
