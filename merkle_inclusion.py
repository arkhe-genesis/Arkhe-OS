...
class MerkleAccumulator:
    ...
    def prove(self, indices: Set[int]) -> Dict:
        ...
        def collect(node_idx, level, start, end):
            ...
            if node_start == node_end:
                if node_start in indices:
                    revealed.append((node_start, self._tree[level][node_start]))
                else:
        ...
        return {
            'revealed': revealed,
            'proof_nodes': [(l, i, self._tree[l][i]) for l, i in proof_nodes],
            'root': self.root,
    ...
    def verify_proof(
        ...
        while len(tree) > 1:
            ...
            processed = set()
            for (l, i) in tree:
                if l != level or i in processed:
                    continue
                sibling_i = i ^ 1
                if (level, sibling_i) in tree:
                    left = tree[(level, i)]
                    right = tree[(level, sibling_i)]
                    if i % 2 == 0:  # i é esquerdo
                        parent_hash = hashlib.sha3_256(left + right).digest()
                    else:
                        parent_hash = hashlib.sha3_256(right + left).digest()
                    next_level_items[i // 2] = parent_hash
                    processed.add(i)
                    processed.add(sibling_i)
                else:
                    # Nó órfão — prova inválida
                    return False
...
class AGILoaderWithMerkle:
    ...
    def load_package_with_merkle(self, package_bytes: bytes):
        """
        Carrega pacote com verificação Merkle de cada artefato.

        Pipeline:
        1. Parse header + manifest (mesmo que antes)
        2. Decifrar payload
        3. Verificar Merkle proof de CADA artefato individualmente
        4. Aceitar apenas artefatos verificados
        """
        manifest = self.load_package(package_bytes)

        # Recriar Merkle tree do payload
        payload_obj = json.loads(
            self._decrypted_payload.decode()
            if hasattr(self, '_decrypted_payload')
            else '{}'
        )

        # Build accumulator from all artifact hashes
        leaf_hashes = []
        artifact_ids = []
        for aid, data_b64 in payload_obj.get('artifacts', {}).items():
            data = base64.b64decode(data_b64)
            decompressed = self._decompress(data)
            leaf_hashes.append(hashlib.sha3_256(decompressed).digest())
            artifact_ids.append(aid)

        accumulator = MerkleAccumulator(leaf_hashes)

        # Verificar que merkle root bate
        if accumulator.root != bytes.fromhex(manifest.merkle_root):
            raise ValueError(
                f"❌ Merkle root mismatch! "
                f"Computed: {accumulator.root.hex()[:32]}... "
                f"Manifest: {manifest.merkle_root[:32]}..."
            )

        # Verificar cada artefato individualmente
        verified_artifacts = []
        all_indices = set(range(len(leaf_hashes)))
        proof_data = accumulator.prove(all_indices)

        verified = MerkleAccumulator.verify_proof(
            all_indices,
            proof_data['revealed'],
            proof_data['proof_nodes'],
            bytes.fromhex(manifest.merkle_root),
            proof_data['total_leaves'],
        )

        if verified:
            log.info("✅ Merkle inclusion proofs verified for all artifacts")
        else:
            raise ValueError("❌ Merkle inclusion proof verification failed")
...
def test_merkle_inclusion():
    ...
    if excl_proof:
        assert excl_proof.verify(b"nonexistent-route")
