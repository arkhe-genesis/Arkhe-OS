...
impl GeometricPolicyEngine {
    ...
    pub async fn authorize(
        ...
        let action_vec_owned;
        let action_vec = match action_emb {
            Some(e) => e.view(),
            None => {
                action_vec_owned = self.geometry.embed(action);
                action_vec_owned.view()
            }
        };

        let output_vec_owned;
        let output_vec = match output_emb {
            Some(e) => e.view(),
            None => {
                output_vec_owned = self.geometry.embed(output);
                output_vec_owned.view()
            }
        };
