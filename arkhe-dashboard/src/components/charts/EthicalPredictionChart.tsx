...
export function EthicalPredictionChart({ currentMetrics, prediction, loading }: { currentMetrics: EthicalMetrics; prediction: PredictionResult | null; loading: boolean }) {
  return (
    <div className="p-4 bg-black/20 rounded-xl border border-white/5">
      {loading ? (
        <div className="animate-pulse text-purple-400">Calculando predição ética...</div>
      ) : prediction ? (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <span className="text-sm text-slate-400">K_Ética Predito</span>
            <span className="text-xl font-bold text-purple-400">{(prediction.predictedKEth * 100).toFixed(2)}%</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-slate-400">Confiança</span>
            <span className="text-xl font-bold text-cyan-400">{(prediction.confidence * 100).toFixed(1)}%</span>
          </div>
        ...
        <div className="text-slate-500 italic">Aguardando dados de telemetria...</div>
      )}
    </div>
  );
}
