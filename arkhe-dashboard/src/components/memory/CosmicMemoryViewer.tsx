  ...
  return (
    <div className="bg-black/40 border border-purple-500/20 rounded-3xl p-6">
      ...
      <div className="flex gap-2 mb-6">
        ...
        />
        <button onClick={handleSearch} className="bg-purple-600 hover:bg-purple-500 text-white px-4 py-2 rounded-xl text-xs font-bold transition-all">
          {isLoading ? '...' : 'BUSCAR'}
        </button>
      </div>
      <div className="space-y-3">
        {results.map((r, i) => (
          <div key={i} className="p-3 bg-white/5 rounded-xl border border-white/5 flex justify-between items-center group hover:border-purple-500/30 transition-all">
            <div className="text-[10px] font-mono text-slate-400 truncate max-w-[150px]">{r.entryId}</div>
            <div className="text-purple-400 font-bold text-[10px]">{(r.quantumSimilarity * 100).toFixed(1)}% SIMILAR</div>
          </div>
        ))}
      </div>
      {stats && (
        <div className="mt-6 pt-6 border-t border-white/5 grid grid-cols-2 gap-4">
          <div>
            <p className="text-[8px] text-slate-500 uppercase tracking-tighter">Entradas Locais</p>
            <p className="text-xs font-bold text-white">{stats.localEntries}</p>
          </div>
          <div>
            <p className="text-[8px] text-slate-500 uppercase tracking-tighter">Nós Federados</p>
            <p className="text-xs font-bold text-white">{stats.federatedNodes}</p>
          </div>
        </div>
      )}
    </div>
