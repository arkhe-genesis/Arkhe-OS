...
class ArkheFHEAdapter:
    """Ponte entre o Parametric Memory Engine e a criptografia homomorfica."""
    ...
    def initialize_keys(self) -> Dict[str, bytes]:
        ...
        logger.info("FHE keys generated: %s, %s", pub_path, priv_path)
    ...
    def build_model_circuit(self, model_path: str) -> bytes:
        ...
        logger.info("Circuit built: %d bytes", len(circuit))
        return circuit

    def blind_inference(self, model_circuit: bytes, encrypted_input: bytes) -> bytes:
        """Executa inferencia cega sobre dados cifrados."""
