...
def generate_seed_corpus(output_dir: str):
    ...
    partial = json.dumps({"name": "test", "ver": "1.0"}).encode()[:15]
...
class AGIFuzzer:
    ...
    def test_property_header_malformed(self, iterations: int = 10000):
        ...
        for i in range(iterations):
            ...
            magic = os.urandom(4)
            version = random.randint(-32768, 32767)  # Int16 range
            artifacts = random.randint(-32768, 32767)
    ...
    def test_property_signature_malleability(self, iterations: int = 5000):
        ...
        for i in range(iterations):
            ...
            mutations = [
                lambda d: bytearray(d).__setitem__(slice(-256, None), os.urandom(256)),
                lambda d: bytearray(d).__setitem__(slice(0, 1280), os.urandom(1280)),
    ...
    def test_property_integer_overflow(self, iterations: int = 5000):
        ...
        for val in extreme_values:
            for _ in range(iterations // len(extreme_values)):
                ...
                fake_manifest = struct.pack('!I', val) + os.urandom(64)
                data += fake_manifest
