...
function normalizeStatus(raw) {
  ...
  if (/^cancelada/i.test(s)) {
    return {status: 'Descartado'};
  }

  // Descartada → Descartado
  if (/^descartada$/i.test(s)) {
    return {status: 'Descartado'};
  }

  // Rechazada → Rechazado
  if (/^rechazada$/i.test(s)) {
    return {status: 'Rechazado'};
  }

  // Rechazado with date → Rechazado (strip date)
  if (/^rechazado\s+\d{4}/i.test(s)) {
    return {status: 'Rechazado'};
  }

  // Aplicado with date → Aplicado (strip date)
  if (/^aplicado\s+\d{4}/i.test(s)) {
    return {status: 'Aplicado'};
  }

  // CONDICIONAL → Evaluada
  if (/^condicional$/i.test(s)) {
    return {status: 'Evaluada'};
  }
  ...
  // Verificar → Evaluada
  if (/^verificar$/i.test(s)) {
    return {status: 'Evaluada'};
  }

  // GEO BLOCKER → NO APLICAR
  if (/geo.?blocker/i.test(s)) {
    return {status: 'NO APLICAR'};
  }
  ...
  if (['enviada', 'aplicada', 'applied', 'sent'].includes(lower)) {
    return {status: 'Aplicado'};
  }
  if (['cerrada', 'descartada'].includes(lower)) {
    return {status: 'Descartado'};
  }
