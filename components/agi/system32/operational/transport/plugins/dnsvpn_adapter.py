#!/usr/bin/env python3
"""
transport/plugins/dnsvpn_adapter.py — Plugin DnsVPN
Integra com DnsVPN via subprocess + socket local.
"""
import asyncio
import subprocess
import socket
...
class DnsVPNAdapter(BaseTransportAdapter):
    """Adapter para transporte via DnsVPN (DNS tunneling)."""
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            print(f"❌ Falha ao iniciar MDnsVPN: {e}")
            return False
    ...
    async def send(self, data: bytes, destination: str,
        ...
        """Envia dados via SOCKS proxy do DnsVPN."""
        try:
            import socks
            ...
            return False, f"DnsVPN send failed: {e}"
    ...
    async def health_check(self) -> Dict[str, float]:
        """Verifica saúde do túnel DnsVPN."""
        import time
        ...
        try:
            ...
            return {
                ...
                'tunnel_status': 1.0 if self._check_socks_port() else 0.0,
            }
        except Exception as e:
            return {
