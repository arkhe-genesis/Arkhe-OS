...
public:
    HedgedReader(int channel_offset = DEFAULT_CHANNEL_OFFSET,
                 ...
                 channel_bit_(channel_bit), channel_offset_(channel_offset), num_channels_(num_channels), logical_index_(0) {
...
private:
    ...
    bool setup_memory() {
        ...
        if (replica_page_ == MAP_FAILED) {
            // Fallback to standard anonymous mapping if Hugepages are not configured/available
            replica_page_ = mmap(nullptr, SUPERPAGE_SIZE, PROT_READ | PROT_WRITE,
                                    MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        }

        if (replica_page_ == MAP_FAILED) {
            perror("mmap (replicas)");
            replica_page_ = nullptr;
            return false;
        }
