...
# Add the parent directory to sys.path so that 'arkhe_os' can be imported
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

from arkhe_os.network.mrc_transport import MRCTransportLayer
...
class TestQuadrumvirato:
    def test_257_qhttp_serialization(self):
        mrc = MRCTransportLayer("node_257", num_planes=4)
        bridge = QHTTPOverMRCBridge("node_257", mrc)
        msg = QHTTPMessage(msg_id="test_001", msg_type=QHTTPMessageType.GRADIENT_SLICE, src_node="node_257", dest_node="node_258",
                           payload=np.array([1.0, 2.0, 3.0]), coherence_signature=0.5, timestamp=datetime.now().timestamp())
        tensor = bridge.serialize_message(msg)
        assert tensor.size > 0 and len(tensor) >= 8

    def test_257_coherence_gate(self):
        mrc = MRCTransportLayer("node_257", num_planes=4)
        for p in mrc.planes: p.coherence = 0.1
        bridge = QHTTPOverMRCBridge("node_257", mrc)
        msg = QHTTPMessage(msg_id="test_002", msg_type=QHTTPMessageType.COHERENCE_PROBE, src_node="node_257", dest_node="node_258",
                           payload=np.array([0.9]), coherence_signature=0.8, timestamp=datetime.now().timestamp())
        result = bridge.send_qhttp_message(msg)
        assert result['status'] == 'REJECTED'

    def test_257_entanglement_channel(self):
    ...
    def test_258_roce_translation(self):
        ...
        roce_pkt = RoCEPacket(opcode=RoCEOpcode.WRITE, src_qp=100, dest_qp=200, r_key=0xABCD, addr=0x1000, length=1024,
                              payload=np.random.randn(256).astype(np.float32), psn=42)
        qhttp_msg = gateway.translate_roce_to_qhttp(roce_pkt, "node_dest")
        assert qhttp_msg.msg_type == QHTTPMessageType.GRADIENT_SLICE

    def test_258_batch_send(self):
        mrc = MRCTransportLayer("node_258", num_planes=4)
        qhttp = QHTTPOverMRCBridge("node_258", mrc)
        gateway = TemporalRoCEGateway("node_258", qhttp)
        packets = [RoCEPacket(RoCEOpcode.SEND, 10, 20, 0, 0, 100, np.array([1.0]), 1),
    ...
    def test_259_node_scoring(self):
        nodes = [NodeProfile("node_a", 100, 80, 0.2, 0.9, {"node_src": 5.0}),
        ...
        score_c = balancer.score_node("node_c", 1024, "node_src")
        assert score_a > score_b

    def test_259_tensor_allocation(self):
        nodes = [NodeProfile("node_a", 100, 80, 0.2, 0.9, {"src": 5.0}),
    ...
    def test_259_rebalance(self):
        nodes = [NodeProfile("node_a", 100, 80, 0.9, 0.9, {"node_b": 5.0}),
        ...
        assert migrations[0]['from'] == 'node_a' and migrations[0]['to'] == 'node_b'

    def test_260_gradient_range_proof(self):
    ...
    def test_260_gradient_norm_proof(self):
        mrc = MRCTransportLayer("node_260", num_planes=4)
        qhttp = QHTTPOverMRCBridge("node_260", mrc)
        zk = MRCZKPrivacyLayer("node_260", qhttp)
    ...
    def test_260_proof_verification(self):
    ...
    def test_260_privacy_transport(self):
    ...
    def test_integration_257_258(self):
        ...
        roce_pkt = RoCEPacket(opcode=RoCEOpcode.WRITE, src_qp=1001, dest_qp=2001, r_key=0xBEEF, addr=0x8000, length=4096,
                              payload=np.random.randn(512).astype(np.float32), psn=1)
        result = gateway.send_roce_over_qhttp(roce_pkt, "hypermesh_node_01")
        assert result['send_result']['status'] == 'SENT'

    def test_integration_258_259_260(self):
        ...
        nodes = [NodeProfile("worker_0", 200, 128, 0.3, 0.95, {"master_node": 3.0}),
        ...
        roce_pkt = RoCEPacket(opcode=RoCEOpcode.WRITE, src_qp=1, dest_qp=2, r_key=0, addr=0, length=gradient.nbytes,
                              payload=gradient[:100], psn=1)
