// parser/frontends/api/coherence/service_calculator.go
package coherence
...
func CalculateSystemCoherence(spec *APISpecification, analysis *APIAnalysis, cfg *APICoherenceConfig) float64 {
...
func CalculateInfrastructureCoherence(infra *InfrastructureSpec, cfg *APICoherenceConfig) float64 {
	...
	patternScores := make(map[ComponentType]float64)
...
func calculateComponentCoherence(comp InfrastructureComponent) float64 {
	score := 1.0
...
func isCriticalComponent(compType ComponentType) bool {
	critical := map[ComponentType]bool{
		TypeDatabase: true, TypeAuthServer: true, TypeAPIGateway: true,
		TypeLoadBalancer: true, TypeServiceMesh: true,
	}
	return critical[compType]
}

func isStatefulComponent(compType ComponentType) bool {
	stateful := map[ComponentType]bool{
		TypeDatabase: true, TypeCache: true, TypeQueue: true, TypeStream: true,
	}
	return stateful[compType]
}

func getComponentCriticality(compType ComponentType) float64 {
	criticality := map[ComponentType]float64{
		TypeDatabase: 1.0, TypeAuthServer: 1.0, TypeAPIGateway: 0.9,
		TypeLoadBalancer: 0.9, TypeServiceMesh: 0.8, TypeCache: 0.7,
		TypeQueue: 0.7, TypeStream: 0.7, TypeCDN: 0.5,
		TypeMetrics: 0.4, TypeLogs: 0.4, TypeTracing: 0.4,
	}
