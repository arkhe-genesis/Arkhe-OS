// parser/frontends/api/models/lfir_mapping.go
package models
...
func (b *APILFIRBuilder) addServiceDependencies(services []Service) {
	// Construir mapa de serviço → endpoints para lookup
	serviceEndpoints := make(map[string][]string)
	for _, svc := range services {
		for _, ep := range svc.Endpoints {
			key := endpointKey(ep)
			serviceEndpoints[svc.Name] = append(serviceEndpoints[svc.Name], key)
		}
	}
	...
	for _, sourceSvc := range services {
		...
		for _, sourceEp := range sourceSvc.Endpoints {
			// Heurística: se o endpoint chama outro serviço (detectar por URL base diferente)
			// Em produção: analisar código ou service mesh config
