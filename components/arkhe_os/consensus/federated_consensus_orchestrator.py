...
class FederatedConsensusOrchestrator:
    ...
    def _initialize_components(self):
        ...
        from .private_metric_aggregator import PrivateMetricAggregator, AggregationStrategy

        # Convert string strategy to enum if necessary
        strategy = self.config.aggregation_config.get('default_strategy')
        ...
        self.audit_ledger = FederatedAuditLedger(
            ...
            key_manager=self.key_manager,
            ledger_path=str(Path(self.config.data_directory) / 'audit_ledger')
        )
