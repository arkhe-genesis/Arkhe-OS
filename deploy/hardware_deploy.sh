...
" || { log "❌ Falha constitucional"; }

# ─── 2. DETECÇÃO DE HARDWARE ───
log "🔍 Detectando hardware..."

# FPGA via PCIe
FPGA_DEV=$(lspci | grep -i "Xilinx" | awk '{print $1}' || echo "")
if [ -z "$FPGA_DEV" ]; then
...
NIC_DEV=$(lspci | grep -i "1969:e0b1\|AR8161\|Killer E2500" | awk '{print $1}' || echo "")
if [ -z "$NIC_DEV" ]; then
...
if [ -d "drivers/" ]; then
    cd drivers/
    make clean && make
    sudo insmod alx-event.ko || { log "❌ Falha ao carregar driver"; }
...
INEOF
    ...
    python3 acquisition_daemon.py --config config.yaml >> "$LOG_FILE" 2>&1 &
    DAEMON_PID=$!
...
sleep 3
if [ "$DAEMON_PID" != "N/A" ] && kill -0 $DAEMON_PID 2>/dev/null; then
    log "✅ Daemon operacional"
else
    log "⚠️ Daemon não está rodando"
fi
