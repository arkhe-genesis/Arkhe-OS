...
class TokenicEngine:
    ...
    def initialize_population(self, size: Optional[int] = None) -> Dict:
        ...
        """
        pop_size = max(2, size or self.population_size)
    ...
    def evolve_generation(self) -> Dict:
        ...
        for i in range(self.population_size):
            ...
            child_prompts = [
                p1 if i % 2 == 0 else p2
                for i, (p1, p2) in enumerate(zip(parent1.prompt_tokens, parent2.prompt_tokens))
            ]
            ...
            child = TokenConfiguration(
                ...
                fine_tune_dataset_hash=hashlib.sha3_256(
                    (parent1.fine_tune_dataset_hash + "-" + parent2.fine_tune_dataset_hash).encode()
                ).hexdigest()[:32],
...
def cmd_evaluate(weights, prompt, node_id):
    ...
    click.echo("  Weight variance: " + "{:.6f}".format(sum(x**2 for x in w)/len(w) if w else 0.0))
    click.echo("  Prompt diversity: " + "{:.2f}".format(len(set(p))/len(p) if p else 0.0))
