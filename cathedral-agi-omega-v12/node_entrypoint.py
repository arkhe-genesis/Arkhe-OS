...
async def sentinel_handler(msg: dict):
    ...
    print("\n[{}] 📨 Mensagem recebida via [{}] de {}".format(NODE_ID, protocol, sender_id))
    ...
    if not BLSKeyPair.verify(message_to_verify, signature, pk):
        print("[{}] 🔒 Assinatura inválida – mensagem rejeitada".format(NODE_ID))
        return

    print("[{}] 🔒 Assinatura válida".format(NODE_ID))
    ...
    if analysis["flagged"]:
        print("[{}] 🚨 Patologia detectada: {} (score: {:.2f})".format(NODE_ID, analysis['state'], analysis['deviation_score']))

        # Aciona ProtocoloCorte
        proposal_id = governance.create_proposal(
            action="ISOLAR_{}".format(sender_id),
        ...
        if governance.is_approved(proposal_id):
            print("[{}] ✅ Proposta aprovada – isolando nó {}".format(NODE_ID, sender_id))
            if rbb:
                rbb.anchor_proposal(proposal_id, "Isolar {}".format(sender_id), sender_id, list(governance.proposals[proposal_id]["votes"].keys()))
        return
    ...
    if protocol == "cathedral/gradient/1.0":
        ...
        if not pouw.verify_gradient_proof(gradient, zk_proof, model_hash, task_id):
            print("[{}] ❌ Prova ZK inválida – gradiente rejeitado".format(NODE_ID))
            return
        print("[{}] ✅ Gradiente verificado via PoUW".format(NODE_ID))

    # 4. Processamento seguro
    print("[{}] ✅ Mensagem aprovada e processada".format(NODE_ID))

# ========== Comportamento Malicioso ==========
async def malicious_behavior(host: CathedralP2PHost):
    """Simula um nó malicioso tentando envenenar a rede."""
    print("[{}] 💀 Iniciando comportamento malicioso".format(NODE_ID))
    ...
    while True:
        ...
        fake_msg = json.dumps({
            ...
            "sender_id": NODE_ID,
            "task_id": "malicious_{}".format(int(time.time()))
        }, sort_keys=True).encode()
        ...
        for addr in host.peers:
            ...
            print("[{}] 💀 Enviado gradiente malicioso para {}".format(NODE_ID, addr))

# ========== Main ==========
async def main():
    print("=" * 60)
    print("  🏛️ CATHEDRAL SENTINEL NODE: {}".format(NODE_ID))
    print("  PoUW: ATIVO | BLS: ATIVO | RBB: {}".format('ATIVO' if rbb else 'NÃO CONFIGURADO'))
    print("  Malicioso: {}".format('SIM 💀' if MALICIOUS else 'NÃO'))
    print("=" * 60)

    host = CathedralP2PHost(listen_addr="/ip4/0.0.0.0/tcp/{}".format(LISTEN_PORT))
    ...
    if ROLE == "connector":
        print("[{}] Aguardando rede...".format(NODE_ID))
        ...
        for target in CONNECT_TARGETS:
            ...
            for attempt in range(5):
                try:
                    ...
                    print("[{}] Conectado a {}".format(NODE_ID, target))
                    break
                except Exception as e:
                    print("[{}] Falha ao conectar a {} (tentativa {}): {}".format(NODE_ID, target, attempt+1, e))
                    await asyncio.sleep(2)

        # Envia gradiente normal (nó honesto)
        print("\n[{}] Enviando gradiente HONESTO...".format(NODE_ID))
        honest_gradient = [0.15, -0.42, 0.88]
        task_id = "honest_{}".format(int(time.time()))
        ...
        print("[{}] Testes concluídos. Encerrando.".format(NODE_ID))
        os._exit(0)
    else:
        print("[{}] Mantendo sentinela ativa...".format(NODE_ID))
        while True:
