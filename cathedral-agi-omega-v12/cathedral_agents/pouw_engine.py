...
class PoUWEngine:
    ...
    def verify_gradient_proof(self, gradient: List[float], proof: bytes, model_hash: str, task_id: str) -> bool:
        ...
        if is_valid:
            ...
            print("[PoUW] Tarefa {} verificada! Saldo: {} tokens".format(task_id, self.token_balance))
    ...
    def submit_work(self, task_id: str, proof: bytes, public_inputs: bytes) -> bool:
        ...
        if task_id in self._submitted_tasks:
            print("[PoUW] Tarefa {} já submetida".format(task_id))
            return False

        is_valid = self.zk_verifier(proof, public_inputs)
        if not is_valid:
            print("[PoUW] Prova inválida para tarefa {}".format(task_id))
            return False
        ...
        print("[PoUW] Tarefa {} aceita! Saldo: {} tokens".format(task_id, self.token_balance))
        return True
